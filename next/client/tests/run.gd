...
# ---- the news ticker (UiNews.gd; docs/CLIENT-2.0.md §30) ----------------------------------------------------------------

func test_news() -> void:
	var math: GDScript = load("res://ui/KUiDialogMath.gd")
	check(math.top_center_anchor(Vector2(312, 30), Vector2(400, 16), Vector2(1024, 768), Vector2(1280, 720)) == Vector2(440, 30), "the news bar stays centred near the top")
	check(math.top_center_anchor(Vector2(312, 30), Vector2(400, 16), Vector2(1024, 768), Vector2(1024, 768)) == Vector2(312, 30), "the theme's own screen keeps the ini position")
	var q: Array = []
	math.news_add(q, "A", 0, 0)
	math.news_add(q, "B", 1, 2)
	check(q.size() == 2 and q[0].text == "B" and int(q[0].count) == 2, "a news joins the front of the list")
	var n: Dictionary = math.news_pick(q)
	check(n.text == "B" and int(n.shown) == 1, "the head is picked first")
	math.news_pass_done(q, n)
	check(q[0].text == "A" and q[1].text == "B", "after a pass the news goes to the end")
	n = math.news_pick(q)
	check(n.text == "A", "then the other news")
	math.news_pass_done(q, n)
	n = math.news_pick(q)
	check(n.text == "B" and int(n.shown) == 2, "a count news comes back for its second pass")
	math.news_pass_done(q, n)
	n = math.news_pick(q)
	check(n.is_empty() and q.is_empty(), "a used-up news is dropped: type 0 after one pass, type 1 after its count")
	math.news_add(q, "C", 1, 0)
	check(int(q[0].count) == 3, "a count of 0 is 3")
	var m := Proto.ScriptAction.new()
	m.set_ui_id(5)
	m.set_count(3)
	var m2 := Proto.ScriptAction.new()
	check(m2.from_bytes(m.to_bytes()) == Proto.PB_ERR.NO_ERRORS and m2.get_ui_id() == 5 and m2.get_count() == 3, "ScriptAction.count round trip")
	var t := Proto.TaskTip.new()
	t.set_text("lenh")
	t.set_kind(1)
	var t2 := Proto.TaskTip.new()
	check(t2.from_bytes(t.to_bytes()) == Proto.PB_ERR.NO_ERRORS and t2.get_kind() == 1, "TaskTip.kind round trip")
