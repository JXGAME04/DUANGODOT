...
- **M13 lát D7 (xong 2026-09-19)**: **hàm vật phẩm của script nhiệm vụ** (`LINUX-SERVER.md` §26): `AddItemEx 0x08120470` → `Lua_NewItem 0x0811F230` (9 số
  `version, seed, quality, genre, detail, particular, level, series, luck` + 6 cấp; **seed ≠ 0 đặt vào bộ sinh ngẫu nhiên trước khi sinh** → cùng seed cùng vật; trả
  chỉ số vật), `GetItemProp` (6 giá trị, thêm luck `+0x200`), `SyncItem`, `RemoveItemByIndex`, `GetItemStackCount`, `GetGlodEqIndex` (`+4 == 1 → +0x80 + 1`),
  `SetItemMagicLevel` (`+0x1e4 + ô·4`), `ITEM_GetItemRandSeed` (`+0x1e0`); `KItem::rand_seed/magic_level/luck` lưu vào role. Test `[scriptfuns][item]`. **Chưa**:
  `GiveItemUI 0x0812BBA0`/`GetGiveItemUnit 0x08114E60` (cửa sổ đưa vật phẩm của client, khối giao dịch `+0x52a4`), PARTNER_*, `AddNote`, quality 1/2 của `AddItemEx`.
- **M13 lát D8 (xong 2026-09-19)**: **hộp đưa vật phẩm cho npc** (`LINUX-SERVER.md` §27, `CLIENT-2.0.md` §27): `GiveItemUI 0x0812BBA0` (gói 0x63 ui 0xb: nội dung +
  tiêu đề, hai hàm trả lời, hàm khi hộp đổi `+0x60a0`, tham số `+0x52b8`, cờ báo đổi `+8`; khối giao dịch chế độ −3), gói 0x89 (`0x080AB980`: ≤ 24 mục `{phòng,x,y,ô}`,
  mã ô `y·6+x+1`, bỏ mục sai, trùng → xoá hết; `kind` 0 → hàm đổi, khác → hàm xác nhận với N rồi xoá trả lời), `GetGiveItemUnit`/`WithPos`, `SetUiGiveItemMsg`/
  `MoreConfirmMsg` (0xd8/0xdf); client `KUiGiveItem` `给予界面.ini` (`UiGiveItem.gd`, hộp 6×4, Đồng ý → `C2G_GIVE_ITEMS`, Hủy → trả lời 1). Test ctest xanh, Godot 518,
  e2e `AUTO_GIVE`. **Chưa**: ràng buộc vật, kéo-thả thật, PARTNER_*, `AddNote`.
- **M13 lát D9 (xong 2026-09-19)**: **ghi chú nhiệm vụ `AddNote 0x08124DC0`** (`LINUX-SERVER.md` §28: gói 0x63 ui 3, chữ | id + int tham số sau chữ, 680 chỗ dùng) và
  **sổ nhật ký "Ký Sự"** của client (`CLIENT-2.0.md` §28: `KUiTaskNote` `任务记事.ini` + 4 trang, `WakeUp 0x004D2550` chèn bản ghi hệ thống đầu danh sách, lưu
  `MissionMemory.dat` → `user://journal_<id>.json`; `UiTaskNote.gd`, mở bằng nút "Nhiệm Vụ" tạm). Godot 522, e2e `AUTO_NOTE`. **Chưa**: đích thật của `Player_Task`,
  hai trang dữ liệu client, PARTNER_*, `AskClientForNumber/String`.
- **M13 lát D10 (xong 2026-09-19)**: **hỏi client số / chuỗi** (`LINUX-SERVER.md` §29, `CLIENT-2.0.md` §29): `AskClientForNumber 0x08115CA0` /
  `AskClientForString 0x08115E90` (gói 0xa3 0x4d byte, hàm vào `+0x5fa0`), trả lời gói 0x82 (`0x081F6830`: kind 3 số → `0x080AC1A0`, kind 2 chuỗi → `0x080ABDF0`,
  xoá 50 ô trước; kind 1 = thuế thành); client **`KUiGetString` `输入字串界面.ini` một hộp cho cả số (9 chữ số, hiện sẵn min, chỉ kiểm ≥ min) lẫn chuỗi (dài
  trong [min, max])** → `UiGetString.gd`; câu báo `G_UiGetString_0..2` từ `\lang\vn\stringtable_client.txt`. ctest, Godot, e2e `AUTO_ASK`. **Chưa**: nơi mở
  `KUiNumberPad` (数字键盘.ini), hộp thông báo `0x004ACBD0`, PARTNER_*, `Player_Task`.
- **S0 kiểm kê hàm script (2026-09-19)** — theo chủ dự án ("bỏ qua đồng hành; kiểm tra hàm để chạy trọn bộ script Linux trước"):
  `tools/script_api_coverage.py` + `tools/re/jx_linux_y.luamap.txt` (1577 hàm C + 174 hàm Lua 4 tĩnh) → `docs/SCRIPT-API.md`: script
  gọi 9 672 tên; zone có 161 (63 806 lượt), **thiếu 866 tên (29 056 lượt)**; 155 tên không có trong nhị phân. Lát S1, S2, … cài
  theo số lượt gọi (`GetLocalDate` 1301, `AddQualityItem` 914, `GetMissionV/SetMissionV` 854/651, `CallPlayerFunction` 691, …).
- **S1 phủ hàm script (xong 2026-09-19)** — §30 `LINUX-SERVER.md`: 20 hàm gọi nhiều nhất không cần gói mới (`GetLocalDate`, `GetCurServerTime`,
  `SubWorldID2Idx/Idx2ID`, `GetBit/SetBit/GetByte/SetByte`, `GetMissionV/SetMissionV`, `GetItemName/GetItemParam`, `GetExp`, 4 hàm điểm mở rộng
  `Player+0x44..`, `CalcFreeItemCellCount`, `SearchPlayer`, `CallPlayerFunction`); phủ 180 có / 847 thiếu.
- **S2 phủ hàm script (xong 2026-09-19)** — §31 `LINUX-SERVER.md`: `AddEventItem`, `AddQualityItem` (quality 0), `CalcEquiproomItemCount`/`ConsumeEquiproomItem`,
  `GetNpcParam`/`SetNpcParam`, `SetNpcScript` (+ tham số `main()`), `DelNpc` (khung kế), `AddNpc`, `AddSkillState` (chế độ 0/1); phủ 190 hàm có (76 090 lượt) / 837 thiếu (16 772 lượt).
- **S3 phủ hàm script (xong 2026-09-19)** — §32 `LINUX-SERVER.md`: `GetSeries`, `GetGameTime`, `ST_GetTransLifeCount`, `SetLogoutRV`, `Get/SetGlbValue`,
  `Get/SetMissionS`, `DynamicExecute(ByPlayer)`, `Msg2SubWorld`/`Msg2Map`, `IsMyItem`, `GetNpcId`, `CalcItemCount`/`ConsumeItem`, `SetPunish`, `DisabledUseTownP`,
  `SetDeathScript`, `SetRank`, `SetNpcTimer` (+ `OnTimer`); phủ 211 hàm có (79 197 lượt) / 816 thiếu (13 665 lượt).
- **S4 phủ hàm script (xong 2026-09-19)** — §33 `LINUX-SERVER.md`: hệ nhiệm vụ bản đồ `KMission` (`settings/task/missions.txt` + `settings/timertask.txt` →
  `jxassets export-missions` → `missions.json`, `zone.mission_file`): `OpenMission`/`RunMission`/`CloseMission`/`JoinMission`, `AddMSPlayer`/`DelMSPlayer`/
  `GetMSPlayerCount`/`GetNextPlayer`/`PIdx2MSDIdx`/`MSDIdx2PIdx`, `Msg2MSAll`/`Msg2MSGroup`, `StartMissionTimer`/`StopMissionTimer`/`GetMSRestTime` (`OnTimer` tuần
  hoàn, script nhiệm vụ chạy không người chơi); phủ 225 hàm có (80 486 lượt) / 802 thiếu (12 376 lượt).
- **S5 phủ hàm script (xong 2026-09-19)** — §34 `LINUX-SERVER.md`: `Set/GetItemBindState`, `SetSpecItemParam`, `GetItemQuality`, `ITEM_Get/SetExpiredTime`,
  `CountFreeRoomByWH`, `AddNpcEx`, `SetNpcDeathScript`, `ForbidEnmity`/`ForbitTrade`/`DisabledStall`/`SetProtectTime`, `SetTmpCamp`/`SetNpcCurCamp`,
  `NpcIdx2PIdx`, `AddStatData`, `AddMagicPoint`, **`AddTimer`/`DelTimer`** (hẹn giờ script gọi `Hàm(tham số, id)`, gia hạn theo giá trị trả),
  `AddNpcSkillState`, `FormatTime2String`, `GetCurrentTime`, `Tm2Time`; phủ 249 hàm có (82 471 lượt) / 778 thiếu (10 391 lượt).
- **S6 phủ hàm script (xong 2026-09-19)** — §35 `LINUX-SERVER.md`: bộ đệm đối tượng `OB_*` (12, `KObjectBufferSet`) + **`RemoteExecute`** (một máy
  chủ → `Hàm(vào, ra)` tại chỗ + callback), `IL` (= `IncludeLib`), `FileName2Id`, `SaveNow`/`SaveQuickly` (lưu ngay khung sau), `WriteGoldLog`,
  `AddMapTrap` (bẫy script thêm vào ô), `NpcChat` (`G2C_NPC_CHAT`, bong bóng client chưa), `CastSkill` (`skill_cast`); phủ 269 hàm có
  (83 676 lượt) / 758 thiếu (9 186 lượt).
- **S7 phủ hàm script (xong 2026-09-19)** — §36 `LINUX-SERVER.md`, §30 `CLIENT-2.0.md`: `PutMessage` (ui 4 → khung tin hệ thống), `AddGlobalNews`/
  `AddGlobalCountNews` (ui 5 phát toàn zone qua `KEvChat.msg_id`; băng tin client = S7b), `SendTaskOrder` (0xb6 trần → kiểu 5); client 2.0 đọc: thông điệp
  0x1f/0x20/0x22, cửa sổ tin `新闻消息来了.ini`, `OpenBox` 0x88, chat npc 0xfb; phủ 273 hàm có (84 036 lượt) / 754 thiếu (8 826 lượt).
- **S7b băng tin toàn cục (xong 2026-09-19)** — §30 `CLIENT-2.0.md`: `UiNews.gd` (`tin-toan-cuc` + `tin-mac-dinh`), luật danh sách `KUiDialogMath.news_*`,
  `top_center_anchor`; `AUTO_NEWS`.
...
| ~~M13 D1 — hộp thoại npc (Say/Talk/trả lời)~~ (xong 2026-09-19, §20 `LINUX-SERVER.md`, §24 `CLIENT-2.0.md`) ~~M13 D2 — giá trị nhiệm vụ (`GetTask/SetTask`…)~~ (xong 2026-09-19, §21 `LINUX-SERVER.md`, §25 `CLIENT-2.0.md`) ~~M13 D3 — hệ nhiệm vụ TASKSYS (`FirstTask/NextTask/GetTaskStatus/SetTaskStatus/StartTask/CloseTask/GetTmpValue/SetTmpValue/TaskXxx`)~~ (xong 2026-09-19, §22 `LINUX-SERVER.md`) ~~M13 D4 — sự kiện giết quái + hàm npc/người chơi~~ (xong 2026-09-19, §23 `LINUX-SERVER.md`) ~~M13 D5 — thư viện `TabFile_*`~~ (xong 2026-09-19, §24 `LINUX-SERVER.md`) ~~M13 D6 — `Describe/TaskTip/WriteLog/GetAccount/AddOwnExp/AddRepute/GetRepute` + bộ đệm chuỗi~~ (xong 2026-09-19, §25 `LINUX-SERVER.md`, §26 `CLIENT-2.0.md`) ~~M13 D7 — hàm vật phẩm `AddItemEx/GetItemProp/SyncItem/RemoveItemByIndex/GetItemStackCount/GetGlodEqIndex/SetItemMagicLevel/ITEM_GetItemRandSeed`~~ (xong 2026-09-19, §26 `LINUX-SERVER.md`) ~~M13 D8 — hộp đưa vật phẩm `GiveItemUI`/gói 0x89/`GetGiveItemUnit[WithPos]`/`SetUiGiveItemMsg`~~ (xong 2026-09-19, §27 `LINUX-SERVER.md`, §27 `CLIENT-2.0.md`) ~~M13 D9 — `AddNote` + sổ nhật ký client~~ (xong 2026-09-19, §28 `LINUX-SERVER.md`, §28 `CLIENT-2.0.md`) ~~M13 D10 — `AskClientForNumber/String` + hộp nhập số/chuỗi `KUiGetString`~~ (xong 2026-09-19, §29 `LINUX-SERVER.md`, §29 `CLIENT-2.0.md`) | **Ưu tiên mới (chủ dự án 2026-09-19): phủ hàm script theo `docs/SCRIPT-API.md` từ trên xuống (S1 xong 2026-09-19 — §30 `LINUX-SERVER.md`; S2 xong — §31; S3 xong — §32; S4 xong — §33 (hệ nhiệm vụ `KMission`); S5 xong — §34 (vật phẩm / npc / cờ / `AddTimer`); S6 xong — §35 (`OB_*`/`RemoteExecute`/`NpcChat`/`AddMapTrap`/`CastSkill`); S7 xong — §36 (`PutMessage`/`AddGlobalNews`/`AddGlobalCountNews`/`SendTaskOrder`); S7b xong — băng tin `UiNews.gd`; **S8 khảo sát bang hội xong (phần 71, §37, `docs/TONG-API.md`) — S8a kế: `KTongSet`/`KTong` + hàm theo khoá**; rồi bang `GetTongName`/`GetTong`/`Msg2Tong`/`TONG_*` (KPlayerTong), `BT_*`, kho `OpenBox`, cửa hàng `Sale`, bạch kim `AddQualityItem` quality 2, …; `python tools/script_api_coverage.py --skip PARTNER_ --out docs/SCRIPT-API.md` sau mỗi lát); PARTNER_* bỏ qua** (đồng hành — khảo sát ở phần 62); còn **M13 D11**: tên subworld/tên npc byte thô, gói 0xa4 (id 0x87), `0xb41 → +0x7cfc`, `Player+0x78ec`; `text_id` của Say (`g_GetStringRes`); `AddNote`, `Describe 0x081242A0`, `AskClientForNumber/String 0x08115CA0/0x08115E90`; gói 0x89 chọn vật phẩm (`0x080AC560`); `+0x158c` tham số npc; sự kiện script 15 (`0x080AEBC0`); chống nghiện `Player+0x7d00`. |
...
### 2026-09-19 (phiên tiếp theo, phần 71) — S8 khảo sát bang hội: bản sao bang của zone, bảng khoá 165 hàm (`docs/TONG-API.md`, §37 `LINUX-SERVER.md`)

- **Đọc được**: máy chủ bang JX2 = relay (`S2SSyncRelayD`; 2003 `S3Relay/KTongSet` + `KTongControl`), zone giữ bản sao `std::map<id, KTong*>` tại
  `0x081B2BD0()+0x24`; mỗi bang: tên `+0x38`, **cây giá trị `+0x768` (`map<khoá word, int>`)** cho mọi thuộc tính lẫn giá trị nhiệm vụ, 256 số tạm
  `+0x798`, thành viên `+0x4fc`, xưởng `TWS_*`; `TONG_Get*` đọc một khoá cố định, `TONG_ApplySet*/Add*` gọi vtable relay `+0x258` (`0xa8` đặt,
  `0xac` cộng…). `KPlayerTong` (`Player+0x5a90`): `+0x28` id thành viên, `+0x2c` con trỏ tên bang chủ (= có bang), `+0x30` chức vụ, `+0x38` id
  tên bang, `+0x3c` tên. `CreateTong` chỉ mở giao diện lập bang (ui 7). Bảng khoá/slot của 165 hàm sinh bằng `scratch/tong_keys.py`
  (dò `cmp word [..+0x10], K; jbe`) → `docs/TONG-API.md`.
- **Kế hoạch S8a** (§37): `KTongSet`/`KTong` trong zone (một tiến trình = quyền uy), `values[khoá]` + số tạm + thành viên + nhật ký + xưởng, lưu JSON
  `zone.tong_file` (DB qua gateway sau), `KPlayerTong` điền lúc vào bản đồ theo tên, ~110 hàm get/set/add theo bảng khoá + hàm người chơi; lập bang
  qua lệnh GM + test; client (cửa sổ bang, ui 7) lát sau.
- commit: `JX NEXT: S8 khao sat bang hoi …`.

### 2026-09-19 (phiên tiếp theo, phần 70) — S7b băng tin toàn cục trên client (`UiNews.gd`, `新闻消息来了.ini`; §30 `CLIENT-2.0.md`)

- **Client**: `UiNews.gd` (`tin-toan-cuc.json`: 400×16 tại (312,30), `Font 14`, `IndentH 11`/`IndentV 2`, màu 255,253,122, viền đen) — `KWndWindow` cắt
  vùng + `KWndText` một dòng; hàng đợi theo luật 2.0 trong `KUiDialogMath` (`news_add` chèn đầu, `news_pick` kiểu 0 một lượt / kiểu 1 `count` lượt (0 → 3),
  `news_pass_done` chuyển cuối); chữ vào từ mép phải vùng 378 px, mỗi `ScrollInterval` 125 ms lùi `(Font+1)/2` rồi `Font/2` (7 px) luân phiên, nội suy
  từng khung (rule 13), hết chữ → lượt xong; trống `MaxIdleTime` 60 s → dòng ngẫu nhiên `tin-mac-dinh` (`\Ui\DefaultMessage.ini`, 17 dòng); neo giữa
  trên (`top_center_anchor`, rule 14). `KUiGameWindows` ui 5 → `add_news(text, param, count)`. `export-ui`: đường dẫn bắt đầu bằng `\` nằm ngoài theme.
- **Test/đo**: Godot 540 (`test_news`: neo, danh sách 2 tin/2 lượt, đếm 0 → 3, proto `count`/`kind`); e2e `AUTO_NEWS` (`?gm ds AddGlobalNews(...)`
  → băng tin hiện, `auto_news.png`).
- **Chưa**: tin kiểu 2 (ngày giờ, `ShowInterval`), `%d` trong tin đếm (`0x004D6470` — đọc chưa khớp), `+0x700` dừng theo bề rộng ký tự (nhịp 2.0 tính
  theo ký tự, ở đây trượt đều 7 px/nhịp), bong bóng lời trên đầu, kho `OpenBox`, cửa hàng `Sale`.
- commit: `JX NEXT: S7b bang tin toan cuc …`.

### 2026-09-19 (phiên tiếp theo, phần 69) — S7 phủ hàm script: `PutMessage`, `AddGlobalNews`/`AddGlobalCountNews`, `SendTaskOrder` + đọc client 2.0 cho ui 4/5, băng tin, kho, chat npc (§36 `LINUX-SERVER.md`, §30 `CLIENT-2.0.md`)

- **Zone** (đọc từng dòng): `PutMessage` (gói 0x63 ui 4, `+5 = 1`, chữ ≤ 0x40, tới chính người chơi `0x080A8510`), `AddGlobalNews` (ui 5, chữ ≤ 0x12c,
  **`0x08077560` = gói relay loại 0x21 đích −1 tới mọi máy chủ**), `AddGlobalCountNews` (`+5 = 1`, dword đếm sau chữ, đếm ≤ 0 → 1), `SendTaskOrder` (gói
  0xb6 trần 0x41 byte, chữ ≤ 0x40). JX NEXT: `broadcast_script_action` → `KChatBroadcast/KEvChat` có `msg_id` → `KGameServer::send_chat` phát tới mọi phiên
  (kênh WORLD sẵn có); `task_tip(kind)`; proto `ScriptAction.count`, `TaskTip.kind`.
- **Client 2.0** (đọc từng dòng): bảng handler 0x63/0x88/0xb6/0xfb; `OnScriptAction` ui 4 → thông điệp 0x1f `SYSTEM_MESSAGE` `{kiểu 2, nháy 2, ưu tiên 0}` →
  khung tin hệ thống; ui 5 → 0x20 `NEWS_MESSAGE` (kiểu 0/1 đếm/2 ngày giờ) → **cửa sổ tin `新闻消息来了.ini`** (`[0x8bf730]`, ctor `0x004D5850`, danh sách
  nút 0x214 byte, Breathe `0x004D69B0`/bước `0x004D6660`/chọn `0x004D6860`/bắt đầu `0x004D63D0` với `%d`, dòng mặc định `\Ui\DefaultMessage.ini`); 0xb6
  không 0x10 → 0x52 `{5, 0x12, 3}`; gói 0x88 `OpenBox` → cờ kho + thông điệp 0x22 `OPEN_STORE_BOX`; gói 0xfb sub 0x2f → `0x00652860` → lời trên đầu
  `0x005EAAB0` + dòng khung chat. Godot: ui 4 → `sys_msg_pane.add_message(text, 2, false, 0)`; `task_tip(text, kind)` → kiểu 5 không nháy; `G2C_NPC_CHAT` →
  thêm dòng `chat_msg` kênh gần mang tên npc; `export-ui` thêm `tin-toan-cuc` (`新闻消息来了.ini`) + `tin-mac-dinh` (`Ui\DefaultMessage.ini`).
- **Test/đo**: `[s7]` 1 ca (50 assertion); ctest 309/309 Release + Debug; Godot 529; e2e xanh. Phủ sau lát: 273 hàm có (84 036 lượt) / 754 thiếu (8 826 lượt).
- **Chưa (S7b)**: vẽ băng tin (`UiNews.gd` theo `tin-toan-cuc.json`: vị trí/khung/font từ ini, bước 4 px luân phiên, số lần, `%d`, dòng mặc định), bong bóng lời
  trên đầu (npc + người chơi), cửa sổ kho (`OpenBox`), `Sale` (cửa hàng), bang hội `TONG_*`, `BT_*`.
- commit: `JX NEXT: S7 phu ham script …`.

### 2026-09-19 (phiên tiếp theo, phần 68) — S6 phủ hàm script: bộ đệm `OB_*` (12) + `RemoteExecute`, `IL`, `FileName2Id`, `SaveNow`/`SaveQuickly`, `WriteGoldLog`, `AddMapTrap`, `NpcChat`, `CastSkill` (§35 `LINUX-SERVER.md`)

- **Bộ đệm đối tượng** (đọc từng hàm): bộ toàn cục `0x9780ce0` = cây id → đối tượng 0x10 byte `{dữ liệu, dung lượng, vị trí đọc, độ dài}`,
  bể + bộ đếm id; `OB_Create/Release/IsEmpty/Clear/Copy/Append`, `OB_Push/Pop Int/Double/Byte/String` (chuỗi = dword `strlen+1` + byte + NUL;
  đọc hết → vị trí đọc về 0; thiếu byte → nil). **`RemoteExecute(script, hàm, bộ đệm[, callback[, tham số]])`**: gói 0x3d qua relay tới máy chủ
  khác; bên nhận `0x08052800` tạo bộ đệm vào (byte gửi) + ra (rỗng), gọi **`Hàm(vào, ra)`** (`0x0804F5B0` → `0x08221ED0`), giải phóng; có callback
  → byte của bộ đệm ra về gói 0x3e → `callback(tham số, bộ đệm)` trong script gọi (`0x08052B00`). JX NEXT một máy chủ: chạy tại chỗ ngay
  (`execute_script_world`, không người chơi) rồi callback với bản sao bộ đệm ra.
- **Còn lại**: `IL` = `IncludeLib` (cùng `0x0812E1D0`) → bí danh trong `KLuaScript::init`; `FileName2Id` → `KScriptCache::id_of` (ô theo thứ tự hỏi);
  `SaveNow` (`Player+0x244 = 1`) / `SaveQuickly` (`+0x248 = 0`) → `KPlayer::save_now` → `take_save_requests` → `KMapInstance::emit_save` khung sau;
  `WriteGoldLog` → dòng log; `AddMapTrap` → `KSubWorld::add_script_trap` (ô theo `to_local`, id `0x40000000+n`, `check_trap` xét sau bẫy bản đồ);
  `NpcChat` → gói **0xfb** (`0x081C9380`) → `G2C_NPC_CHAT` ngay hay sau `giây·18` khung (đối tượng hẹn giờ `0x08139430`, chỉ khi npc còn) —
  client mới log + tín hiệu `npc_chat` (bong bóng của 2.0 chưa đọc); `CastSkill` → `0x0812B1D0` (kỹ năng 1..0x7cf, cấp 1..63, style 0xe/≤4) →
  `skill_cast` (2 đối số = tự thân; `−1` mục tiêu / `−2` hướng / điểm), gói 0x85 không gửi riêng.
- **Test/đo**: `[s6]` 2 ca (146 assertion): đẩy/lấy 4 kiểu + copy/append + hết → nil + release/unknown/1 đối số; RemoteExecute 3 lần
  (`Hàm(vào, ra)` đọc 41/"tin", callback nhận 9 và 42, script ""/2 đối số → 0, bộ đệm 0 → vào rỗng), đếm bộ đệm về đúng; FileName2Id cùng tệp khác
  hoa/thường, ô kế; SaveNow → yêu cầu lưu sid 7 sau tick; NpcChat ngay / sau 18 khung / "" / npc lạ / 1 đối số (trả đối số cuối); AddMapTrap
  (bản đồ khác 0, 3 đối số nil, id lạ 0, id `FileName2Id`, ngoài bản đồ 0) + người chơi bước vào ô → `main()` chạy một lần, ô thứ hai lần hai.
  ctest 308/308 Release + Debug; Godot 529; e2e xanh. Phủ sau lát: 269 hàm có (83 676 lượt) / 758 thiếu (9 186 lượt).
- **CI**: run 257 (`da9214c` nhánh) đỏ ở job **Go ubuntu `go test`** (annotation chỉ còn `FAIL`, log cần đăng nhập); run 258 cùng commit trên
  `main` xanh, `go test ./...` tại chỗ xanh → test Go không ổn định trên Linux, cần theo dõi.
- **Chưa**: bong bóng npc / gói 0x85 client; `AddMapTrap` đối số 5; `OpenBox` 92 (gói 0x88 `{cờ +0x5164/+0x5180/+0x519c, +0x507c/+0x5080}` client),
  `SendTaskOrder` 56 (gói 0xb6 chuỗi 0x40 client); tiếp theo `docs/SCRIPT-API.md`: `Sale` 288 (client), bang `GetTongName` 260 / `GetTong` 107 /
  `Msg2Tong` 94 / `TONG_*`, `BT_*` (`BT_GetGameData` 230…), `AddGlobalNews` 195 / `AddGlobalCountNews` 91 (client ui 5), `LG_*`, `Ladder_*`,
  `ST_SyncMiniMapObj` 78, `Title_*`, `ClearMapNpcWithName` 41.
- commit: `JX NEXT: S6 phu ham script …`.

### 2026-09-19 (phiên tiếp theo, phần 67) — S5 phủ hàm script: 25 hàm vật phẩm / npc / cờ người chơi / hẹn giờ `AddTimer` / đồng hồ (§34 `LINUX-SERVER.md`)

- **Hàm** (đọc từng dòng, §34): `SetItemBindState`/`GetItemBindState` (Item+0x350; gói 0xca `{id, trạng thái + giờ từ 2000 + 168}` chỉ khi > 0 → zone
  `sync_item`), `SetSpecItemParam` (Item+0x1e0+n·4 = mảng `magic_level` của `GetItemParam`), `GetItemQuality` (Item+4 = `ex_type`),
  `ITEM_GetExpiredTime`/`ITEM_SetExpiredTime` (Item+0x34c; phút tương đối ≤ 20 000 000 hay `YYYYMMDD[, HHMMSS]` → `mktime`, quá khứ → −1, 0 = bỏ hạn),
  `CountFreeRoomByWH` (`0x081F8FE0`: đếm hình chữ nhật w×h trống trên bản sao lưới túi, quét cột rồi hàng, dừng khi đủ `cần`), `AddNpcEx` (hệ + phụ bản
  tường minh; phần thêm từ đối số 7 dùng chung `npc_extras` với `AddNpc`), `SetNpcDeathScript` (= `SetNpcScript`, cùng `0x08101500`), `ForbidEnmity`
  (+0x5a5c, cờ == 1), `ForbitTrade` (+0x374), `DisabledStall` (bit 0x800 giá trị nhiệm vụ 135 + gói 0xa4), `SetProtectTime` (+0x86dc), `SetTmpCamp`
  (Npc+0x1900 + gói 0xd1 tới chính người chơi / người xem → `EntityCamp.tmp_camp`), `SetNpcCurCamp` (`SetCurrentCamp 0x0807B850`, phe ≤ 6),
  `NpcIdx2PIdx`, `AddStatData` (bộ đếm), `AddMagicPoint` (Player+0x5928 điểm kỹ năng, gói 0x5e `{0, −1, điểm}` = `G2C_SKILL_LEVEL`), **`AddTimer`/`DelTimer`**
  (đối tượng hẹn giờ 0x114 byte: **tên hàm** trong script đang gọi + tham số; tới khung gọi `Hàm(tham số, id)`; trả 1 số ≠ 0 → chu kỳ mới, 2 số →
  (chu kỳ, tham số mới), không → xoá), `AddNpcSkillState` (= `0x08125D70` với npc bất kỳ, extra 0), `FormatTime2String` (`localtime` + `strftime`),
  `GetCurrentTime` (`time(0)` không lệch), `Tm2Time` (`mktime`, thiếu tháng → 0, ngày → 1).
- **JX NEXT**: proto `ItemData.bind_state/expire_time`, `EntityCamp.tmp_camp` (Go/GD sinh lại); `KItem`, `KInventory::count_free_rects`, `KPlayer` 3 cờ,
  `KNpc::tmp_camp`, `KLuaScript::call_numbers` (LUA_MULTRET), `KSubWorld` hẹn giờ script (`KScriptTimer`, `script_timer_tick` sau `mission_tick`) +
  `add_stat_data` + `set_tmp_camp`; `ScriptFuns.cpp` tách `npc_template_arg`/`npc_extras`/`add_skill_state` dùng chung S2–S5.
- **Test/đo**: `[s5]` 2 ca (192 assertion): cờ, bit 0x800, phe tạm (một gói tới chính mình), điểm kỹ năng 0+5−100 → 0 với hai gói skill 0 / cấp −1,
  thống kê 1+4, `GetCurrentTime` ±5 s, `Tm2Time` 6/3/1/0 đối số, `FormatTime2String` (so với `strftime(localtime)` cùng máy), `AddNpcEx` 9 kiểu gọi
  (cấp âm, hệ nguyên xi, 5 đối số → nil, khác bản đồ / mẫu lạ → 0, boss 1/2, xoá khi chết), `SetTmpCamp`/`SetNpcCurCamp` (7 bị từ chối), `NpcIdx2PIdx`,
  `AddNpcSkillState` −1, hẹn giờ: khung 2 → gọi (11, id), gia hạn `(2, 23)` → `1` → `0` → hết, `DelTimer` 1/0/0; vật: ràng buộc 0→2 (đồng bộ) →0 (im),
  bậc n 1..6, chất lượng, hạn dùng phút / 20200101 → −1 / 20991231 235959 / 0 / −5, túi: `(1,1)` = ô trống, `(1,H)` = W−1 cột, `(W,H)` = 0, `(2,2,3)` = 3,
  lưới 3×3 tay. ctest 306/306 Release + Debug; Godot 529; e2e xanh. Phủ sau lát: 249 hàm có (82 471 lượt) / 778 thiếu (10 391 lượt).
- **Chưa**: gói 0xca / 0xd1 phía client, chỗ đọc ba cờ người chơi, `Npc+0x18e4` (bảng `0x830c7a0`), bộ quản lý hẹn giờ `0x82e8cac` (id khi gia hạn,
  chu kỳ âm); tiếp theo `docs/SCRIPT-API.md`: `Sale` 288 (client), bang `GetTongName` 260 / `GetTong` 107 / `Msg2Tong` 94, `BT_*`, `OB_*` (`OB_Release` 223,
  `OB_Create` 214 — thư viện `remoteexc.lua`), `AddGlobalNews` 195 (client ui 5), `LG_*`, `RemoteExecute` 141, `TONG_*`, `Ladder_*`, `IL` 95, `OpenBox` 92
  (gói 0x88 — đã đọc dở: `Player+0x52a4 = −2`, vị trí npc `0x080EF710`, cờ `+0x5164/+0x5180/+0x519c`, `+0x507c/+0x5080`), `AddGlobalCountNews` 91,
  `ST_SyncMiniMapObj` 78, `CastSkill` 68 (`0x0812B1D0`: kỹ năng 1..0x7cf, cấp 1..63, style 0xe → `0x080EA920`), `NpcChat` 53 (bể đối tượng 0x110 byte
  `0x9780d40` + `KTimerMgr::Add` với `frames·18`), `AddMapTrap` 62 (`KSubWorldSet::GetSubWorldIdx 0x080F68A0` + `0x080EFCF0(vùng, x, y, script)`).
- commit: `JX NEXT: S5 phu ham script …`.

### 2026-09-19 (phiên tiếp theo, phần 66) — S4 phủ hàm script: hệ nhiệm vụ bản đồ `KMission` — 15 hàm `OpenMission`/`RunMission`/`CloseMission`/`JoinMission`/`AddMSPlayer`/`DelMSPlayer`/`GetMSPlayerCount`/`GetNextPlayer`/`PIdx2MSDIdx`/`MSDIdx2PIdx`/`Msg2MSAll`/`Msg2MSGroup`/`StartMissionTimer`/`StopMissionTimer`/`GetMSRestTime` (§33 `LINUX-SERVER.md`)

- **Mô hình** (mã 2003 `Core/Src/KMission.h` + nhị phân, đọc từng hàm): mỗi bản đồ một mảng nhiệm vụ `SubWorld+0x60 + i·0x73a0` (`0x08135527`); một nhiệm vụ = id
  (`+0x190`, `0x08138135`), danh sách người chơi `TMissionPlayerInfo` 24 byte từ `+0x308` (nhóm ở `+8`, `0x0813556B`; danh sách ô dùng `+0x7394`/đếm `+0x739c`; ô
  **1-based**, ô trống thấp nhất dùng lại), 3 hẹn giờ `KTimerTaskFun` 20 byte từ `+0x1a4` (danh sách `+0x1ec`, `0x082096B0`). Bảng `\settings\task\missions.txt`
  (69 dòng `MISSION\tSCRIPT`; `KTabFile 0x9780b00` dòng id+1 cột 2 — `0x0813313A`/`0x0813816E`) + `\settings\timertask.txt` (114 dòng `TASK\tSCRIPT`). **Mọi lời
  gọi vào script nhiệm vụ không có người chơi** (`0x082095A0`/`0x08209520` = 2003 `KMission::ExecuteScript`, chỉ đặt global `SubWorld`): `InitMission(0)` (OpenMission
  `0x0813377E`), `RunMission(0)` (`0x08133150`), `EndMission(0)` (CloseMission `0x08132B0A`), `JoinMission(người chơi, nhóm)` (`0x08138266` — script tự `AddMSPlayer`),
  `OnLeave(người chơi)` (2003 `RemovePlayer`), `OnTimer(0)` của script hẹn giờ (`0x08209610`). Hẹn giờ `0x08209670` → `KTimerTaskFun::Activate 0x080F9480`: tới khung →
  đặt lại `đích = khung + chu kỳ` (`0x080F94A6`) rồi gọi (`0x080F94A9`) — tuần hoàn tới `StopMissionTimer`; chu kỳ 0 = đóng ô; tối đa 3 ô.
- **JX NEXT**: `KMission.h/.cpp` (`KMissionTable` ← `missions.json`; `KMission` add/remove/data_index/entry/next_player/player_count/players/start_timer/stop_timer/
  rest_time/fire_timers/clear), `jxassets export-missions` (+ `tools/dev.py assets`; GBK → UTF-8; **chủ dự án chạy lại `python tools/dev.py assets`**),
  `zone.mission_file` (`config/zone.json`, mặc định `client/assets/missions.json`; thiếu → nhiệm vụ mở không script), `KSubWorld::open_mission/run_mission/close_mission/
  join_mission/mission_remove_player/mission_message/execute_script_world/mission_tick` (sau `flush_pending_removes`; gom hẹn giờ tới lúc rồi mới gọi `OnTimer`),
  log `zone.mission`.
- **Test/đo**: `[mission]` 2 ca (115 assertion): `KMission` thuần (ô 1-based, ô trống dùng lại, id người chơi 0 bị từ chối, 3 hẹn giờ, chu kỳ, `SetTimer(0)` đóng,
  đếm ngược, dừng, xoá) và API qua Lua (mở / mở lại / dòng không script / id lạ / id âm, `RunMission`, `AddMSPlayer` 2–3 đối số, `GetMSPlayerCount` theo nhóm,
  `GetNextPlayer` hai kết quả + `top < 2` → `0, 0` + `nil` đếm như 0, `PIdx2MSDIdx`/`MSDIdx2PIdx`, `Msg2MSAll`/`Msg2MSGroup` đếm gói CH_SYSTEM từng người chơi,
  `StartMissionTimer` 3 khung → `OnTimer(0)` tuần hoàn, `Stop`, id hẹn giờ không script, chu kỳ 0, `DelMSPlayer` → `OnLeave(người chơi)`, `JoinMission` →
  `JoinMission(người chơi, nhóm)`, `CloseMission` → `EndMission` + xoá). ctest 304/304 Release + Debug; e2e xanh (8 bước AUTO_). `export-missions` thật: 69 nhiệm vụ +
  114 hẹn giờ. Phủ sau lát: 225 hàm có (80 486 lượt) / 802 thiếu (12 376 lượt).
- **Chưa**: `SetPMParam`/`GetPMParam`, nhiệm vụ toàn cục (`OpenGlbMission`… ≤ 3 lượt), chỗ gọi `RemovePlayer` khi thoát / đổi bản đồ chưa dò; tiếp theo `docs/SCRIPT-API.md`:
  `Sale` 288 (client), `GetTongName` 260 / `GetTong` 107 / `Msg2Tong` 94 (bang), `BT_*` (`BT_GetGameData` 230, `BT_SortWorldLadder` 230, `BT_GetData`/`BT_SetData` 215),
  `OB_*` (`OB_Release` 223, `OB_Create` 214), `AddGlobalNews` 195 (client ui 5), `SetItemBindState` 183, `AddNpcEx` 170, `LG_GetLeagueTask` 166, `SetSpecItemParam` 159,
  `RemoteExecute` 141, `TONG_GetTaskValue` 141, `ITEM_SetExpiredTime` 136, bạch kim `AddQualityItem` quality 2.
- commit: `JX NEXT: S4 phu ham script …`.

### 2026-09-19 (phiên tiếp theo, phần 65) — S3 phủ hàm script: 21 hàm số liệu / giá trị toàn cục / chuỗi nhiệm vụ / DynamicExecute / dòng hệ thống / cờ / hẹn giờ (§32 `LINUX-SERVER.md`)

- **Hàm**: `GetSeries`, `GetGameTime` (khung online ×20/18 — `KPlayer::login_tick`), `ST_GetTransLifeCount`, `SetLogoutRV`, `GetGlbValue`/`SetGlbValue` (5001 số
  toàn cục, atomic), `GetMissionS`/`SetMissionS` (100 chuỗi/bản đồ), `DynamicExecute`/`DynamicExecuteByPlayer` (gọi hàm của script khác — `KLuaScript::push_function`
  hiểu `a.b`/`a:b`; chỉ một số trả về qua hai state), `Msg2SubWorld`/`Msg2Map` (`KSubWorld::msg_to_all`, CH_SYSTEM), `IsMyItem`, `GetNpcId`, `CalcItemCount`/
  `ConsumeItem` (phòng −1/3/0/1 → `room_arg`; `bag_matches`/`consume_matches` dùng chung với S2), `SetPunish` (+0x1818 = 0/3), `DisabledUseTownP` (bit 0x100000 của
  giá trị nhiệm vụ 135), `SetDeathScript` (+`player_corpse` gọi `OnDeath(kẻ đánh cuối)`), `SetRank` (+0x30, gói 0xa5 chưa gửi), `SetNpcTimer` (+0x19a8; vòng tick chạy
  `OnTimer(npc)` của script npc khi tới khung).
- **Sửa nhân tiện**: `AddNpc` (S2) đặt lại `ctx.player` sau `spawn_npc` — `EntityTable` là vector nên chèn có thể dời con trỏ người chơi của script (test S3 bắt được:
  `SetPunish` sau `AddNpc` ghi vào bộ nhớ cũ); mối nguy chung "spawn trong tick" vẫn còn ở §0.4.
- **Test/đo**: `[s3]` 2 ca (112 assertion): chuỗi nhiệm vụ, giá trị toàn cục biên 0/5000/5001, DynamicExecute ""/đường dẫn/`Lib:Ten`/`Lib.Twice`/script lạ/hàm lạ,
  ByPlayer đổi người chơi rồi trả lại, hai dòng hệ thống tới cả hai người chơi (dòng tới bản đồ lạ không đi), bit 135, hạng, hẹn giờ 3 khung → `OnTimer(id npc)`,
  `KillPlayer` → khung chết → `OnDeath(kẻ đánh cuối)`; vật: `IsMyItem`, phòng −1/3/0/lạ, 4–5 đối số → 0/−1. ctest 302/302 Release + Debug; e2e xanh (8 bước AUTO_ kể cả AUTO_DEATH). Phủ sau lát: 211 hàm có (79 197 lượt) / 816 thiếu (13 665 lượt).
- **Chưa**: nhóm nhiệm vụ (`GetNextPlayer`, `GetMSPlayerCount`, `Msg2MSAll`, `Open/Run/Close/JoinMission`, `StartMissionTimer`…), gói 0xa5 hạng, thần hành phù đọc
  bit 0x100000, `player_default_death.lua` mặc định, chỗ đọc `Player+0x40`, bạch kim (S3 đổi thành S4), `AddGlobalNews`, `Sale`, `GetTongName`, `BT_*`/`OB_*`.
- commit: `JX NEXT: S3 phu ham script …`.

### 2026-09-19 (phiên tiếp theo, phần 64) — S2 phủ hàm script: 10 hàm vật phẩm / npc / trạng thái (§31 `LINUX-SERVER.md`)

- **Hàm**: `AddEventItem` (485 lượt), `AddQualityItem` (914 — chỉ quality 0; quality 2 bạch kim để S3), `CalcEquiproomItemCount`/`ConsumeEquiproomItem` (443/426 —
  `Item+0x308` là số trong chồng của vật chồng được, `+0x14` cờ chồng), `GetNpcParam`/`SetNpcParam` (371/341 — `Npc+0x19b0..` 10 số), `SetNpcScript` (254 — kèm
  `+0x158c` = tham số `main()`), `DelNpc` (203 — nút 0x3e9, xoá ở khung kế), `AddNpc` (289 — x, y đơn vị thế giới, hệ `rand()%5`, boss 1/2), `AddSkillState` (510 —
  chế độ 0/1; chế độ 2 theo ngày chưa làm). Mới trong zone: `KNpc::script_param[10]`, `script_main_param` (dialog_npc/NpcDialog truyền `main(param)`),
  `KNpcTemplateSet::find_by_name`, `KSubWorld::find_npc_by_name`, `queue_remove_npc`/`flush_pending_removes`, `make_gold_npc`; log `npc added`, `npc script set`,
  `npc removed by script`, `AddNpc failed`, `AddQualityItem: too few arguments`, `AddSkillState: date mode not built` (+ trường `this_map`, `time`).
- **Test/đo**: `[s2]` 3 ca (103 assertion): vật sự kiện theo detail/tên lạ, quality 0 → "Kiem 2" / quality 2 → 0 / thiếu đối số → 0, đếm theo genre bắt buộc và −1 mở,
  chồng 5 thuốc đếm 5, tiêu cả vật rồi hết → −1, tiêu 0 giữ nguyên, tách chồng 5→2→0; AddNpc theo số/tên/thiếu đối số/bản đồ khác/mẫu lạ, cấp âm → 1, boss 2 → `gold.is_gold`,
  tham số 1..10, script + tham số, DelNpc theo chỉ số/tên/người chơi rồi `tick` mới mất; AddSkillState −1 khi sai chế độ/thời gian/kỹ năng, chế độ 0/1 trên kỹ năng
  kiểu 2 của arena test_KAutoSkill. ctest 300/300 Release + Debug; e2e xanh (7 bước AUTO_ như trước). Phủ sau lát: 190 hàm có (76 090 lượt) / 837 thiếu (16 772 lượt).
- **Chưa (S3…)**: bạch kim `AddQualityItem` quality 2 (bảng `set+0x1d14`, `0x080663D0`), `AddSkillState` chế độ 2, bảng tên npc kind 3 (`0x830c7a0`), AddNpc sang
  bản đồ khác của zone, `Msg2MSAll`/`AddGlobalNews` (cần gói + client), `Sale`, `GetTongName` (bang hội), `BT_*`/`OB_*`.
- commit: `JX NEXT: S2 phu ham script …`.

### 2026-09-19 (phiên tiếp theo, phần 63) — S1 phủ hàm script: 20 hàm gọi nhiều nhất không cần gói mới (§30 `LINUX-SERVER.md`)

- **Hàm**: `GetLocalDate` (1301 lượt), `GetCurServerTime`, `SubWorldID2Idx`/`SubWorldIdx2ID`, `GetBit`/`SetBit`/`GetByte`/`SetByte`, `GetMissionV`/`SetMissionV`,
  `GetItemName`/`GetItemParam`, `GetExp`, `GetExtPoint`/`AddExtPoint`/`AddExtPointForGS`/`PayExtPoint`, `CalcFreeItemCellCount`, `SearchPlayer`, `CallPlayerFunction` —
  mỗi hàm đọc tới `ret` (địa chỉ và luật trong §30). Mới trong zone: `KSubWorldConfig.time_offset`; `KSubWorld::mission_values_[100]`, `hosted_maps_`
  (`KGameServer` đặt), `find_player_by_name`; `KInventory::free_cells`; `KPlayer::ext_point[8]` (+`add_ext_point`/`pay_ext_point`, lưu `RoleStats.ext_point = 17`,
  proto sinh lại Go/GD); log `ext point`, `call player function failed` (+ trường `index`, `gs`, `left`).
- **Quy ước**: chỉ số người chơi = id thực thể; chỉ số phụ bản = id bản đồ; `GetLocalDate` chỉ nhận định dạng C99 (CRT Windows huỷ với ký tự lạ);
  `SearchPlayer`/`CallPlayerFunction` chỉ thấy người chơi trên bản đồ của script (tìm khắp zone → cần `KGameServer`, để sau).
- **Test/đo**: `[s1]` 2 ca (153 assertion: bit/byte kể cả −1 → 4294967294, biến nhiệm vụ 0..99, id phụ bản, đồng hồ ±5 s và năm, điểm mở rộng + lưu,
  ô trống túi, `SearchPlayer`, `CallPlayerFunction` đổi người chơi (GetName → "B") và trả nhiều giá trị, `GetItemName`/`GetItemParam` trên vật thật);
  ctest 297/297 Release + Debug; Godot 529; e2e xanh (AUTO_TRADE/DIALOG/TASK/DESCRIBE/GIVE/NOTE/ASK như trước). Phủ sau lát: **180 hàm có (71 854 lượt) / 847 thiếu (21 008 lượt)** (`docs/SCRIPT-API.md` sinh lại).
- **Kế tiếp (S2)**: `AddQualityItem` (914), `AddEventItem` (485), `CalcEquiproomItemCount`/`ConsumeEquiproomItem` (443/426), `GetNpcParam`/`SetNpcParam`
  (371/341), `Msg2MSAll` (369, cần gói), `AddNpc`/`DelNpc`/`SetNpcScript` (289/203/254), `Sale` (288, cần client), `GetTongName` (260, bang hội chưa có),
  `AddSkillState` (510), `BT_*`/`OB_*`.
- commit: `JX NEXT: S1 phu ham script …`.

### 2026-09-19 (phiên tiếp theo, phần 62) — khảo sát đồng hành (dừng theo chủ dự án) + S0 kiểm kê hàm script

- **Chủ dự án**: "Bỏ qua phần đồng hành. Bạn kiểm tra phần hàm để có thể chạy full script bản linux trước." → PARTNER_* để lại, ưu tiên phủ hàm script.
- **Đồng hành — đã đọc trước khi dừng** (để lát sau khỏi đọc lại): 48 hàm `PARTNER_*` + `GetPartnerBagLevel 0x081143F0`/`SetPartnerBagLevel 0x0811A7B0`/
  `PartnerCallout 0x08260440`…; `Player+0x8078` = `KPartnerSet` (vector `KPartner*` tại `+0x807c..+0x8080`, chỉ số hiện tại `+0x8088` (0-based, script thấy +1),
  `+0x808c`); `KPartnerSet::Add 0x08162B30`: tối đa **20** (`count ≤ 0x13`), gọi `partner->vtable[3](set)`, đếm `[0x8badfc4]`; `PARTNER_AddFightPartner 0x0816C400`:
  9 đối số `(a1, a2, a3, 6 tư chất)` → `0x08166520` (`0x08167B70(a1, 0)` kiểm a1 trong cây `0x97865e0`, `0x08167840`/`0x08167510` sinh thuộc tính, `0x08165750` tạo)
  hoặc 4 đối số `(a1, a2, a3, kiểu tư chất)` → `0x08166790` → `0x08167C70` quay 6 tư chất theo `aptitude_mode.txt` (`0x816bcc0` tìm dòng, `0x81674b0` quay 10 nấc);
  script `Pet_Create(nSeries)` gọi `(5-nSeries, nSeries, 1, 5,5,5,5,5,5)`. Đối tượng: `+4 = 1` (đồng hành chiến đấu), `+0xc` = khối dữ liệu (`+0x10` feature,
  `+0x14` tính cách, `+0x20` cấp, `+0x28` hệ, `+0x128` map giá trị nhiệm vụ, `+0x148` bền, `+0x158/+0x15c` sinh/ngày sinh, `+0x1505` tên, `+0x1530` chỉ số cấu hình),
  `obj+0x18` exp. Bảng `settings/partner/`: `aptitude_mode.txt` (2 kiểu × 6 tư chất × 10 nấc %), `aptitude_range.txt` (10 nấc min/max), `attrib_range.txt`
  (5 hệ: init/increment sinh mệnh, sức, chính xác, thủ, tốc, may), `character.txt` (4 tính cách = AI 21..24), `feature.txt` (5 diện mạo × 3 kỳ npc + giới),
  `level_exp.txt` (100 cấp), `init_skill.ini`, `resist.txt`, `skill_requirement.txt`, `partner_setting.ini` (thân mật 0..100 khởi 50, giảm mỗi 7200 s
  quá 50; phạt chết 300 s; gọi cách 30 s; hồi 200/10000 mỗi 5 s), `partner_bag.ini`, `partner_event.ini` (31 sự kiện thoại), `talk/*.txt`.
- **S0**: `tools/script_api_coverage.py` (đối chiếu luamap ↔ `ScriptFuns.cpp`/`KLuaScript.cpp` ↔ lời gọi trần trong `data/script`, bỏ định nghĩa trong
  script, tách thư viện Lua 4 tĩnh ≥ 0x08230000), `tools/re/jx_linux_y.luamap.txt` đưa vào kho, `docs/SCRIPT-API.md` sinh ra (đừng sửa tay),
  `docs/SCRIPTS.md` §6 trỏ tới. Số đo: có 161 / thiếu 866 tên (29 056 lượt), 155 tên không có trong nhị phân (script gốc cũng lỗi).
- **Kế tiếp (S1…)**: đọc và cài theo bảng "Còn thiếu" từ trên xuống, mỗi lát 10–20 hàm, test C++ cho từng hàm, cập nhật báo cáo bằng công cụ.
- commit: `JX NEXT: S0 kiem ke ham script …`.

### 2026-09-19 (phiên tiếp theo, phần 61) — M13 lát D10: hỏi client một số / một chuỗi (AskClientForNumber 0x08115CA0 / AskClientForString 0x08115E90 gói 0xa3, trả lời gói 0x82 0x081F6830; client KUiGetString 输入字串界面.ini — một hộp cho cả số lẫn chuỗi)

- **jx_linux_y** (`LINUX-SERVER.md` §29): hai hàm đặt `+0x78e8 = 0` trước khi kiểm đối số, ghi hàm vào `+0x5fa0` (`+0x78e4 = 0`, `+0x78e8 = 1`), gói 0xa3 0x4d byte
  `{0xa3, word 0x4c, 2, kind, tiêu đề[0x20], int min, int max, mặc định[0x20]}`; trả lời 0x82: `[+3]` 3 → `0x081F6680` gọi hàm với int (`0x080AC1A0`), 2 → `0x081F6730`
  gọi hàm với chuỗi (`0x080ABDF0`, chuỗi rỗng bỏ), cả hai xoá 50 ô trả lời trước (`0x080A7E60`); **kind 1 là đặt thuế thành** (`0x08058700`), không phải script.
- **gamecl.exe** (`CLIENT-2.0.md` §29): 0xa3 sub 2 `0x006AC330` → thông điệp 0x37 → `0x0051CD00` = OpenWindow của **`KUiGetString`** (RTTI 0x8120dc): `Init 0x0051CA10`
  chỉ thêm 4 con (tiêu đề, ô nhập, Đồng ý, Hủy bỏ), `0x0051C690` nạp `输入字串界面.ini` — **cùng một hộp cho số và chuỗi**; số: ô nhập tối đa 9 ký tự, hiện sẵn min
  (`0x00456890`), Đồng ý `0x0051C800` chỉ kiểm `giá trị ≥ min` (không kiểm max) rồi callback(0x501, mode 3, giá trị) → `0x006AC510` `{0x82, 7, 3, int}`; chuỗi: dài
  trong [min, max] → `0x006AC470` `{0x82, dài+5, 2, word dài, chữ}`; từ chối → hộp thông báo `0x004ACBD0` với `G_UiGetString_0/1/2` của `\lang\vn\stringtable_client.txt`.
  `数字键盘.ini` (`KUiNumberPad`, cửa sổ 0x8c0394) là cửa sổ khác, chưa dò nơi mở.
- **Zone**: `dialog_ask_client`, `script_input_request` (`KSubWorldDialog.cpp`), 2 hàm Lua, proto `ScriptAsk`/`ScriptInput` + `G2C_SCRIPT_ASK 2144`/`C2G_SCRIPT_INPUT 1125`,
  gateway/KGameServer/KMapInstance, `log.vi.json` +3 câu +3 trường. **Client**: `export-ui` +1 (`nhap-chuoi`), `UiGetString.gd` (số/chuỗi, giữa màn hình, luật 14),
  `KWndEdit.set_limits(max, chỉ số)`, `KUiDialogMath.ask_number/ask_check_number/ask_check_text/ask_reject_text` (+3 câu báo Unicode), `KUiGameWindows._on_script_ask`
  / `_on_ask_rejected` (bảng thông điệp hệ thống thay hộp thông báo 2.0 chưa có), `KProtocolProcess.script_ask/script_input`, `_auto_ask`.
- **Test/đo**: `[dialog]` +1 ca (0xa3 số/chuỗi, 0x82 kind 3 → hàm(42), kind 2 → hàm("Dai Hiep"), rỗng bỏ, kind 1 bỏ, đối số sai không gửi; `lua_isstring` nhận số làm tên
  hàm như Lua 4); ctest 295/295 Release + Debug; Godot 529; e2e `AUTO_ASK kind=1 title=Nhap so luong min=1 max=100 window=true numeric=true initial=1 kept_below_min=true typed=42 confirmed=true` + zone `script ask` → `script input ignored kind=3 function=OnNumber` (không có script trong ngữ cảnh gm), các AUTO_ khác xanh.
- **Lỗi đã mắc (sửa trước khi commit)**: lát này lúc đầu nối gói 0xa3 kind 1 với **bàn phím số `数字键盘.ini`** (viết cả `UiNumberPad.gd`, e2e chụp được) — ảnh nền
  `小键盘界面底板.spr` không có ô cho `[Back]/[00]` nên đọc lại `Init 0x0051CA10`: chỉ 4 con và nạp `输入字串界面.ini` → xoá `UiNumberPad.gd` + hàng `ban-phim-so`,
  `UiGetString.gd` thêm chế độ số. Bài học: **đọc `Init` của cửa sổ (số con + tên mục) trước khi tin tên tệp ini trong RTTI/chuỗi**. Ngoài ra: test Lua dùng
  `strlen` (Lua 4) — zone là Lua 5.4 → `string.len`; `lua_isstring` nhận số nên ca "số làm tên hàm" phải bỏ; log `script input ignored` là debug nên e2e chạy zone với
  `JX_LOG__LEVELS=zone.dialog=debug` để thấy gói tới (`?gm ds` không có script → bỏ đúng luật).
- **Chưa**: chuỗi gọi từ callback 0x8468b0 tới `0x006AC510/0x006AC470` (không có xref trực tiếp), nơi mở `KUiNumberPad`, hộp thông báo `0x004ACBD0`
  (`KUiInformation`), PARTNER_*, đích nút `Player_Task`.
- commit: `JX NEXT: M13 lat D10 - …`.

### 2026-09-19 (phiên tiếp theo, phần 60) — M13 lát D9: ghi chú nhiệm vụ AddNote 0x08124DC0 (gói 0x63 ui 3) + sổ nhật ký "Ký Sự" của client (KUiTaskNote 任务记事.ini, WakeUp 0x004D2550, MissionMemory.dat → user://journal_<id>.json)

- **jx_linux_y** (`LINUX-SERVER.md` §28): `AddNote(chữ | id[, tham số])`: chỉ số ≥ 0, `top > 0`; số → id chuỗi (`+6 = 1`, nội dung `{id, tham số}` 8 byte); chuỗi → chữ + int
  tham số ghi đè NUL (`+0xd = strlen + 4`); gói `+4 = 3`, `+7 = 1`; 680 chỗ dùng. PARTNER_* trong `task_main.lua` chỉ 3 chỗ (`GetCurPartner/GetSettingIdx/GetName`) —
  hệ đồng hành chưa có, để sau.
- **gamecl.exe + mã 2004** (`CLIENT-2.0.md` §28): ô 3 `0x00601058` → thông điệp 0x24 `GDCNI_MISSION_RECORD` → `KUiTaskNote::WakeUp 0x004D2550` chèn bản ghi hệ thống
  `{time, giá trị, chữ}` vào đầu danh sách (`KTaskDataFile::InsertSystemRecord`), lưu `MissionMemory.dat` ngay cả khi cửa sổ đóng; `任务记事.ini` khung 420×287 + 4 trang
  (`系统任务分页` danh sách + "Xóa", `个人记事分页` ô soạn 2000 ký tự + "Lưu", `游戏更新记录`, `装备绑定`); mở qua `Switch([[tasknote]])` (bảng tên `0x80db08`); nút
...
### 2026-09-19 (phiên tiếp theo, phần 59) — M13 lát D8: hộp đưa vật phẩm cho npc (GiveItemUI 0x0812BBA0 gói 0x63 ui 0xb, gói 0x89 0x080AC560/0x080AB980, GetGiveItemUnit 0x08114E60 / WithPos 0x08114D90, SetUiGiveItemMsg 0x0810B020 / MoreConfirm 0x0810AF50; client KUiGiveItem 给予界面.ini)

- **jx_linux_y** (đọc từng dòng, `LINUX-SERVER.md` §27): `GiveItemUI` gửi ui 0xb với nội dung + tiêu đề sau NUL, `+8` = đối số 7 (báo đổi), `+9` = đối số 5; không có hàm
  xác nhận → không gửi; hai hàm trả lời vào `+0x5fa0/+0x6020` (`+0x78e4 = 2`), đối số 6 vào `+0x60a0`; khối giao dịch đặt vào chế độ đưa vật (`+0x52a4 = −3`). Gói 0x89:
  `0x080AB980` kiểm (không giao dịch, chế độ −3, cỡ = N·5 + 10, N ≤ 24, mỗi mục `{phòng,x,y,ôX ≤ 5,ôY ≤ 3}` → vật tại chỗ `0x081FB0D0`, bỏ mục sai chỗ/ô; vật khoá khi
  tham số 5 = 0 → xoá hết; trùng vật/ô → xoá hết) ghi `+0x2b4[]`/`+0x314[]` (mã ô `y·6 + x + 1`); `kind == 0` → hàm `+0x60a0` trong script npc với N (`0x080AC400`),
  khác → hàm xác nhận trong script hộp thoại với N, xoá 50 ô trả lời (`0x080AC4B0`). `SetUiGiveItemMsg`/`MoreConfirmMsg`: gói `{0xd8|0xdf, int dài+9, int 0, chữ}`.
- **gamecl.exe** (`CLIENT-2.0.md` §27): ui 11 `0x0060194C` → thông điệp 0x3e → `KUiGiveItem::OpenWindow 0x00519C80` (`给予界面.ini`: `[Items]` 6×4 = 24 ô, "Đồng ý"/"Hủy bỏ",
  `[ContentList]`/`[Title]`/`[LabelTitle]` "Giao Nộp"); vật vẫn ở túi, hộp nhớ danh sách (core `GetGameData 0x7da`), "Đồng ý" → 0x89, huỷ → `OperationRequest 0x5d`
  → gói 0x5f chỉ số 1; 0xdf → thông điệp 0xa8 (hộp hỏi lại); 0xd8 client này hiểu khác.
- **Zone**: `KPlayerGiveItem`/`KGiveItemEntry` (`KPlayerDialog.h`), `KPlayer::give`, `dialog_give_item_ui`/`give_items_request`/`give_item_msg` (`KSubWorldDialog.cpp`),
  `send_script_action(…, notify)`, 5 hàm Lua, proto `GiveItemEntry/GiveItemsReq/GiveItemMsg` + `C2G_GIVE_ITEMS 1124` / `G2C_GIVE_ITEM_MSG 2143` + `ScriptAction.notify_changes`,
  gateway allowlist, `KGameServer`/`KMapInstance`, `log.vi.json` +5 câu +5 trường. **Client**: `export-ui` `dua-vat-pham`, `UiGiveItem.gd` (nhấp vật túi cầm lên → nhấp ô
  hộp → `put_item`; nhấp vật trong hộp → bỏ; Đồng ý → `Game.give_items(1)`; báo đổi → `give_items(0)`; Hủy/đóng → `dialog_answer(1)`; câu hỏi lại → Đồng ý hai lần),
  `KUiDialogMath.give_box_place/give_cell_code/give_entries`, `KUiGameWindows` ui 11 + tay cầm (`_put_in_give_box`), `KProtocolProcess.give_items` + `give_item_msg`,
  `_auto_give`.
- **Test/đo**: `[item][world][dialog]` 1 ca (GiveItemUI gói ui 11 + hai hàm + select; 0x89 kind 0 → hàm đổi với N = 2, mã ô 1/9; mục sai chỗ/ô bị bỏ; trùng ô/vật → từ chối
  và xoá; kind 1 → hàm xác nhận rồi xoá trả lời; huỷ = trả lời 1; không hàm xác nhận → không gửi; đang giao dịch → từ chối; hai câu 0xd8/0xdf); ctest 293/293 Release + Debug; Godot 518;
  e2e `AUTO_GIVE ui=11 window=true placed=1 placed_id=235 confirmed=true unit=unit 235` (GetGiveItemUnit(1) trả đúng id vật đã đặt), `AUTO_TRADE end=1`, `AUTO_DIALOG ui=0 window=true answered=true`, `AUTO_DESCRIBE ui=12`.
- **Chưa**: ràng buộc vật (`Item+0x350`) để từ chối vật khoá khi tham số 5 = 0; gợi ý 0xd8 trên client (client 2.0 không có); kéo-thả thật (hiện nhấp-cầm-nhấp như túi);
  PARTNER_*, `AddNote 0x08124DC0`, tên subworld/tên npc byte thô, quality 1/2 của `AddItemEx`.
- commit: `JX NEXT: M13 lat D8 - …`.

### 2026-09-19 (phiên tiếp theo, phần 58) — M13 lát D7: hàm vật phẩm của script nhiệm vụ (AddItemEx 0x08120470 → Lua_NewItem 0x0811F230 với seed, GetItemProp 0x080FF260, SyncItem 0x08114EF0, RemoveItemByIndex 0x08114F80, GetItemStackCount 0x080FD250, GetGlodEqIndex 0x080FEF90, SetItemMagicLevel 0x080FD020, ITEM_GetItemRandSeed 0x081548E0)

- **jx_linux_y** (đọc từng dòng, `LINUX-SERVER.md` §26): `Lua_NewItem` nhận 9 số `version, seed, quality, genre, detail, particular, level, series, luck` + 6 cấp; **seed ≠ 0 →
  `g_SetRandomSeed(seed)` trước khi sinh rồi trả seed cũ** (`0x0811F808`), `KItemSet::Add 0x0806E110(bộ, genre, quality, series, level, luck, detail, particular, cấp, version,
  seed, ngữ cảnh, cờ, 0, 1)`; `AddItemEx` = `Lua_NewItem` + `KPlayer::AddItem(idx, 1, 1, 0)` → đẩy chỉ số (không được → giải phóng, 0), chuỗi đầu bị `lua_remove`,
  ≤ 8 đối số → in câu lỗi + 0; `GetItemProp` đẩy `Item+0/+8/+0xc/+0x24/+0x28/+0x200` (luck); `GetItemStackCount`: `+0x14` chồng được và `0 < +0x308 ≤ max(+0x30c,1)` →
  `+0x308`, khác 1, sai đối số/chỉ số → −1; `GetGlodEqIndex`: `+4 == 1` → `+0x80 + 1`; `SetItemMagicLevel`: `top == 3`, ô 1..6 → `+0x1e4 + ô·4`; `ITEM_GetItemRandSeed`
  → `+0x1e0`; `SyncItem` → `KItemList::SyncItem 0x081FB9A0`; `RemoveItemByIndex` → `KItemList::Remove` + `KItemSet::Remove` lý do 0xd. Đọc dở `GiveItemUI 0x0812BBA0` (gói
  0x63 ui 0xb, hai hàm trả lời vào `+0x5fa0/+0x6020`, đặt lại khối giao dịch `+0x52a4..`) và `GetGiveItemUnit 0x08114E60` (`Player+0x2b0[24]`) — cần cửa sổ client → D8.
- **Zone**: `KItem::rand_seed/magic_level[6]/luck` (+proto `ItemData 22/23/24`, `to/from_proto`), `KItemGenerator::equipment` ghi seed/luck/cấp, `make_script_item`
  (dùng chung `AddItem`/`AddItemEx`), `script_item` (chỉ số = `KItem::id` của người chơi), 8 hàm Lua, `KSubWorld::sync_item`, `log.vi.json` +2.
- **Test/đo**: `[scriptfuns][item]` 1 ca (test_KItem.cpp: AddItemEx cùng seed → cùng tiền tố, GetItemProp 6 giá trị, seed/stack/gold/level, SyncItem → `G2C_ITEM_ADD`,
  RemoveItemByIndex → `G2C_ITEM_REMOVE`, lưu/nạp giữ seed/cấp/luck); ctest 292/292 Release + Debug; Godot 509; `check_log_catalog`/`check_includes`/`go vet`
  sạch; e2e `AUTO_TRADE … end=1 item_gone=true money=580->575`, `AUTO_DIALOG … ui=0 text_len=81 window=true answered=true`, `AUTO_TASK … task_id=101`, `AUTO_DESCRIBE ui=12 options=2 window=true tip_len=37 pane=true`; zone log lần chạy: `item traded` 1, `script failed`/`job threw`/`script api called without a player` 0.
- **Lỗi e2e tìm ra (không phải D7)**: bước giao dịch `AUTO_TRADE end=-1` ở hai lần chạy liền — túi nhân vật bot (`data/gateway/chars/44.json` Auto2) đã 18 vật sau 13 lần
  nhận đồ, `trade_exchange` (`fits`: túi bên nhận phải chứa được ô bên kia, `0x081FA250`) hạ `ok` của bot và gửi 0xb/0xc, không có 0x78; rồi `_auto_dialog` bị từ chối vì
  "đang giao dịch" (`0x080B1315`) → `AUTO_DIALOG ui=-1`. Sửa: `jxbot -partner` khi nhận `G2C_ITEM_LIST` gửi `?gm ds RemoveItemByIndex(id)` cho từng vật trong túi
  (log `bag cleared · vật phẩm=18`).
- **Chưa** (D8): `GiveItemUI`/`GetGiveItemUnit` (+ cửa sổ đưa vật phẩm của client và gói đặt vật), PARTNER_*, `AddNote`, tên subworld/tên npc byte thô, `AddItemEx`
  quality 1/2 (hoàng kim/bạch kim qua `Lua_NewItem`), thẻ tệp:dòng của `Lua_NewItem` (`0x081E82D0`).
- commit: `JX NEXT: M13 lat D7 - …`.
