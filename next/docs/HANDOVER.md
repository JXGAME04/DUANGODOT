...
- **M12 lát B5c (xong 2026-09-18)**: **khối tên/máu trên đầu npc như 2.0** (`CLIENT-2.0.md` §17): vòng vẽ `0x00670130` (`[core+0xa8c4]` npc dưới chuột;
  ...
  dưới chuột → cỡ 14 nền đen, `0x0066B600(2)` → cỡ 12, không → chỉ người/npc), `PaintLife 0x005EACF0` (không có điều kiện bị thương), từ tuỳ chọn `+0x1eb4`
...
- **gamecl.exe** (đọc từng dòng): vòng vẽ `0x00670130` (`0x0067021A` npc dưới chuột, `0x00670243..0x0067029A` thanh máu theo `0x0066B660`, `0x006702BD..0x00670331`
  khối chữ theo `0x0066B600` với cỡ 14 nền đen / 12 / ẩn), `PaintLife 0x005EACF0` (`!force && kind ∉ {1,2}` → thôi), từ tuỳ chọn `[0x21a0438+0x1eb4]` (4 giá trị 2 bit,
...
- **Client**: `KNpcGold.gd` `toggle_switch/name_block/life_bar`; `KNpc.gd` `name_switch/life_switch` (static), `hovered/set_hovered/refresh_info`, `_life_label`
  "%d/%d", khối rộng theo chữ, `StyleBoxFlat` đen khi 14, thanh máu theo `life_bar`; `UiGame.gd` `_set_hovered` (chuột), F7/F8 `set_show_switches`.
- **Kiểm**: Godot 438/438; e2e `JX_ZONE__TEST_NPC_GOLD=7`: `auto_fight.png` mục tiêu "78/160 / npc3/Lv:10" cỡ 14 nền đen, quái khác cỡ 12, exit=0.
