...
- Client: `ui/uicase/UiMiniMap.gd` (khung `ban-do-nho`, `MapRect` vẽ ảnh + chấm, `SceneName` xanh, `ScenePos` = toạ độ thế giới/32 như `NewWorld` — `scene_pos` của client là toạ độ cục bộ region đầu nên cộng `region_left×512`, `region_top×1024`); `KUiGameWindows.minimap`; `UiGame._bind_minimap` khi vào map/đổi map, `set_own` khi mình xuất hiện; `KWorldView.map_info()` (2D `map.json`, 3D `map3d.json`, 2.5D = `map.json` của `KGround25D`). Vị trí `Left=876;+35`: cú pháp ";+35" chưa giải, neo mép phải [tự chọn].
- **Map 3D** (A14): `make_map3d.export_minimap` đọc `ui_map_view` (id scene → `world_baling_big.png` 1680×917 trong bundle UI `9431897d8969`, toạ độ Unity trái-dưới/phải-trên (−59,94; −12,66)…(383,56; 229,21)) → `assets3d/maps/9053/minimap.png` **xoay 180°** (bộ xuất đảo trục X, y scene tăng về phía camera) + `map3d.json "minimap" {left, top, right, bottom}` (đơn vị scene: x −1258..20917, y 391..12484).
- Kiểm: `--auto3d` ảnh `auto3d_4.png` (Ba Lăng: ảnh tham khảo + chấm), `auto3d_8.png` (Phượng Tường: ảnh 24.jpg, mình ở tâm, "1551/3150"); UiCheck 162, Godot 589, e2e 4/4 OK.
- commit: `JX NEXT 3D: 3D-50 - minimap 2.0 (KUiMiniMap 0x004C4BB0, <map>24.jpg minimap.pak, KScenePlaceMapC 32 px/region, mau Setting.ini) cho map 2D + anh ui_map_view cho map 3D; jxassets export-minimap, export-ui them 3 man ban do`.
...
- **C10** `eSkillEventType` đọc từ `global-metadata.dat` (`D:\game3gtQ_mo\enum_values.py` — giá trị mã hoá compressed-int zigzag của IL2CPP v31: 8 ChildOject, 2 Hit, 101 AnimPlay, 112 PlaySound, 111 Ghost, 120 SfxObject, 103 CameraShake, 122 SkillWarning, 26 COSetLineParticle…; `eSkillEventTriggerType` 0 TimeFrame 2 HitPass 3 ArriveTargetPos 7 TriggerEnd; `eSkillChildObjTrait`, `eSkillHitType`, `eSkillGroupType`). Trong các đoạn kỹ năng: 8 ×400, 101 ×311, 112 ×121, 111 ×112, 4 ×23, 201 ×19 (AddState, ngoài enum — mã riêng), 2 ×17, 11 ×17, 6 ×13, 120 ×5.
- `map_skills.py` đọc thêm: `skill_main` cột 4 **nhiều đoạn** ("815*830", "106*107*108" — trước chỉ lấy đoạn đầu → 0 hình), chuỗi đoạn cột 11, cột 21 hiệu ứng ra chiêu của kỹ năng (116 kỹ năng: sfx 1..4 = 土/火/水/木 — bản tham khảo **không có "kim"**, `ELEMENT_CAST` bỏ 0), sự kiện riêng của **vật con lồng nhau** (cột 4 của `skill_childobj`: 132 vật con sinh vật con khác — `children` với `when` time/arrive/hit/end; vật con không có ảnh "空子物体" là vật mang), `fly` = đồng bộ 3/4/5/6 (bay theo/tới, sát đất), sự kiện 201 AddState → hào quang của trạng thái (`state_list`) vào `aura` (một khí tam thanh, kim chung tráo, tĩnh tâm quyết…), thống kê `events`/`event_kinds`.
- Client `KSkillFx3D`: `flying()`/`hit_res()` đệ quy; `_cast_children` (vật con đứng + con lồng theo thời gian, vật con "trúng" đặt ảnh trúng tại đích), `_arrive_children` (con sinh khi đạn tới/trúng — gọi từ gói hiệu ứng đạn của zone); `HANG_HEIGHT sys_state 2,2 m`.
- **C2** `export_sfx.py --all` thật: duyệt prefab gốc (Transform không cha), đoán thư mục theo hash md5 (Skill 210, Cmn 49, State 33, Daoguang 29, Halo 12, UI 9) → **343/343 prefab** (trước 273); `sfx_index`: 280 được bảng gọi (childobj 152, sfx_object 122, state_list 63, anim_effect 15, sfx_ui 7, skill_hit 3), 63 do mã/Lua gọi (`cmn_select` vòng chọn mục tiêu, `cmn_droplight_*` vật rơi, `cmn_rocker_*`, `cmn_chuansong` dịch chuyển, `cmn_ma_tui_*` bụi ngựa, `hg_*` vũ khí phát sáng theo phẩm chất).
- **Tween NGUI** (398 node: enhance 114, rotation 94, alpha 85, scale 83, position 19, color 3): bố cục byte đọc theo thứ tự trường metadata (`read_tween`: method/style/delay/duration/curve + from/to; kiểm tổng byte 180/180/204/188 đúng) → `tweens` trong JSON; `Scn3DSfx._start_tween` (Tween Godot: ease theo method, Loop/PingPong, xoay/scale/vị trí/alpha/màu/enhance trên vật liệu).
- **Shader tham khảo đọc được**: bundle shader của **APK** là GLSL văn bản (`D:\game3gtQ_mo\shaders_apk\`, 68 shader) — `blend_dst_mask`: rgb = tex × vcol × (1 + uv.z)·_Enhance, alpha = tex.a × vcol.a × enh (mask: ngưỡng tan theo vcol.a), `Blend SrcAlpha [_BlendDst]`; `SFXMeshModify.Update` (disasm 0x6fdb70): màu đỉnh = color × adjustColor **+ emissive** (bản tham khảo dựng linear-space) → `Scn3DSfx` sửa `extra = c + e` (trước c × max(1, e)).
- **C9** `--auto3d --factions[=<phái>]`: quét 10 phái (theo dải id `skill_main` 100/200/…/1000), mỗi kỹ năng có hình thi triển tại chỗ (chỉ ảnh, không luật zone) + đạn/trúng + hào quang, ảnh `auto3d_fx_<phái>.png` và `_b.png`: **170 kỹ năng phái, 133 có hình, 133/133 hiện** (`AUTO3D_FACTIONS`); 37 không hình = bí kíp bị động (quyền/kiếm/đao pháp, tâm pháp) + 断魂刺/轻功 (chỉ Ghost/KinematicMove). `--factions=<phái>` in `AUTO3D_FXNODE` (mesh/vật liệu đang hiện).
- Còn lại đợt 1: C3 `SFXMixerMesh` (27 node / 18 prefab: 7 hào quang, 8 bẫy Đường Môn, 沧海明月, 传送), C4 `XWeaponTrail` (16 prefab, `anim_effect`), `SFXMeshTrailDrag` (3), `SFXLineMesh` (1), `PC2Anim` (1 — 追心箭), Ghost 111, CameraShake 103.
- Kiểm: `--auto3d` (`AUTO3D_SKILL fx_spawned=3`, `AUTO3D_AURA halos=1`, `AUTO_FIGHT` 80→25, 144 FPS), Godot 589/589.
- commit: `JX NEXT 3D: 3D-47 - dot 1 MO-NHI-PHAN: ghep tay 28 ten (234 ky nang), nhieu doan + vat con long nhau + AddState, eSkillEventType tu metadata, export 343/343 prefab, tween NGUI, shader GLSL APK, quet 10 phai --factions`.
...
- **Vũ khí của người khác**: gói 0xad (`G2C_ENTITY_RES`, `KNpc::SetPlayerRes 0x005ED920`) chỉ mang **hàng ảnh** (MeleeRes.txt cột 2 − 2), không mang vật phẩm; `items/item_res.json` (bảng `*Res.txt` của server cũ, `KItemChangeRes`: hàng = nhóm×10 + cấp + 2) đảo ngược → hàng ảnh → (nhóm, cấp) **đầu tiên** vẽ hàng đó → id vũ khí tham khảo (`WEAPON_BASE_ID` + cấp − 1). Client 2.0 vẽ chung một ảnh cho nhiều vật phẩm (kiếm cấp 1..3 = hàng 1, 4..6 = hàng 2, 7..8 = hàng 3, 9 = hàng 20, cấp 10 = hàng 4 chung với đao 1..3 — đúng bảng MeleeRes.txt [2.0]); bản 3D chọn vật phẩm đầu của hàng [tự chọn]. `KWorldView3D._load_res_inverse/_weapon_particular/_weapon_id_of/_refresh_weapon(node)` cho mọi người chơi (mình từ túi, người khác từ `equip_rows[2]`), `Game.entity_res` → `_refresh_weapon_of` (deferred, sau khi `UiGame._on_entity_res` ghi hàng), vệt kiếm chỉ cho mình.
- **Ngựa theo hàng ảnh** (sửa 3.6): client 2.0 vẽ ngựa theo `HorseRes.txt` cột 2 − 2 (`GetHorseRes`), nhiều vật phẩm chung một hàng (hàng 4 = nhóm 1 cấp 6..10 + nhóm 2 cấp 1..3; hàng 8 = 普通黄马 nhóm 0 cấp 1..9) → ghép theo **nhóm màu vật phẩm** như trước là sai so với 2.0. Cột 3 HorseRes.txt [2.0] cho tên ảnh (普通黄马, 赤兔，顶级红, 赤龙驹，顶级血红…) → `make_map3d.HORSE_RES` (36 hàng, tên + cha_pic theo màu [tự chọn]: 白→1500/顶级 1510, 黑→1501/1511/绝地 1561, 青→1503, 黄棕→1502, 红→1512, 血红·龙驹→1562; hổ/sư tử/lạc đà/hươu/lừa/sói/lợn không có model → ngựa mặc định) → `models.json horses.by_res` (+`names`); client `_horse_res_of(node)` (hàng 0xad của mọi người, kể cả mình; dự phòng tính từ vật phẩm mặc `_item_res_row`) → `_horse_for`. `KNpc3DView.reload_horse()` khi hàng ngựa đổi lúc đang cưỡi; `Scn3DNpc.cha` được gán khi nạp.
- `KNpc.setup`: không gọi `_res.set_equips` khi `no_2d` (lỗi `set_equips in base Nil` trong map 3D).
- Kiểm `--auto3d` (map 9053): `AUTO3D_WEAPON bag=1 rows=1 row=1` (đường túi = đường hàng ảnh), `AUTO3D_HORSE rows=1503 row=4 item_row=4 shown=1503` (Liệt Bạch Mã nhóm 2 cấp 1 → hàng 4 普通青马 → 青马 1503, ảnh `auto3d_ride.png`), đánh Heo trắng 80→58, kỹ năng 153 (3 hiệu ứng), hào quang 211, bẫy về map 1, 144 FPS. e2e (`JX_CONFIG=Release`): BOT/CLIENT/CLIENT 2D/CLIENT WS OK, items 7, skills 13–18, cưỡi `horse_row=8`, ngồi, PK như main. Godot 589/589, UiCheck 162/162.
- commit: `JX NEXT 3D: 3D-45 - gop main (B6a/B6b/PK), vu khi + ngua nguoi khac tu goi 0xad (item_res dao nguoc), ngua theo hang anh HorseRes.txt, dev.py chon Release`.
...
- **3.3** Nước `水体高光双浪_水底扭曲` và lá/cỏ `晃动`: **tham số** của vật liệu (scene.json `floats`: `_NormalTex` tiling 12×6, `_FlowDirX/Y`, `_FlowSpeed`, `_ripple`, `_DistortStrength`, `_Brightness 3,27`, `_SpecSize/_SpecSt`; lá `_radius/speed/noroff`, cỏ `windSpeed/windStr/gHeight`) lấy từ bản tham khảo, **công thức** shader tự viết (`scn3d_water.gdshader`, `scn3d_lm_sway.gdshader`) vì shader gốc đã biên dịch — ghi rõ [tự chọn] trong tệp. Sương theo `render` của scene đã có từ trước; ngày/đêm chưa.
- **3.6** Ngựa: bản tham khảo có 7 ngựa thường (`ma_putong`: 白马 1500 … 红骊 1512) + 3 long mã (`zq_longma` 1560–1562), nhóm animation 21 (ngựa) / 20 (người cưỡi: xx 85, zp 84, đánh 80/81, nội công 82), điểm treo `ma_qi1`. Ghép với ngựa JX1 (80 vật phẩm, 8 nhóm màu `particular`): trùng tên chính xác Ngọc Hoa Thông = 玉花骢, Hắc Kỳ = 黑骐, Hồng Ly = 红骊; còn lại theo màu [tự chọn] (`make_map3d.py HORSES` → `models.json`). Client: `KNpc.riding` + `set_riding` (EntityInfo.riding, G2C_ENTITY_RIDE), `KNpc3DView._on_riding_changed` (ngựa dưới người, người gắn vào `ma_qi1`, nhóm 20/21, tên cao hơn), `export_npc.py` xuất thêm nhóm 20 cho nhân vật và **gộp** `npc_models.json` thay vì ghi đè (lỗi vừa gặp: xuất riêng ngựa xoá hết model khác). Kiểm: `AddItem(0,10,2,1)` Liệt Bạch Mã → mặc ô 10 → `C2G_RIDE` → ngồi trên 白马 (`auto3d_ride.png`).
- Hoãn có lý do: **3.4 minimap** — client 2D chưa có minimap/bản đồ 2.0 (mục 4.1 của PHAN-TICH), phần chụp từ trên xuống làm khi cổng UI đó có; **3.5 âm thanh** — tiếng đòn/đạn trong 3D đang theo đúng luật pan/âm lượng của 2.0 (`KWavSound` + Camera2D tại nhân vật), đổi sang `AudioStreamPlayer3D` sẽ lệch luật 2.0; nhạc nền theo vùng chờ bảng nhạc map của 2.0.
- commit: `2ec203e` (3.2), `51221b3` (3.3), và `JX NEXT 3D: M3D-3 (3.6) - cuoi ngua 3D (ngua ban tham khao 1500-1562, nhom 20/21, ma_qi1), KNpc.riding, export_npc gop npc_models.json`.

### 2026-09-19 (nhánh exp/3d-baling, phần 3D-40) — M3D-3 bước 3.1: hiệu ứng kỹ năng 3D ghép từ bảng bản tham khảo (205 kỹ năng JX1), sửa hai lỗi xuất hiệu ứng

- **Phát hiện**: `skill_main` của bản tham khảo dùng **tên Hán của chính kỹ năng JX1** (怒雷指 = Nộ Lôi Chỉ, 剑飞惊天 = Kiếm Phi Kinh Thiên…) — ghép được bằng âm Hán-Việt: `tools/scn3d/hanviet.py` (389 chữ, kèm cách viết của bản dịch JX1: 当 "đang", 正 "chánh", 梯 "thế", 套 "thao", 佑 "hữu", 霁 "tề", 错 "tá", 及 "cấp", 刚 "cang", 相 "tướng", 梵 "phạn", 沿 "diên", 倩 "trái", 羁 "ki", 蛊 "độc"), `map_skills.py` chỉ nhận **khớp đúng** tên `SkillName` (chuẩn hoá chữ thường, bỏ số/khoảng trắng) → `client/assets3d/sfx/skill_map.json`: 205 kỹ năng JX1 ↔ 288 kỹ năng tham khảo, 99 có hiệu ứng cụ thể (cast từ `skill_event` loại 120 → `sfx_object`, vật con loại 8 → `skill_childobj`: đường dẫn prefab, khung sinh /30, cao, điểm treo, bay (`sync 4`) + hiệu ứng trúng cột 23), 28 không khớp (liệt kê trong `unmatched`: 弹指烈焰, 厉魔夺魂… tên bản dịch khác). Không khớp → hiệu ứng chung theo ngũ hành (`sfx_object` 1–4 ra chiêu, 20+ trúng; `Series` của skills.json).
- `client/scenes3d/KSkillFx3D.gd`: `cast()` (EntityAction có `skill`: hiệu ứng ra chiêu tại `sys_bd` ≈ 0,9 m, vật con đứng tại điểm nhắm, thời điểm = khung/30 × `frames/18` s của zone), `attach_flying()` (đạn của zone = vật con bay của kỹ năng thay cầu sáng), `hit()` (va chạm `G2C_MISSLE collided` → hiệu ứng trúng của kỹ năng hoặc ngũ hành). `KWorldView.action(node, a)` mới trong giao diện; `add_missle_effect` thêm `skill_id`.
- **Lỗi xuất tìm ra và sửa** (`export_sfx.py` xuất lại 273 prefab): (1) ô atlas của `SFXMeshModify` (`uvOffset`) **đếm từ mép trên** ảnh — kiểm trên 362 mesh: theo cách cũ 208 mesh rơi vào vùng đen (độ sáng 18,9 so với 52,6) → `v = (oy + (1 − v_unity)) / ny`; (2) `Scn3DSfx.build` tìm node theo **tên** nên prefab có node trùng tên (`shifa_tuxi`: "shang" hạt tầng 1 và "shang" mesh tầng 2) gắn vật liệu sai → hình vuông trắng đổ bóng; giờ ghép theo **đường chỉ số con** (JSON và glTF cùng thứ tự duyệt trước). `Scn3D.gd --sfx=<tên>` chụp riêng một hiệu ứng để kiểm.
- Zone: `check_command` ghi trace `why` của lệnh kỹ năng bị từ chối (peace mode / not held / cannot cast / cost…) — tìm ra Bất Động Minh Vương bị từ chối vì `EqtLimit` (đang cầm kiếm), đúng luật.
- Đo (`--auto3d`, Võ Đang `add_wd(30)` + `AddMagic(153,1)`): Nộ Lôi Chỉ bắn vào Hươu đốm → 3 hiệu ứng (vòng sáng hệ Thổ tại ngực, đạn `Skill/wd_nuleizhi`, trúng), ảnh `auto3d_6/7.png`; 144 FPS.
- Còn của 3.1: hào quang/trạng thái (`Halo/*` qua bảng state), 28 tên chưa khớp, stretched billboard/trail module, tinh chỉnh vị trí điểm treo theo xương thật (`sys_bd` đang là hằng 0,9 m).
- commit: `JX NEXT 3D: M3D-3 (3.1) - ghep ky nang JX1 <-> hieu ung ban tham khao qua am Han-Viet (skill_map.json), KSkillFx3D, sua o atlas SFXMeshModify + ghep node theo chi so`.
...
- **1.3** `KNpc3DView`: `doing_changed` → clip đúng `frames/18` s (`Scn3DNpc.play_once`, xác giữ khung cuối); vũ khí: vật đang mặc ô 3 (`KItemList::GetWeaponType 0x0060D660` [2.0]) → `particular` JX1 (0 kiếm 1 đao 2 côn 3 thương 4 chùy 5 song đao, từ `items/base.json`) → `weapons.json` id = gốc loại + (level − 1) — **danh sách vũ khí bản tham khảo chính là của JX1, cùng thứ tự** (铁匕首 = Thiết Trủy thủ…); vệt đao khi clip `gj*`. Tiếng hành động khi `no_2d` (`_sound_of_doing` = `act_no` + `action_sound`, không cần ảnh). Khối tên/máu 3D theo `KNpcGold` (14 viền/12/ẩn, "tên/Lv", "hp/max" quái, màu theo camp/vàng, thanh 38×3 màu theo %).
- **1.4** Bẫy: `make_map3d.py` biến ExitPoint/EnterPoint của map tham khảo thành `traps` 3×3 ô + Lua `data/script/3d/script/3d/exit_9053_<n>.lua` (`NewWorld(1, 1551, 3150)` = điểm sinh Phượng Tường, Mps tuyệt đối), `dev.py LUA_ROOT_NAMES` thêm `3d`. Chọn mục tiêu = tia camera vào trụ, vật rơi = ảnh ObjData billboard (1 px = UNIT).
- Đo (`--auto3d`, tài khoản mới): AddMagic(53,10) + AddExp → cấp 21, Thiết Trủy thủ mặc được (kiếm cấp 3 bị từ chối vì yêu cầu 30 sức/18 cấp — đúng luật), đánh Heo trắng 13 đòn → chết (`AUTO_FIGHT dead=true`), bẫy → map 1 → bản vẽ về 2D (`AUTO3D_TRAP view3d=false`), 143 FPS. Ảnh `auto3d_0..6.png`, `auto_fight.png`.
- Lỗi tìm ra trên đường: gateway cũ (bản copy 19:36) không chuyển `C2G_ITEM_EQUIP` ("message not allowed in state") → phải build Go trong bản 3D; bản ghi nhân vật `data/gateway/chars/*.json` có `itemUid` (gateway mới hơn `origin/main`) → dời sang `build/chars-itemuid-backup`; zone thêm log `zone.item` cho equip (parse lỗi cũng báo).
- Còn để M3D-3: camera chui vào tán cây (chưa có mờ/va chạm với cây, 3.2), hiệu ứng đạn tạm là cầu sáng (3.1), hội thoại NPC (M13).
- commit: `4601f3b` (gộp main), `cb77a1e` (1.3), và `JX NEXT 3D: M3D-1 (1.4, 1.6) - bay chuyen map 3D<->2D, --auto3d tron bo, tai lieu THU-NGHIEM-3D`.

### 2026-09-19 (nhánh exp/3d-baling, phần 3D-37) — M3D-1 bước 1.1–1.2 + 1.5: thế giới 3D nối zone thật (map thử 9053 Ba Lăng 3D), UI 2.0 đè lên

- `client/scenes3d/`: `KScenePlace3D.gd` (nạp `map3d.json` → glTF + lightmap/địa hình shader + môi trường + nắng + va chạm địa hình; `to_world/to_scene` theo `origin` của map, `ground_height` raycast, `ray_ground` cho con trỏ; map không có bộ 3D → mặt phẳng phẳng), `KCamera3D.gd` (quỹ đạo theo `cameraInit` của map [TK]; kéo chuột phải xoay, **click phải không kéo vẫn là kỹ năng chuột phải 2.0** qua `right_click`), `KNpc3DView.gd` (Node3D bám `KNpc`/`KObj` trạng thái: vị trí đổi đơn vị, chân đặt trên địa hình, yaw = `yaw_of_dir(res_dir)` mượt theo khung hình thật, `doing_changed` → clip model dài đúng `frames/18` s (`Scn3DNpc.play_once`), xác giữ khung cuối (`hold_last/seek_end`), vòng chọn mục tiêu, khối trụ đánh dấu khi chưa có model), `KMissle3DView.gd` (đạn = cầu sáng + vệt, tan = bụi; chờ 3.1), `KWorldView3D.gd` (thực thi `KWorldView`: model theo `models.json` template JX1 → cha_pic; `pick` = tia camera vào trụ `cp_radius × sys_bar`; `screen_to_scene` = tia xuống địa hình; nhãn tên + thanh máu 2D vẽ bằng `draw_string` theo luật `KNpc::Paint/PaintLife`; LOD animation 45 m; `KWavSound` giữ nguyên với Camera2D đặt tại nhân vật).
- `UiGame.gd`: đổi bản vẽ khi qua ranh 2D/3D lúc `map_changed` (`Game.want_3d()`); `--gm=<lua>` chạy một lệnh GM lúc vào (dev); `--auto3d` (chờ bản 3D, chụp 3 góc camera, đi 200 đơn vị, đánh quái gần, in `AUTO3D_OK`).
- Tài sản bản 3D: `client/assets` không còn là junction sang `swrod3` (bộ đó chỉ có 4 màn UI) mà là thư mục thật: junction `maps` (980) / `sprites` / `npcres` sang `swrod3/next/client/assets`, còn `ui` (32 màn), `items`, `sounds`, `text`, `missles`, `*.json` chép từ worktree `swrod3/.claude/worktrees/handover-doc-review-b0d015` (bộ xuất đầy đủ). Zone nạp 14 187 vật phẩm, 1 628 kỹ năng, 981 map.
- Đo (`--auto --auto3d --gm=NewWorld(9053,232,194)`): vào map 1 (2D) → NewWorld → map 9053 chuyển sang `WorldView3D`, 14 thực thể của zone đều có model 3D (tên Việt theo template JX1: Võ Đang Chưởng môn, Thợ rèn…), đi 200 đơn vị đến nơi, thanh trên/dưới 2.0 hiện đè, 134–144 FPS; ảnh `user://logs/auto3d_0..3.png`. Chưa có quái trong 400 đơn vị quanh điểm sinh → `AUTO_FIGHT none` (làm ở 1.3).
- commit: `JX NEXT 3D: M3D-1 (1.1-1.2, 1.5) - KWorldView3D/KScenePlace3D/KCamera3D/KNpc3DView/KMissle3DView noi zone that, map 9053, --auto3d`.

### 2026-09-19 (nhánh exp/3d-baling, phần 3D-36) — M3D-0: ADR-008 client 3D, quy ước toạ độ, tách lớp vẽ thế giới `KWorldView` (chủ dự án: làm từng bước tới khi xong, mọi số có nguồn nhị phân)
...
- Kiểm: `tests/run.gd` 554/554, `UiCheck` 26/26 (bộ ui đầy đủ chưa xuất trên máy này), `dev.py e2e` xanh TCP + WS, `dev.py screenshot` ảnh 2D như trước (map, tên, thanh dưới 2.0) — ảnh 2D là kiểm hồi quy của việc tách lớp, không phải nhầm dự án.
- commit: `JX NEXT 3D: M3D-0 (0.1-0.3) - ADR-008, quy uoc toa do KScene3DMath, tach lop ve the gioi KWorldView/KWorldView2D` (a682626), `.uid` (cdebc23).
- **0.4** `tools/scn3d/make_map3d.py world_baling --id 9053` → `client/assets3d/maps/9053/` (gitignore): `map.json` (schema MAPS.md §3, NPC theo template JX1: `CHA_TO_TEMPLATE` thợ rèn 197, dược điếm 203, xa phu 235, chưởng môn các phái 64/73/82/92/98/104/111/118/125/130, heo trắng 43, hươu đốm 42…; `dir` từ góc mark), `obstacle.bin` rasterize navmesh AIS (tâm ô trong tam giác; 560×384 ô, 44 % vật cản, gốc (−358,4; −20,48) m), `map3d.json` (glTF, UNIT, origin, camera `scn_list`, ground_y), `models.json` (template JX1 → cha_pic). Zone: `zone.maps_dir_extra` (`main.cpp`, danh sách "a;b", id trùng lấy thư mục đầu) + `config/zone.json` → nạp **981 map**, log `map loaded id 9053 … npcs 494`; build Release trong bản 3D (`build/build_zone.cmd`, không commit), ctest 222/222.
- Luật mới của chủ dự án (lưu memory `jxnext-progress-reporting`): mỗi bước phải báo "Đang làm …" trước khi làm.
- commit: `JX NEXT 3D: M3D-0 (0.4) - make_map3d.py (navmesh -> obstacle.bin, map.json, map3d.json, models.json), zone.maps_dir_extra, map thu 9053`.

### 2026-09-19 (nhánh exp/3d-baling, phần 3D-35) — 3D bước 3–5 trên `exp/3d-baling`: vũ khí treo tay, hiệu ứng kỹ năng, navmesh, phân tích thiếu gì, lộ trình từng bước
