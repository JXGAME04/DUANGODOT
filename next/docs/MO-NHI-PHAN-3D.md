...
| A13 | `scn_list` (65), `scn_area_list` (93) | scene: đường dẫn bundle, camera `scn_list`; vùng: an toàn, nhạc `sound_group`, sương mù/ambient/đèn đổi theo vùng, camera theo vùng, bóng 45 m, tiếng bước chân | camera map, vùng an toàn, nhạc vùng (3.5), sương mù theo vùng | `map3d.json camera`, `map.json/map3d.json areas` | **một phần**: camera map xong; **vùng an toàn xong (3D-74)**: `TaskScnArea.LogicTick 0x52ee00` + `ScnUnit.InArea 0x4a2e30` → zone `KSubWorld::check_area` đổi `fight_mode`, client nêu tên vùng; nhạc/sương/đèn/camera theo vùng **chưa** | 1.1, 3.5 |
...
| Nhân vật – da áo | `AssetPool_Skin` (+`SkinPartItem/Request/SkinInfo`), `Creature.InitEquipModel/CreateChangeBodyInst`, `Player.SetEquipModel`, bảng `model_list` 101–122/301–322 | `export_npc.py --costumes`, `Scn3DNpc.set_costume`, `KWorldView3D.COSTUME_FAMILY` | **có** (3D-76): 44 bộ da theo họ áo JX1 × 3 bậc, model nữ `cha_pic 2`; *chưa*: 时装/定国 (không có vật phẩm 2.0 tương ứng) |
| Cảnh | `Scene`, `SceneLoad`, `SceneBuildData/DataInfo/DataNode`, `Scene_Ref`, `PrefabRef`, `SceneConfiger`, `SceneRenderSetting`, `CullDistances`, `T4M*` (địa hình), `Grass/MeshGrass/UGrid2DGrass` (cỏ sinh), `MarkPoint/Area/Line`, `NavContext`, `PathFindContext.*`, `MapHandle/MapHelper`, `AreaData`, `TaskScnArea` | `KScenePlace3D`, `export_scene.py`, `batch_maps.py`, zone obstacle, `KMapArea` | **có** 45 map + hiệu ứng cảnh (3D-58) + `CullDistances` theo lớp (3D-65) + vùng an toàn/chiến đấu `TaskScnArea` (3D-74) + height mesh AIS `NavContext.GetNearHeight` (3D-75); `Grass/MeshGrass/UGrid2DGrass` *bỏ* (0 bundle dùng — cỏ là mesh `T4MPlantObjSC` đã xuất) |
| Vẽ / hậu kỳ | `ScnRenderPipeline`, `PostEffectBase/Bloom/Distortion/RadialBlur`, `DistortionMapCamera`, `DepthMapCamera`, `CustomDepthTexture`, `GrabPassFeature`, `FullScreenPassRendererFeature`, `MirrorReflection` (phản chiếu nước), `SkyboxCam/SkyBoxCollection` (bầu trời), `DynamicShadowProjector.*`, `ShadowProjMgr`, `SeqProjectorHelper`, `Decal/DecalBuilder/NavMeshDecal`, `RenderQueueModifier`, `GUIColorTransit`, `TaskTweenDirLight/Fog/SupColor` (đổi đèn/sương theo vùng) | shader lightmap/địa hình/nước/cỏ; bóng đĩa; glow (3D-66) | *một phần* — **có** bloom URP (3D-66); *bỏ* theo bằng chứng: distortion/radial blur (`IsEnable` stub), phản chiếu nước, skybox, decal, `ShadowProjMgr` (0 bundle), đổi đèn/sương theo vùng (0 dữ liệu); B7 bóng chiếu thật dùng bóng nắng Godot |
...
4. Hậu kỳ: lớp riêng `PostEffectBloom`/distortion *bỏ* (`IsEnable` = stub `0x33e630` trả false) **nhưng** bloom URP **có** — `ScnUnit.Init 0x49cbf7` nạp `Global.postEffectVolume_path` = `Assets/Settings/GlobalVolume.prefab` (profile `SampleSceneProfile`: Bloom ngưỡng 1,0 / cường độ 1,0, Tonemapping tắt), camera cảnh `renderPostProcessing = 1`, `UniversalRP-Scene` HDR bật (LowQuality tắt) → **xong 3D-66** (glow Godot; renderer GL của ADR-008 chỉ toả vài pixel, `JX_RENDER=mobile` cho quầng rộng).
...
6. Hiệu ứng còn lại: ~~`SFXMeshTrailDrag`~~ *bỏ* (3 prefab của boss 道清真人 riêng bản 3D), ~~`SFXLineMesh`/`LineCOCtrl`~~ **xong 3D-61** (2 sự kiện kind 26: 昆仑天际迅雷), ~~`Trail`~~ **xong 3D-60**, ~~`SFXXWeaponAdapter`~~ *bỏ* (71 vũ khí `sfx` = 0, không vũ khí nào có hào quang), ~~`PC2Anim`~~ **xong 3D-63** (1: 唐门追心箭, kỹ năng JX 50), ~~bay vòng moveType 7~~ **xong 3D-64** (3 dòng `skill_childobj`: 101 五毒百毒穿心 = JX 384, 306 tuyệt kỹ Thiên Nhẫn, 507 boss; lớp `Orbiter` chỉ là script demo PigeonCoop `TankController` → *bỏ*).
7. ~~Camera rung~~ *bỏ* (bảng `skill_event` **không có** sự kiện 103; enum có nhưng không dùng); nhạc/tiếng vùng: `AreaSound` 0 thể hiện trong bundle → nhạc theo `scn_area_list` cột 8 / `scn_list` cột 26, **chờ nhạc 2.0 của main** (3.5); ~~đổi đèn/sương theo vùng~~ *bỏ* (`TaskTweenDirLight/Fog` 0 thể hiện; `scn_area_list` cột 9–11 trống ở 4 vùng toàn game). Hiệu ứng chung còn chưa gắn (prefab đã xuất): `Cmn/cmn_levelup` (sfx_object 10000, 30 khung) và `Cmn/cmn_taskfinish` (10002) — chờ main mổ `MSG_LEVEL_UP` của 2.0 (client 2.0 không có sprite lên cấp trong chuỗi, chỉ thông báo) để quyết hiển thị.
8. ~~`bonechain` vải/tóc, phi phong (B5)~~ *bỏ* (0 prefab nhân vật mang `bonechain`/`ClothStrings` trong 68 bundle — bản 3D không có vải vật lý); ~~`GFX` LOD hạt~~ *bỏ* (0 prefab mang `GFX`, không chuỗi nào trong mã nhắc tới); ~~`CullDistances` theo lớp~~ **xong 3D-65** (10 cảnh: lớp 14 đồ nhỏ 25/40 m, 15 cây 50/55 m, 16 tán 0/100 m); `T4MPlantObjSC` (6 427) chỉ là lớp đánh dấu không trường → *bỏ*.
