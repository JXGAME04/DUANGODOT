...
`--auto`: `AUTO_TRADE opened=true sign=2 drawn=true closed=true changes=2`, `auto_trade.png` (biểu tượng kim đĩnh + "ban gi cung mua" trên đầu). Chưa: giao dịch hai client (cần `jxbot` trả lời — luật đã có test `[trade]` phía zone), sprite menu 2.0 của `0x00475690`, các mục menu khác (chat/bạn/theo sau/thông tin/bang), phòng 4 kim đĩnh, `SendHoldMsg` lặp khi giữ, kéo–thả đồ vào ô cụ thể (2.0 kéo bằng tay, client mới nhấp = ô trống đầu).
