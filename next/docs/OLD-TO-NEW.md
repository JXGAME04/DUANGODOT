...
| `Core/Src/KItemList.cpp` `ExchangeItem` (tay cầm: nhấc ô → đặt → cái bị đè lên tay), `EatMecidine` (Linux `0x08204710`: cấm thuốc, đếm thuốc, `ApplyMagicAttribToNPC(npc, 3)`, stack), `KNpcAttribModify::LifePotionV` (`0x08097E70`), khối thuốc của `KNpc::ProcessState` (`0x0808B7BC`), `KProtocolProcess::PlayerMoveItem/PlayerEatItem`, `s2c_syncitem/removeitem/playermoveitem/syncmoney` | `KSubWorld::item_move_request` (`KItemList::exchange` gộp hai bước), `item_equip_request`, `item_unequip_request`, `item_use_request`, `process_potions`, `send_item_list`, `give_item/take_item`; `KNpc::life_state`, `life_gain_percent`, `forbid_medicine`, `potion_counter`; `KMapInstance::apply_client_packet` (`C2G_ITEM_*`), `G2C_ITEM_LIST/ADD/REMOVE/MOVE/RESULT`, `G2C_MONEY`; client `KProtocolProcess.items` + `item_move/equip/unequip/use/drop` | luật uống thuốc và hồi máu theo nhị phân Linux (`docs/LINUX-SERVER.md` §9); `Check_ItemUsable`/`OnUseItem` (Lua) và ngồi/đứng chờ M11 E |
