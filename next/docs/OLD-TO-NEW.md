...
| `Core/Src/KSkills.cpp` (`KSkill::Cast`, `CastInitiativeSkill`, `CastPassivitySkill`, `CreateMissleMagicAttribsData`), `KMissle::ProcessDamage` phần lên một mục tiêu | `server/zone/src/KSkills.cpp` (`KSubWorld::skill_cast`, `cast_initiative_skill`, `cast_passivity_skill`, `skill_start_event`, `create_missle_magic_attribs_data`, `deliver_attribs`, `swing_skill`, `on_skill`), `KSkill::basic_attack` (dòng 2/3 `Skills.txt` khi không có bảng/script) | style 0/14 (đạn) tạm đặt tải lên mục tiêu ngay ở 60 % đòn; đạn bay là B2b |
...
Sắp tới (chưa có, sẽ dùng đúng tên): `KMissle`/`KMissleSet` (`missles.txt`, CastMissles, bay, va chạm), `KSkillList` (kỹ năng của từng nhân vật),
