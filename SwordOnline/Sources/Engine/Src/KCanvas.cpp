...
// 返回:	void
//---------------------------------------------------------------------------
BOOL KCanvas::MakeClip(int nX, int nY, int nWidth, int nHeight, KClipper* pClipper)
{
	// 初始化裁减量
	pClipper->x = nX;
	pClipper->y = nY;
...
// 返回:	void
//---------------------------------------------------------------------------
void KCanvas::FillCanvas(UINT wColor)
{
	if (m_pSurface)
	{
		DDBLTFX ddbltfx;
...
inline void KCanvas::UpdateBackBuffer(LPRECT lpRect)
{
	...
	if (GetUpdateRect(&SrcRc, &DestRc))
	{
		...
		g_pDirectDraw->ShortUpdateScreen(m_pSurface, &DestRc, &SrcRc);
	}
...
// 返回:	void
//---------------------------------------------------------------------------
void KCanvas::DrawPixelAlpha(int nX, int nY, int nColor, int nAlpha, int bScreenMode)
{
	KDrawNode	Node;
	Node.m_pPrev = NULL;
	Node.m_pNext = NULL;
...
/*	m_DrawList.AddTail(pNode);
	...
	{
		if(g_pDirectDraw->GetRGBBitCount() == 32)
			g_DrawPixelScreen32b(&Node, this);
		else
			g_DrawPixelScreen16b(&Node, this);
	}
}
//---------------------------------------------------------------------------
// 函数:	Draw line
						 ...
						 void* lpSprite, void* lpPalette, int nAlpha, UINT nColor, int nMode, BOOL bScreenMode)
{
	...
	Node.m_pPalette = lpPalette;
	Node.m_nAlpha = nAlpha;
	if(nMode < 0 || nMode >= CBM_COUNT)
		Node.m_nColor = CBM_COLOR;
	else
		Node.m_nColor = nMode;
	if(g_pDirectDraw->GetRGBBitCount() == 32)
	{
...
void KCanvas::DrawAlphaRecImage(int nX, int nY, int nWidth, int nHeight, void* lpBitmap, int nAlpha, int bOpa, int bScrMode)
{
	KDrawNode	Node;
	Node.m_pPrev = NULL;
	Node.m_pNext = NULL;
	...
	if(g_pDirectDraw->GetRGBBitCount() == 32)
	{
		if(bOpa)
			...
			g_DrawAlphaRecImage32b(&Node, this, bScrMode);
	}
	else
	{
		if(bOpa)
