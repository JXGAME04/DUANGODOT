...
public:
	...
	BOOL		OpenExtract(const char* pszFileName, int nExNum = -1, unsigned int byuId = 0);
	bool		IsTypeFrame();
...
//---------------------------------------------------------------------------
	...
	ENGINE_API SPRHEAD*		SprGetHeader(const char* pszFileName, SPROFFS*& pOffsetTable, int bFindById = 0);
