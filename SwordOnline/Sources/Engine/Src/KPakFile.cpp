...
SPRHEAD* SprGetHeader(const char* pszFileName, SPROFFS*& pOffsetTable, int bFindById)
{
	pOffsetTable = NULL;
	...
	if(bFindById)
	{
		...
		//_ASSERT(g_pPakList);
		unsigned int uId = (unsigned int)strtoul(pszFileName, NULL, 10);
		if (g_pPakList->FindElemFile(uId, PakRef))
		{
			pSpr = g_pPakList->GetSprHeader(PakRef, pOffsetTable);
			if (pSpr)
				pSpr->Reserved[PAK_INDEX_STORE_IN_RESERVED] = (WORD)(short)PakRef.nPackIndex;
		//g_DebugLog("findid ok [%u]spr[%d][%s]", (unsigned int)strtoul(pszFileName, NULL, 10), pSpr?1:0,pszFileName);
		}
	}
	else
	{
...
BOOL KPakFile::OpenExtract(const char* pszFileName, int nExNum, unsigned int byuId)
{
	if (pszFileName == NULL || pszFileName[0] == 0)
		return false;
	...
	Close();
	if (g_pPakList)
		bOk = g_pPakList->FindElemFileEx(pszFileName, m_PackRef, nExNum, byuId);
	return bOk;
}

bool KPakFile::IsTypeFrame()
{
	if (g_pPakList && m_PackRef.nPackIndex >= 0)
	{
		return g_pPakList->IsTypeFrame(m_PackRef);
	}
	return false;
}

char* KPakFile::ReadTypeFrame(unsigned int& uSize)
{
	if (g_pPakList && m_PackRef.nPackIndex >= 0)
	{
		return g_pPakList->ReadTypeFrame(m_PackRef, uSize);
	}
	return NULL;
}
