...
struct IInlinePicEngineSink
{
	...
	//绘制指定图片
	virtual long DrawPic(unsigned short wIndex, int x, int y, unsigned char nAlpha = 255) = 0;
...
extern "C"
{
	...
	//获取编码文本的行数与最大行宽
	int	TGetEncodedTextLineCount(const char* pBuffer, int nCount, int nWrapCharaNum, int& nMaxLineLen, int nFontSize, int nSkipLine = 0, int nNumLineLimit = 0, int bPicSingleLine = false);
	int	TGetEncodedTextLineCountAE(char* pBuffer, int nCount, int nWrapCharaNum, int& nMaxLineLen, int nFontSize, int nSkipLine, int nNumLineLimit, int bPicSingleLine, int& nAddEnterLen);
