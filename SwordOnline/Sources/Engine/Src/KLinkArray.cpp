...
void KLinkArray::Init(int nSize, int nOwnerType)
{
	if (pNode)
	{
		delete [] pNode;
...
void KLinkArray::Remove(int nIdx)
{
	...
	{
		g_DebugLog("[error]Node:%d Remove twice, owner[%d]", nIdx,nOwner);
		return;
	}
...
void KLinkArray::Insert(int nIdx)
{
	...
	{
		g_DebugLog("[error]Node:%d Insert twice, owner[%d]", nIdx,nOwner);
		return;
	}
