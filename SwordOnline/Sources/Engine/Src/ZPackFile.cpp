...
char *ZMapFile::map(unsigned int offset, unsigned int size) {
	unsigned int align = offset % 0x10000;									//64K边界
	if(m_Ptr) {
...
ZCache::ZCache(int size) {
	int align = size % 0x10000;				//必须是64k对齐的，不然会降低效率
...
char *ZCache::getNode(unsigned int index_high, unsigned int index_low, int size) {			//得到一个空闲缓冲区数据块
	size += sizeof(item_info);
	unsigned int align = size % MINIMIZE_BLOCK_SIZE;
...
char *ZCache::searchNode(unsigned int index_high, unsigned int index_low) {
	m_Mutex.Lock();
...
int ZPackFile::getNodeIndex(unsigned int id) {								//二分法找到指定的索引
...
unsigned int ZPackFile::getSize(unsigned int index) {
...
bool ZPackFile::_readData(int node_index, char *node) {
	...
	unsigned int compress_type = index_list[node_index].compress_size >> 24;
	int r = 1;
...
unsigned int g_hash(const char *file_name) {
	unsigned int id = 0;
...
char *ZPackFile::getData(unsigned int id) {
