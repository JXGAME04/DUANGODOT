...
int TSplitString(const char* pString, int nDesirePos, int bLess)
{
	...
	if (pString)
	{
		...
		{
			if (!m_bTCVN && (unsigned char)pString[nPos] > 0x80)
				nPos += 2;
	...
	if (pBuffer)
	{
		...
		while(nReadPos < nCount)
		{
			...
			else if (cCharacter == 0x0d)	//换行
			{
				...
				cCharacter == 0x0a || cCharacter == 0x09 || cCharacter == 0x0b)
			{
				pBuffer[nShortCount++] = cCharacter;
				nReadPos++;
				if(cCharacter == 0x0b && nReadPos < nCount)
				{
	...
	case KTC_COLOR:
		...
		else if (nParamLen < 12)
		{
			...
			for (int i = 0; i < s_nColorCount; i++)
			{
				...
				{
					UINT uColor = (UINT)strtoul(Color, NULL, 0);
					pEncodedBuffer[nEncodedSize] = KTC_COLOR;
					pEncodedBuffer[nEncodedSize + 1]= (uColor >> 16) & 0xff;
					pEncodedBuffer[nEncodedSize + 2]= (uColor >> 8) & 0xff;
					pEncodedBuffer[nEncodedSize + 3]= uColor & 0xff;
					nEncodedSize += 4;
					break;
				}
#ifndef __linux
				else if (stricmp(Color,s_ColorTable[i].Token) == 0)
#else
				else if(strcasecmp(Color,s_ColorTable[i].Token) == 0)
#endif
				{
					pEncodedBuffer[nEncodedSize] = KTC_COLOR;
							 ...
							 BOOL bPicSingleLine/* = FALSE*/)
{
	...
	while(nPos < nCount)
	{
		...
		if (!m_bTCVN && bNextLine == false && fNumChars && fNumChars + 3 >= nWrapCharaNum)
		{
...
extern "C" ENGINE_API
int	TGetEncodedTextLineCountAE(char* pBuffer, int nCount, int nWrapCharaNum, int& nMaxLineLen,
		int nFontSize, int nSkipLine, int nNumLineLimit, BOOL bPicSingleLine, int& nAddEnterLen)
{
	_ASSERT(nFontSize >= 4 && nFontSize < 64);
	nFontSize = max(4, nFontSize);
	nFontSize = min(64, nFontSize);
...
int	TFindSpecialCtrlInEncodedText(const char* pBuffer, int nCount, int nStartPos, char cControl)
{
	...
	if (pBuffer)
	{
		while(nStartPos < nCount)
		{
			...
			if (!m_bTCVN && cCharacter > 0x80)	//可能是中文文字
				nStartPos += 2;
			else if (cCharacter == KTC_COLOR || cCharacter == KTC_BORDER_COLOR)
				nStartPos += 4;
			else if (cCharacter == KTC_INLINE_PIC)
	...
	if (pBuffer)
	{
		if ((unsigned char)(cControl) <= 0x80)
		{
			...
			while(nReadPos < nCount)
			{
				...
				else if (!m_bTCVN && cCharacter > 0x80)
				{
