#ifndef ZPACKFILE_H
#define ZPACKFILE_H

//#include <windows.h>
//#include "ucl/ucl.h"
#include "KWin32.h"
#include "KMutex.h"
#define MINIMIZE_MAP_SIZE		65536 * 1024			//最小映射的大小为64M

class ZMapFile {
public:
	HANDLE m_hFile;
	HANDLE m_hMap;
	char *m_Ptr;						//可以直接访问的缓冲区指针
	unsigned int m_Size;				//文件的大小
	unsigned int old_offset;
	ZMapFile(const char *file_name);
	char *map(unsigned int offset = 0, unsigned int size = 0);
	void unmap();
	~ZMapFile();
};

typedef struct {
	unsigned int index_high;
	unsigned int index_low;
	int offset; 
	int size; 
} item_info;

#define MINIMIZE_BLOCK_SIZE	16					//一个块最少16个字节
#define LAST_ITEM			10

class ZCache {
	char *buffer;								//实际的缓冲区
	item_info *free_items;						//
	int cache_size;
	KMutex m_Mutex;
	item_info last_item[LAST_ITEM];
	int last;
public:
	ZCache(int size);
	virtual ~ZCache();
	char *getNode(unsigned int index_high, unsigned int index_low, int size);
	char *searchNode(unsigned int index_high, unsigned int index_low = 0xfffffffful);
};

//---------------------------------------------------------------------------------------------------------------------------------
typedef struct {		//索引信息
	unsigned int id;
	unsigned int offset;
	int size;
	int compress_size;
} index_info;

#define TYPE_NONE			0					//没有压缩
#define TYPE_UCL			1					//UCL压缩
#define TYPE_BZIP2			2					//bzip2压缩
#define TYPE_FRAME			0x10				//使用了独立帧压缩

unsigned int g_hash(const char *file_name);									//计算指定字符串的hash值

//一个Pack文件具有下面的结构:
//首先是四个字节的文件的头标志:字符串'PACK',然后是项的数目然后是索引开始的偏移量\数据开始的偏移量,然后是校验和,然后是保留的字节:
typedef struct {
	unsigned char signature[4];			//"PACK"
	unsigned int count;				//数据的条目数
	unsigned int index_offset;			//索引的偏移量
	unsigned int data_offset;			//数据的偏移量
	unsigned int crc32;
	unsigned char reserved[12];
} z_pack_header;

class ZPackFile {
protected:
	ZCache		*cache;
	ZMapFile	*data_map;
	index_info	*index_list;
	z_pack_header header;
	bool _readData(int node_index, char *node);								//将指定node_index的数据全部读到指定内存中，如果需要的话完成解压缩
public:
	bool opened;
	ZPackFile(const char *file_name, ZCache *the_cache);							//打开文件
	virtual ~ZPackFile();
	int getNodeIndex(unsigned int id);
	char *getData(unsigned int index);
	unsigned int getSize(unsigned int index);
	char *getData(const char *name);												//获取指定节点的数据
};

#endif