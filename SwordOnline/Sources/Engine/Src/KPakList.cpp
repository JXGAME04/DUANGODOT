							...
							unsigned int byuId)
{
	...
	if(byuId)
	{
		...
		{
			if (m_PakFilePtrList[i]->FindElemFile(byuId, ElemRef))
			{
				g_DebugLog("filefound int pak[%d][%x][%u]",i, byuId, byuId);
				bFounded = true;
				break;
			}
		}
	}
	else if (pszFileName && pszFileName[0])
	{
		char szPackName[128];
		#ifdef WIN32
			szPackName[0] = '\\';
		...
		for (int i = nExNum; i < m_nPakNumber; i++)
		{
			if (m_PakFilePtrList[i]->FindElemFile(uId, ElemRef))
			{
				g_DebugLog("filefound int pak[%d][%x][%s]",i, uId, pszFileName);
				bFounded = true;
				break;
			}
		}
...
bool KPakList::IsTypeFrame(XPackElemFileRef& ElemRef)
{
	if (ElemRef.nPackIndex < m_nPakNumber)
		return m_PakFilePtrList[ElemRef.nPackIndex]->IsTypeFrame(ElemRef);
	return false;
}

char* KPakList::ReadTypeFrame(XPackElemFileRef& ElemRef, unsigned int& uSize)
{
	if (ElemRef.nPackIndex < m_nPakNumber)
		return m_PakFilePtrList[ElemRef.nPackIndex]->ReadTypeFrame(ElemRef, uSize);
	return NULL;
}
