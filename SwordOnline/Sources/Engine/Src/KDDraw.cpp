...
BOOL KDirectDraw::GetSurfaceDesc()
{
	...
	ddsd.dwSize = sizeof(ddsd);
	ddsd.dwFlags = DDSD_ALL;
	hres = m_lpDDSPrimary->GetSurfaceDesc(&ddsd);
	if (hres == DD_OK)
	{
	g_DebugLog("Surface Width   = %d", ddsd.dwWidth);
	g_DebugLog("Surface Height  = %d", ddsd.dwHeight);
	g_DebugLog("Surface Picth   = %d", ddsd.lPitch);
	}*/
...
{
	RECT	rc = {0, 0, (LONG)m_dwScreenWidth, (LONG)m_dwScreenHeight};
