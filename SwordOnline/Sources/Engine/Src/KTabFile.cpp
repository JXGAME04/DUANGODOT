...
BOOL KTabFile::Save(LPSTR FileName)
{
	...
	if (!File.Create((char*)FileName))
		return FALSE;
	LPVOID DataBuf = m_Memory.GetMemPtr();
	UINT nSize	= m_Memory.GetMemLen();
	return File.Write(DataBuf, nSize);
}
...
BOOL KTabFile::SetString(int nRow, LPSTR szColumn, LPSTR lpRString, DWORD dwSize, BOOL bColumnLab)
{
	int nColumn;
	if (bColumnLab)
		nColumn = FindColumn(szColumn);
		...
		nColumn = Str2Col(szColumn);
	if (SetValue(nRow - 1, nColumn - 1,  lpRString, dwSize))
		return TRUE;
	return FALSE;
}

BOOL KTabFile::SetString(int nRow, int nColumn, LPSTR lpRString, DWORD dwSize)
{
	if (SetValue(nRow - 1, nColumn - 1,  lpRString, dwSize))
		return TRUE;
	return FALSE;
...
BOOL KTabFile::SetString(LPSTR szRow, LPSTR szColumn, LPSTR lpRString, DWORD dwSize)
{
...
BOOL KTabFile::SetValue(int nRow, int nColumn, LPSTR lpRString, DWORD dwSize)
{
	if (nRow >= m_Height || nColumn >= m_Width || nRow < 0 || nColumn < 0)
		return FALSE;

	LPSTR Buffer = (LPSTR)m_Memory.GetMemPtr();
	UINT nSize	= m_Memory.GetMemLen();
	TABOFFSET *TempOffset = (TABOFFSET *)m_OffsetTable.GetMemPtr();
	TempOffset += nRow * m_Width + nColumn;

	Buffer += TempOffset->dwOffset;
	if (TempOffset->dwLength == dwSize)
	{
		memcpy(Buffer, lpRString, dwSize);
		return TRUE;
	}
	int nAddOff;
	if (TempOffset->dwLength < dwSize)
		nAddOff = dwSize - TempOffset->dwLength;
	else
		nAddOff = -(int)(TempOffset->dwLength - dwSize);
	UINT nNewSize = nSize + nAddOff;
	char* pNewData = new char[nNewSize];
	char* pCopyData = pNewData;
	Buffer = (LPSTR)m_Memory.GetMemPtr();
	memcpy(pCopyData, Buffer, TempOffset->dwOffset);
	Buffer += TempOffset->dwOffset;
	Buffer += TempOffset->dwLength;
	pCopyData += TempOffset->dwOffset;
	memcpy(pCopyData, lpRString, dwSize);
	pCopyData += dwSize;
	memcpy(pCopyData, Buffer, nSize - (TempOffset->dwOffset + TempOffset->dwLength));
	TempOffset->dwLength = dwSize;
	m_Memory.Free();
	Buffer = (LPSTR)m_Memory.Alloc(nNewSize);
	memcpy(Buffer, pNewData, nNewSize);
	delete [] pNewData;
	pNewData = NULL;
	for(int i = nRow * m_Width + nColumn; i < (m_Width * m_Height - 1); i++)
	{
		TempOffset++;
		TempOffset->dwOffset += nAddOff;
	}
	return TRUE;
}
