...
//---------图资源相关---------------
    ...
    virtual unsigned int CreateImage(
        ...
        int nHeight, 
        //## 图形类型
        int nType, bool bNodeCache = false, bool bScrMode = false) = 0;
    ...
    virtual void* GetBitmapDataBuffer(
		...
		KBitmapDataBuffInfo* pInfo, int nType = 0/*ISI_T_BITMAP16*/) = 0;
...
//---------绘制操作---------------
    ...
    virtual void DrawPrimitives(
        ...
        int bSinglePlaneCoord, int bFindById = 0) = 0;
	...
	virtual void DrawPrimitivesOnImage(
        ...
        //## 目标图形的id
        unsigned int uImage,
		short& nImagePosition, int bFindById = 0)	= 0;
	...
	virtual void ClearAlphaImgData(
        //## 图形名
        const char* pszImage, 
        //## 目标图形的id
        unsigned int uImage,
		short nImagePosition,
		unsigned int uColor = 0)	= 0;
	virtual void UpdateAlphaImgData(
        const char* pszImage, void* pNewData, unsigned int uImage, short nImagePosition)	= 0;
	virtual void UpdateAlphaImgSrcMode(
        const char* pszImage, int bScrMode, unsigned int uImage, short nImagePosition)	= 0;
	virtual void ConvertImgRectToHardSrcMode(
        const char* pszImage, unsigned int uImage, short nImagePosition)	= 0;
	virtual void* LoadExternImg(const char* pszImage, unsigned int& uImage,
					short& nImagePosition, int nFrame, int nType, void*& pFrameData, int bFindById = 0) = 0;
    //## 设置视觉观察的目的焦点。
	...
	virtual bool UpdateWindowSize(int nWidth, int nHeight) = 0;
	...
	virtual int	GetEncodedTextLineCount(const char* pFontName, char* pBuffer, int nCount, int nWrapCharaNum,
					...
					int bPicSingleLine = false, int* nAddEnterLen = NULL) = 0;
	virtual void DrawUFontRich(const char* psFontName, unsigned int uSize, KOutputTextParam* pParam, const char* psText,
					int nCount = KRF_ZERO_END, int nLineWidth = 0) = 0;
	virtual void GetClipRect(RECT &rClip) = 0;
	virtual void SetClipRect(RECT rClip) = 0;
	virtual void FillColorTriangle(int x1, int y1, int x2, int y2, int x3, int y3, unsigned int uColor) = 0;
};
