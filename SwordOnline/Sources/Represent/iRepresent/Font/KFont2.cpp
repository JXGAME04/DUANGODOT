					...
					unsigned int nColor/*=0xff000000*/, int nLineWidth/*=0*/)
{
	...
	{
		UINT a = nColor & 0xff000000;
		KRColor		c;
		c.Color_dw = nColor;
		nColor = g_RGB(c.Color_b.r, c.Color_b.g, c.Color_b.b);
		nColor |= a;
	}
	if (nLineWidth < m_nOutputWidth + m_nOutputWidth)
		nLineWidth = 0; //不做自动换行处理
	BOOL bVNFont = TIsTCVNText();
	while (nPos < nCount)
	{
		//*********字符的判断与处理*********
		if (!bVNFont && lpByte[nPos] > 0x80 && nPos + 1 < nCount)
		{
			DrawCharacter(nX + h, nY, lpByte[nPos], lpByte[nPos + 1], nColor);
			nPos += 2;
			h += m_nOutputWidth;
		}
		else	//西文字符
		{
			...
			{
				if (nL > 0x20 && nL < 0x5F)
					DrawCharacter(nX + h, nY, 0xa3, (0x20 + nL), nColor);
				else if (nL >= 0x5F && nL < 0x7F)
					DrawCharacter(nX + h, nY, 0xa3, (0x21 + nL), nColor);
			}
			else
				DrawCharacter(nX + h, nY, nL, 0, nColor, TRUE);
			if (nL != 0x0a)
			{
		...
		if (nLineWidth)
		{
			...
			else if (!bVNFont && h && nPos + 3 < nCount &&
...
*****************************************************************************/
void KFont2::DrawCharacter(int x, int y, unsigned char cFirst, unsigned char cNext, int nColor, int bVNF) const
{
	if (m_pDevice && m_bLoaded)
	{
		//取得字符在字库里的数据区指针
		unsigned char* pCharacterData = NULL;
		if(!bVNF)
			pCharacterData = m_Resources.GetCharacterData(cFirst, cNext);
		else
			pCharacterData = m_Resources.GetCharacterDataVN(cFirst);
		if (pCharacterData)
		{
