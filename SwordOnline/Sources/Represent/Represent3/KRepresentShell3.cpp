...
int KRepresentShell3::Create(int nWidth, int nHeight, bool bFullScreen)
{
...
unsigned int KRepresentShell3::CreateImage(const char* pszName, int nWidth, int nHeight, int nType, bool bNodeCache, bool bScrMode)
{
	if(m_bDeviceLost)
		return 0;
...
void KRepresentShell3::DrawPrimitives(int nPrimitiveCount, KRepresentUnit* pPrimitives, unsigned int uGenre, int bSinglePlaneCoord, int bFindById)
{
	if(!pPrimitives)
	{
		assert(pPrimitives);
...
void* KRepresentShell3::GetBitmapDataBuffer(const char* pszImage, KBitmapDataBuffInfo* pInfo, int nType /*ISI_T_BITMAP16*/)
{
	if(!pszImage || !pszImage[0])
		return NULL;
