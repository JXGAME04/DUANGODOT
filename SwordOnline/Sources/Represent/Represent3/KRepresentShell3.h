...
//---------绘制操作---------------
	...
	void* LoadExternImg(const char* pszImage, unsigned int& uImage,
				short& nImagePosition, int nFrame, int nType, void*& pFrameData, int bFindById = 0){return NULL;};
    //## 设置视觉观察的目的焦点。
    //## (把输入的坐标作调整使它在z=0平面上)，
	...
	virtual bool UpdateWindowSize(int nWidth, int nHeight){return false;};
	...
	virtual int	GetEncodedTextLineCount(const char* pFontName, char* pBuffer, int nCount, int nWrapCharaNum,
					...
					int bPicSingleLine = false, int* nAddEnterLen = NULL){return 0;};
