...
//---------图资源相关---------------
    ...
    unsigned int CreateImage(
		...
		//##Documentation
        //## 图形类型
        int nType, bool bNodeCache = false, bool bScrMode = false);
    ...
    void* GetBitmapDataBuffer(
		...
		KBitmapDataBuffInfo* pInfo, int nType = 0/*ISI_T_BITMAP16*/);
...
//---------绘制操作---------------
    ...
    void DrawPrimitives(
        ...
        int bSinglePlaneCoord, int bFindById = 0);
	...
	void DrawPrimitivesOnImage(
        ...
        //## 目标图形的id
        unsigned int uImage,
		short&	nImagePosition, int bFindById = 0);
	...
	void UpdateAlphaImgData(
        const char* pszImage, void* pNewData, unsigned int uImage, short nImagePosition);
	void UpdateAlphaImgSrcMode(
        const char* pszImage, int bScrMode, unsigned int uImage, short nImagePosition);
	void ConvertImgRectToHardSrcMode(
        const char* pszImage, unsigned int uImage, short nImagePosition);
	void* LoadExternImg(const char* pszImage, unsigned int& uImage,
				short& nImagePosition, int nFrame, int nType, void*& pFrameData, int bFindById = 0);
	...
	virtual bool UpdateWindowSize(int nWidth, int nHeight);
	virtual void DrawUFont(const char* psFontName, unsigned int uSize, const char* psText,
        int nCount = KRF_ZERO_END,
        int nX = 0,
        int nY = 0,
		...
		int nZ = TEXT_IN_SINGLE_PLANE_COORD,
		unsigned int BorderColor = 0);
	virtual int	GetEncodedTextLineCount(const char* pFontName, char* pBuffer, int nCount, int nWrapCharaNum,
					int& nMaxLineLen, int nFontSize, int nSkipLine = 0, int nNumLineLimit = 0,
					int bPicSingleLine = false, int* nAddEnterLen = NULL);
	virtual void DrawUFontRich(const char* psFontName, unsigned int uSize, KOutputTextParam* pParam,
					const char* psText, int nCount = KRF_ZERO_END, int nLineWidth = 0);
	virtual void GetClipRect(RECT &rClip);
	virtual void SetClipRect(RECT rClip);
	virtual void FillColorTriangle(int x1, int y1, int x2, int y2, int x3, int y3, unsigned int uColor);
private:
...
private:
    ...
    void CoordinateTransform(
        ...
        //## 传入三维坐标z分量
        int nZ);
	inline void DrawPointLine(int sx, int ex, int ny, unsigned int uColor = 0xffffffff, BOOL bScreenMode = FALSE, bool bCheckDupl = false);
	void DrawTriangle(int x1, int y1, int x2, int y2, int x3, int y3, unsigned int uColor = 0xffffffff);
	void FillTriangle(int x1, int y1, int x2, int y2, int x3, int y3, unsigned int uColor = 0xffffffff, bool bCheckDupl = false);
	void DrawUFontChar(UCharPIXInfo* pPXInfo, unsigned int uSize, int nX, int nY,
						UINT Color, UINT BorderColor, UINT al, UINT ab);
	void DrawTriangleOnBuffer(int x1, int y1, float u1, float v1,
						int x2, int y2, float u2, float v2,
						int x3, int y3, float u3, float v3,
						int nNewWidth, int nNewHeight, UINT*& pDesData,
						BYTE *tex, int nWidth, int nHeight);
	UINT* ImageScaleBuffer(int nNewW, int nNewH, AlphaRecContent*& pSprData);
	BYTE* ArrangeSpriteData(UINT* pBuffer, BYTE*& pPalette, int nWidth, int nHeight);
	BYTE* ArrangeSpriteData32b(UINT* pBuffer, BYTE*& pPalette, int nWidth, int nHeight);
	void DrawScaleSprite(BYTE*& pSprite, BYTE*& pPal, int nX, int nY, KRUImage*& pTemp, SPRHEAD*& pSprHeader,
