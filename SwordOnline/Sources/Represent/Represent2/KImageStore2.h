...
//====针对单个图形的操作====
    ...
    void* GetImage(
        ...
        int nType,
		//额外需要的帧数据buffer
		void*& pFrameData, int bFindById = 0);
	...
	char* GetAdjustColorPalette(
		...
		unsigned uColor);//偏色量
	void* CreateSprData(int& bExistData, int nWidth, int nHeight, unsigned int uImage,
						short nImagePosition, int nFrame, int nTotalFrame);
	unsigned char* FindScaleData(unsigned int uImage, short nImagePosition, int nFrame,
								int nWidth, int nHeight, unsigned char*& pPal);
	void AddScaleData(unsigned int uImage, short nImagePosition, int nFrame, int nWidth, int nHeight,
						unsigned char* pSprite, unsigned char* pPal);
	void FreeScaleData(unsigned int uImage, short nImagePosition);
private:
    ...
    struct _KISImageObj
    {
		...
		ScaleImageInfo* pScaleImg;
		AlphaRecContent** pImgData;
		int	nFrameNum;
		int	nScaleNum;
    };
