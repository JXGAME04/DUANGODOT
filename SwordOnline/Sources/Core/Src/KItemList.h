...
private:
	...
	static int	ms_ActiveEquip[itempart_horse][MAX_ITEM_ACTIVE];	// 某个部分的装备所激活的装备
	static int	ms_ActivedEquip[itempart_horse][MAX_ITEM_ACTIVE];	// 某个部分的装备被什么装备激活
...
public:
	...
	void		RecoverTrade();
	void		StartTrade();
	BOOL		IsFullGoldSet(int nEIdx);
	BOOL		IsActivateAll() {return m_bAllActived;};
	BOOL		CheckCanPlaceInEquipment(int nWidth, int nHeight, int *pnX, int *pnY, int nRoom = room_equipment);// 判断一定长宽的物品能否放进物品栏
#ifdef	_SERVER
	...
	int			GetTaskItemNum(int nGenre, int nDetailType, int nParticular,
						int nRoomPos = pos_equiproom, int nLevel = 0, int nSeries = -1);
	BOOL		RemoveTaskItem(int nGenre, int nDetailType, int nParticular, int nCount = 1,
						int nRoomPos = pos_equiproom, int nLevel = 0, int nSeries = -1);
	...
	// 丢失一个穿在身上的装备(马不会掉)
	void		AutoLoseEquip();
	void		ClearItem(int nRoomPos = pos_equiproom);
#endif
