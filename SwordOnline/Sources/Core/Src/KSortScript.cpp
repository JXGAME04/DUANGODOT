...
unsigned int   g_nCurrentScriptNum = 0;
...
static BOOL LoadScriptToSortListA(char * szRelativeFile)
{
...
#endif
	if (g_nCurrentScriptNum < MAX_SCRIPT_IN_SET)
	{
		g_ScriptSet[g_nCurrentScriptNum].Init();
		g_ScriptSet[g_nCurrentScriptNum].RegisterFunctions(GameScriptFuns, g_GetGameScriptFunNum());
		g_StrCpyLen(g_ScriptSet[g_nCurrentScriptNum].m_szScriptName, szRelativeFile, sizeof(g_ScriptSet[g_nCurrentScriptNum].m_szScriptName));
		if (g_ScriptSet[g_nCurrentScriptNum].Load(szRelativeFile))
		{
	...
	ScriptNode.SetScriptIndex(g_nCurrentScriptNum++);
...
//	PathRelativePathTo(szRelativePath,szRootPath, FILE_ATTRIBUTE_DIRECTORY,szCurrentDirectory , FILE_ATTRIBUTE_NORMAL );
	...
	//defDEBUG_TRAP
	//g_DebugLog("[Script][%u][%s]", g_FileName2Id(szRelativeFile), szRelativeFile);
//
