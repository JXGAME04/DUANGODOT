...
int KItemList::ms_ActiveEquip[itempart_horse][MAX_ITEM_ACTIVE] = 
{
...
int KItemList::ms_ActivedEquip[itempart_horse][MAX_ITEM_ACTIVE] =
{
...
//			return 0;
	...
	sItem.m_Count = Item[nIdx].GetStack();
	if(Item[nIdx].GetExType() == extype_gold)
	{
		sItem.m_Genre = item_gengold;
		sItem.m_Detail = Item[nIdx].GetGenParam();
	}
	else if(Item[nIdx].GetExType() == extype_platina)
	{
		sItem.m_Genre = item_genplatina;
		sItem.m_Detail = Item[nIdx].GetGenParam();
	}
	else if(Item[nIdx].GetExType() == extype_purple)
	{
		sItem.m_Genre = item_genpurple;
		sItem.m_Count = Item[nIdx].GetGenParam();
	}
	...
	for (j = 0; j < 6; j++)
	{
		sItem.m_MagicLevel[j*2] = Item[nIdx].m_aryMagicAttrib[j].nAttribType;
		sItem.m_MagicLevel[j*2+1] = Item[nIdx].m_aryMagicAttrib[j].nValue[0];
	}
	for (j = 0; j < 2; j++)
	{
		sItem.m_MagicLevel[12+j*2] = Item[nIdx].m_aryMagicAttribEx[j].nAttribType;
		sItem.m_MagicLevel[12+j*2+1] = Item[nIdx].m_aryMagicAttribEx[j].nValue[0];
	}
...
BOOL KItemList::Equip(int nIdx, int nPlace /* = -1 */)
{
	...
	case itempart_horse:
		...
		if (Npc[nNpcIdx].m_HorseType < 0)
			Npc[nNpcIdx].m_HorseType = 0;
		Npc[nNpcIdx].m_bRideHorse = TRUE;
		break;
	case itempart_mantle:
		if(Item[nIdx].GetExType() == extype_gold)
		Npc[nNpcIdx].m_MantleType = g_ItemChangeRes.GetGoldRes(Item[nIdx].GetGenParam());
		else
	...
	if(!m_bAllActived)	//b¾t ®Çu check ®ñ bé hoµng kim
	{
		m_bAllActived = IsFullGoldSet(nIdx);
		if(m_bAllActived)
		{
	...
	Item[nIdx].ApplyMagicAttribToNPC(&Npc[nNpcIdx], nActive, m_bAllActived);
	if(!m_bAllActived)
...
BOOL KItemList::IsFullGoldSet(int nEIdx)
{
	if(Item[nEIdx].GetExType() != extype_gold || Item[nEIdx].GetDetailType() >= equip_horse)
		return FALSE;
	int nGroupId, nExGroupId, nGroupSerial;
	Item[nEIdx].GetGroups(nGroupId, nExGroupId, nGroupSerial);
	if(nGroupId && !nExGroupId && !nGroupSerial)
	{
		int nCount = 0;
		int nBgIdx = ItemGen.FindGoldStartId(nGroupId);
		if(nBgIdx > 0)
		{
			--nBgIdx;
			int nEndIdx = -1;
		...
		if(nCount > 0)	//sè l­îng item trong 1 bé
		{
			BOOL bExistAry[10];
			ZeroMemory(bExistAry, sizeof(BOOL)*10);
			int i;
			for (i = 0; i < itempart_horse; ++i)
			{
				int nIdx = m_EquipItem[i];
				if(nIdx > 0 && //duyÖt c¸c item cïng group
				Item[nIdx].GetExType() == extype_gold &&
				Item[nIdx].m_nGroupId == nGroupId)
				{
					int nCheckBg = -1;
					int b;
					for(b = Item[nIdx].m_nGenParam-1; b >= 0; --b)
					{
					...
					if(nCheckBg < 0)
						nCheckBg = 0;
					if((Item[nIdx].m_nGenParam-1) - b > nCount)	//2 bé cïng group n»m gÇn nhau
						nCheckBg += nCount;
					nCheckBg -= nBgIdx;	//kho¶ng c¸ch víi bé ®Çu tiªn
					bExistAry[(Item[nIdx].m_nGenParam-1)-nCheckBg-nBgIdx] = TRUE;
				}
			}
			for(i=0; i<nCount; ++i)
			{
	...
	else if(nGroupId && nExGroupId && nGroupSerial)
	{
		int nBgIdx = ItemGen.FindGoldStartId(nGroupId);
		if(nBgIdx > 0)
		{
			...
			if(nCount > 0)
			{
				...
				for(i = 0; i < nCount; ++i)
				{
					...
					{
						int nIdx = m_EquipItem[k];
						if(nIdx > 0 && Item[nIdx].GetExType() == extype_gold
						&& p->m_nExGroupId == Item[nIdx].m_nExGroupId
						&& p->m_nGroupSerial == Item[nIdx].m_nGroupSerial)
						{
							aryGrSerieal[p->m_nGroupSerial-1] = TRUE;
						}
					}
				}
				int nActCount = 0, nExistCount = 0;
				for(i=0; i<nCount; ++i)
				{
					if(aryGrSerieal[i])
						++nExistCount;
				}
				for(i=0; i < ItemGen.GetSuitACNumber();++i)
				{
					const KEQCP_REQ* pSAC = ItemGen.GetSuitACRecord(i);
					if(pSAC->nType == nGroupId)
					{
						nActCount = pSAC->nPara;
						break;
					}
				}
				if(nActCount == 0)
					nActCount = nCount;
				if(nExistCount >= nActCount)
					return TRUE;
			}
		}
	}
...
BOOL KItemList::UnEquip(int nIdx, int nPos/* = -1*/)
{
	...
	if(i != itempart_horse || Npc[nNpcIdx].m_bRideHorse)
	Item[nIdx].RemoveMagicAttribFromNPC(&Npc[nNpcIdx], nActive, m_bAllActived);
	nIdx = m_EquipItem[i];
	m_EquipItem[i] = 0;
	if(m_bAllActived)
	{
		m_bAllActived = FALSE;
		for(int e=0; e < itempart_horse; ++e)
		{
			if(m_EquipItem[e] > 0)
			{
				m_bAllActived = IsFullGoldSet(m_EquipItem[e]);
				if(m_bAllActived)
					break;
			}
		}
...
int KItemList::GetEquipEnhance(int nPlace)
{
	...
	if(m_bAllActived || nPlace >= itempart_horse)
		return 3;
...
BOOL KItemList::EatMecidine(int nIdx)
{
	...
	else if(nGenre == item_mascript)
	{
#ifdef _SERVER
		const KBASICPROP_MASCRIPT* pMAScript = ItemGen.GetMAScriptRecord(Item[nIdx].GetGenParam());
		if(!pMAScript)
			return FALSE;
		UINT dwScriptId = g_FileName2Id(const_cast<char*>(pMAScript->m_szScript));
		KLuaScript * pScript = (KLuaScript* )g_GetScript(dwScriptId);
		if(!pScript)
			return FALSE;
		Npc[nNpcIdx].m_ActionScriptID = dwScriptId;
		Lua_PushNumber(pScript->m_LuaState, m_PlayerIdx);
		pScript->SetGlobalName(SCRIPT_PLAYERINDEX);		
		Lua_PushNumber(pScript->m_LuaState, Npc[nNpcIdx].m_SubWorldIndex);
		pScript->SetGlobalName(SCRIPT_SUBWORLDINDEX);
		int nTopIndex = 0;
		int nReturn = 1;	//hµm main tr¶ l¹i 0 lµ xãa
		try
		{
			pScript->SafeCallBegin(&nTopIndex);
			pScript->CallFunction("main", 1, "");
		...
		{
			Npc[nNpcIdx].m_ActionScriptID = 0;
			Player[m_PlayerIdx].m_bWaitingPlayerFeedBack = false;
			return FALSE;
		}
		if(!nReturn)
		{
			if(Item[nIdx].m_CommonAttrib.nStack <= 1)
			{
				Remove(nIdx);
				ItemSet.Remove(nIdx);
			}
			...
			{
				--Item[nIdx].m_CommonAttrib.nStack;
				char szPack[16];
				DYNAMIC_COMMAND* pCmd = (DYNAMIC_COMMAND*)&szPack[0];
				pCmd->ProtocolType = s2c_dynamic_structure;
				pCmd->nBranch = s2cdnmbr_itemstack;
				pCmd->m_wLength = sizeof(DYNAMIC_COMMAND) - 1 + sizeof(UINT)*2;
				UINT* p = (UINT*)(pCmd+1);
				*p++ = Item[nIdx].GetID();
				*p = Item[nIdx].m_CommonAttrib.nStack;
				g_pServer->PackDataToClient(Player[m_PlayerIdx].m_nNetConnectIdx, pCmd, pCmd->m_wLength + 1);
			}
		}
	...
	else if (nGenre == item_townportal)
	{
		if (!Npc[Player[m_PlayerIdx].m_nIndex].m_FightMode)
		{
			...
			strcpy(Msg.szMessage, "ÎÞ·¨ÔÚÕâÀïÊ¹ÓÃÍÁµØ·û£¡");
	...
	case pos_traderoom:
		...
		if (m_Hand)
		{
			if (m_Room[room_trade].PlaceItem(DesPos->nX, DesPos->nY, m_Hand, Item[m_Hand].GetWidth(), Item[m_Hand].GetHeight()))
			{
				...
				sItem.m_Count = Item[m_Hand].GetStack();
				if(Item[m_Hand].GetExType() == extype_gold)
				{
					sItem.m_Genre = item_gengold;
					sItem.m_Detail = Item[m_Hand].GetGenParam();
				}
				else if(Item[m_Hand].GetExType() == extype_platina)
				{
					sItem.m_Genre = item_genplatina;
					sItem.m_Detail = Item[m_Hand].GetGenParam();
				}
				else if(Item[m_Hand].GetExType() == extype_purple)
				{
					sItem.m_Genre = item_genpurple;
					sItem.m_Count = Item[m_Hand].GetGenParam();
				}
				...
				for (j = 0; j < 6; j++)
				{
					sItem.m_MagicLevel[j*2] = Item[m_Hand].m_aryMagicAttrib[j].nAttribType;
					sItem.m_MagicLevel[j*2+1] = Item[m_Hand].m_aryMagicAttrib[j].nValue[0];
				}
				for (j = 0; j < 2; j++)
				{
					sItem.m_MagicLevel[12+j*2] = Item[m_Hand].m_aryMagicAttribEx[j].nAttribType;
					sItem.m_MagicLevel[12+j*2+1] = Item[m_Hand].m_aryMagicAttribEx[j].nValue[0];
				}
...
#ifdef _SERVER
int		KItemList::GetTaskItemNum(int nGenre, int nDetailType, int nParticular,
						int nRoomPos/* = pos_equiproom*/, int nLevel/* = 0*/, int nSeries/* = -1*/)
{
	if(nRoomPos <= 0)
		nRoomPos = pos_equiproom;
	int		nNo = 0;
	int		nIdx = 0;
	while ((nIdx = m_UseIdx.GetNext(nIdx)))
	{
		if(m_Items[nIdx].nPlace != nRoomPos)
			continue;
		int nGameIdx = m_Items[nIdx].nIdx;
		if (nGenre != Item[nGameIdx].GetGenre()
		|| nDetailType != Item[nGameIdx].GetDetailType()
		|| nParticular != Item[nGameIdx].GetParticular())
			continue;
		if(nLevel > 0 && Item[nGameIdx].GetLevel() != nLevel)
			continue;
		if(nSeries >= 0 && nSeries < series_num && nSeries != Item[nGameIdx].GetSeries())
			continue;
		nNo += Item[nGameIdx].GetStack();
	}
	return nNo;
}
#endif
...
#ifdef _SERVER
BOOL KItemList::RemoveTaskItem(int nGenre, int nDetailType, int nParticular, int nCount/* = 1*/,
						int nRoomPos/* = pos_equiproom*/, int nLevel/* = 0*/, int nSeries/* = -1*/)
{
	if(nCount <= 0)
		nCount = 1;
	if(nRoomPos <= 0)
		nRoomPos = pos_equiproom;
	std::vector<int> vDelItem;//thïng chøa item idx sÏ xãa
	int nIdx = 0;
	while ((nIdx = m_UseIdx.GetNext(nIdx)))
	{
		if(m_Items[nIdx].nPlace != nRoomPos)
		...
		if(nSeries >= 0 && nSeries < series_num && nSeries != Item[nGameIdx].GetSeries())
			continue;
		int nStack = Item[nGameIdx].GetStack();
		if(nCount >= nStack)	//vÉn cßn sè l­îng cÇn xãa ë loop tiÕp theo
		{
			vDelItem.push_back(nGameIdx);
			nCount -= nStack;
			if(!nCount)	//®· xãa hÕt sè l­îng
				break;
			continue;
		}
		nStack -= nCount;	//®· xãa hÕt sè l­îng, update xÕp chång cßn l¹i cho item nµy
		Item[nGameIdx].m_CommonAttrib.nStack = nStack;
		char szPack[16];
		DYNAMIC_COMMAND* pCmd = (DYNAMIC_COMMAND*)&szPack[0];
		pCmd->ProtocolType = s2c_dynamic_structure;
		pCmd->nBranch = s2cdnmbr_itemstack;
		pCmd->m_wLength = sizeof(DYNAMIC_COMMAND) - 1 + sizeof(UINT)*2;
		UINT* p = (UINT*)(pCmd+1);
		*p++ = Item[nGameIdx].GetID();
		*p = nStack;
		g_pServer->PackDataToClient(Player[m_PlayerIdx].m_nNetConnectIdx, pCmd, pCmd->m_wLength + 1);
		break;
	}
	for(size_t i = 0; i < vDelItem.size();++i)
	{
		Remove(vDelItem[i]);
		ItemSet.Remove(vDelItem[i]);
	}
	vDelItem.clear();
	if(!nCount)
		return TRUE;
...
void	KItemList::ClearItem(int nRoomPos)
{
	if(nRoomPos <= 0)
		nRoomPos = pos_equiproom;
	std::vector<int> vDelItem;//thïng chøa item idx sÏ xãa
	...
	while ((nIdx = m_UseIdx.GetNext(nIdx)))
	{
		if(m_Items[nIdx].nPlace != nRoomPos)
			continue;
		vDelItem.push_back(m_Items[nIdx].nIdx);
	}
	for(size_t i = 0; i < vDelItem.size();++i)
	{
		Remove(vDelItem[i]);
