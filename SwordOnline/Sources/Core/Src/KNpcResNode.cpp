...
//		g_SetFilePath(RES_INI_FILE_PATH);
		...
		if(nFindNo == 2)
		{
			strcpy(m_nSectInfo[10].szSectResName, "男主角左手武器_effect.txt");
			strcpy(m_nSectInfo[10].szSectSprInfoName, "男主角左手武器信息_effect.txt");
			strcpy(m_nSectInfo[11].szSectResName, "男主角右手武器_effect.txt");
			strcpy(m_nSectInfo[11].szSectSprInfoName, "男主角右手武器信息_effect.txt");
			m_nSectInfo[10].nFlag = 1;
			m_nSectInfo[11].nFlag = 1;
		}
		else
		{
			strcpy(m_nSectInfo[10].szSectResName, "女主角左手武器_effect.txt");
			strcpy(m_nSectInfo[10].szSectSprInfoName, "女主角左手武器信息_effect.txt");
			strcpy(m_nSectInfo[11].szSectResName, "女主角右手武器_effect.txt");
			strcpy(m_nSectInfo[11].szSectSprInfoName, "女主角右手武器信息_effect.txt");
			m_nSectInfo[10].nFlag = 1;
			m_nSectInfo[11].nFlag = 1;
		}
		// 得到每个部件的具体的资源文件名
...
//				g_SetFilePath(RES_INI_FILE_PATH);
				...
				{
					UINT uColor = 0;
					SectFile.GetString( j + 2, "ChangeColor", "", szTemp, sizeof(szTemp));
					if(szTemp[0])
					{
						uColor = (UINT)strtoul(szTemp, NULL, 16);
						uColor |= 0xff000000;
					}
...
//							80);
						...
						m_cResInfo[i].m_cSprInfo[j * nActionCount + k].uColor = uColor;
						//sscanf(szTemp, "%d,%d,%d", 
...
UINT		KNpcResNode::GetColor(int nPartNo, int nActionNo, int nEquipNo)
{
	if (nPartNo < 0 || nPartNo >= MAX_PART)
		return 0;

	return m_cResInfo[nPartNo].GetColor(nActionNo, nEquipNo);
}
//---------------------------------------------------------------------------
// 功能:	获得某武器某状态下的动作编号
//---------------------------------------------------------------------------
...
//	g_SetFilePath(RES_INI_FILE_PATH);
	...
	for (i = 0; i < MAX_RES_STATE; i++)
	{
		...
		cTabFile.GetString(i + 2, 5, "", szBuffer, sizeof(szBuffer));
		if (strcmp(szBuffer, "Loop") == 0)
			m_nPlayType[i] = 0;
		else
			m_nPlayType[i] = 1;
		cTabFile.GetInteger(i + 2, 6 , 0, &m_nBackStart[i]);
		cTabFile.GetInteger(i + 2, 7 , 0, &m_nBackEnd[i]);
		cTabFile.GetInteger(i + 2, 8 , 1, &m_nTotalFrame[i]);
		cTabFile.GetInteger(i + 2, 9 , 1, &m_nTotalDir[i]);
		cTabFile.GetInteger(i + 2, 10, 1, &m_nInterVal[i]);
...
UINT		CRESINFO::GetColor(int nActionNo, int nEquipNo)
{
	if (nActionNo < 0 || nActionNo >= m_nActionKind || nEquipNo < 0 || nEquipNo >= m_nEquipKind)
		return 0;

	return m_cSprInfo[nEquipNo * m_nActionKind + nActionNo].uColor;
}
//--------------------------- class CRESINFO end ----------------------------
