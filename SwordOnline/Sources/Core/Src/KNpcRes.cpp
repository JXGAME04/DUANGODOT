...
//	功能：	绘制
//---------------------------------------------------------------------------
void	KNpcRes::Draw(int nNpcIdx, int nDir, int nAllFrame, int nCurFrame, BOOL bInMenu, BOOL bOther)
{
	...
	if (nAllFrame > 0)
	{
		...
		bool bSpecRes = false;
		if(!m_bRideHorse && m_nArmorType >= 42 && m_nArmorType <= 46
		&& (m_nDoing <= cdo_run || m_nDoing == cdo_sit))
		{
			for (i = MAX_BODY_PART_SECT; i < MAX_BODY_PART_SECT+4; i++)
			{
		...
		for (i = 0; i < MAX_PART; i++)
		{
			if (m_cNpcImage[i].CheckExist())
			{
				if (!bSpecRes && nFirst == 0)	//head part info
				{
#ifdef TOOLVERSION
					KImageParam	sImage;
				...
				else	//follow head part info
				{
					m_cNpcImage[i].m_nTotalDir = nGetDir;
					m_cNpcImage[i].m_nTotalFrame = nGetFrame;
					m_cNpcImage[i].SetCurFrame(nCurFrameNo);
					/*KImageParam	sImage;
					if (g_pRepresent->GetImageParam(m_cNpcImage[i].m_szName, &sImage, ISI_T_SPR))
					{
...
// ---------------------------------- 处理绘制列表 -------------------------------
	...
	m_cDrawFile[nPos].fScaleX = 0;
	m_cDrawFile[nPos].fScaleY = 0;
	m_cDrawFile[nPos].Color.Color_dw = 0xff000000;
	m_cDrawFile[nPos].bRenderStyle = IMAGE_RENDER_STYLE_ALPHA;
	nPos++;

	// 脚底状态特效
	for ( i = 12; i < MAX_STATE_ID; i++)
	{
	...
	for (i = 0; i < MAX_PART; i++)
	{
		if (m_nSortTable[i] >= 0 && m_nSortTable[i] < MAX_PART)
		{
			...
			&& (m_nDoing <= cdo_run || m_nDoing == cdo_sit))
			{
				if(m_nSortTable[i] < MAX_BODY_PART_SECT || m_nSortTable[i] >= MAX_BODY_PART_SECT+4)
					continue;
			}
			if(m_nSortTable[i] == 10 || m_nSortTable[i] == 11)
				continue;
			...
			m_cDrawFile[nPos].fScaleY = 0;
			nPos++;
			if(m_nSortTable[i] == 8 && m_cNpcImage[10].m_szName[0])
			{
				g_pRepresent->DrawPrimitives(nPos, m_cDrawFile, RU_T_IMAGE, bInMenu);
				nPos = 0;
				strcpy(m_cDrawFile[nPos].szImage, m_cNpcImage[10].m_szName);
				m_cDrawFile[nPos].uImage = m_cNpcImage[10].m_dwNameID;
				m_cDrawFile[nPos].nFrame = m_cNpcImage[10].m_nCurFrame;
				m_cDrawFile[nPos].oPosition.nX = nScreenX;
				m_cDrawFile[nPos].oPosition.nY = nScreenY;
				m_cDrawFile[nPos].oPosition.nZ = nScreenZ;
				m_cDrawFile[nPos].fScaleX = 0;
				m_cDrawFile[nPos].fScaleY = 0;
				m_cDrawFile[nPos].Color.Color_dw = m_cNpcImage[10].m_uColor;
				if(m_cDrawFile[nPos].Color.Color_dw == 0)
					m_cDrawFile[nPos].Color.Color_dw = 0xff000000;
				m_cDrawFile[nPos].bRenderStyle = IMAGE_RENDER_STYLE_ALPHA;
				g_pRepresent->DrawPrimitives(1, m_cDrawFile, RU_T_IMAGE, bInMenu);
			}
			else if(m_nSortTable[i] == 9 && m_cNpcImage[11].m_szName[0])
			{
				g_pRepresent->DrawPrimitives(nPos, m_cDrawFile, RU_T_IMAGE, bInMenu);
				nPos = 0;
				strcpy(m_cDrawFile[nPos].szImage, m_cNpcImage[11].m_szName);
				m_cDrawFile[nPos].uImage = m_cNpcImage[11].m_dwNameID;
				m_cDrawFile[nPos].nFrame = m_cNpcImage[11].m_nCurFrame;
				m_cDrawFile[nPos].oPosition.nX = nScreenX;
				m_cDrawFile[nPos].oPosition.nY = nScreenY;
				m_cDrawFile[nPos].oPosition.nZ = nScreenZ;
				m_cDrawFile[nPos].fScaleX = 0;
				m_cDrawFile[nPos].fScaleY = 0;
				m_cDrawFile[nPos].Color.Color_dw = m_cNpcImage[11].m_uColor;
				if(m_cDrawFile[nPos].Color.Color_dw == 0)
					m_cDrawFile[nPos].Color.Color_dw = 0xff000000;
				m_cDrawFile[nPos].bRenderStyle = IMAGE_RENDER_STYLE_ALPHA;
				g_pRepresent->DrawPrimitives(1, m_cDrawFile, RU_T_IMAGE, bInMenu);
...
BOOL	KNpcRes::SetHelm(int nHelmType)
{
	...
	for (int i = MAX_BODY_PART_SECT * 0; i < MAX_BODY_PART_SECT * 0 + MAX_BODY_PART_SECT; i++)
	{
		if ( m_pcResNode->CheckPartExist(i) )
		{
...
BOOL	KNpcRes::SetArmor(int nArmorType)
{
	...
	char	szBuffer[80];
	for (int i = MAX_BODY_PART_SECT * 1; i < MAX_BODY_PART_SECT * 1 + MAX_BODY_PART_SECT; i++)
	{
		if ( m_pcResNode->CheckPartExist(i) )
		{
...
BOOL	KNpcRes::SetWeapon(int nWeaponType)
{
	...
	for (i = MAX_BODY_PART_SECT * 2; i < MAX_BODY_PART_SECT * 2 + MAX_BODY_PART_SECT; i++)
	{
		if ( m_pcResNode->CheckPartExist(i) )
		{
			...
			m_cNpcImage[i].SetSprFile(szBuffer, m_pcResNode->GetTotalFrames(i, m_nAction, m_nWeaponType, 16), m_pcResNode->GetTotalDirs(i, m_nAction, m_nWeaponType, 16), m_pcResNode->GetInterval(i, m_nAction, m_nWeaponType, 0), m_pcResNode->GetColor(i, m_nAction, m_nWeaponType));
		}
		else
		{
			m_cNpcImage[i].Release();
...
BOOL	KNpcRes::SetHorse(int nHorseType)
{
	...
	char	szBuffer[80];
	for (int i = MAX_BODY_PART_SECT * 3; i < MAX_BODY_PART_SECT * 3 + MAX_BODY_PART_SECT; i++)
	{
		if ( m_pcResNode->CheckPartExist(i) )
		{
			m_pcResNode->GetFileName(i, m_nAction, m_nHorseType, "", szBuffer, sizeof(szBuffer));
...
BOOL	KNpcRes::SetMantle(int nMantleType)
{
	...
	for (int i = MAX_BODY_PART_SECT * 4; i < MAX_BODY_PART_SECT * 4 + MAX_BODY_PART_SECT; i++)
	{
		if ( m_pcResNode->CheckPartExist(i) )
		{
			m_pcResNode->GetFileName(i, m_nAction, nMantleType, "", szBuffer, sizeof(szBuffer));
			m_cNpcImage[i].SetSprFile(szBuffer, m_pcResNode->GetTotalFrames(i, m_nAction, nMantleType, 16), m_pcResNode->GetTotalDirs(i, m_nAction, nMantleType, 16), m_pcResNode->GetInterval(i, m_nAction, nMantleType, 0));
		}
		else
		{
			m_cNpcImage[i].Release();
...
void	KNpcRes::SetState(KList *pNpcStateList, KNpcResList *pNpcResList)
{
	...
	int		i, nFind, nFindFlag[MAX_STATE_ID];
	KStateNode	*pNode;
