...
void KNpc::SetCurrentCamp(int nCamp)
{
	...
	BroadCast(&NetCommand, sizeof(NetCommand), nMaxCount);
#endif
}
...
//	HurtAutoMove();	
	...
	&& !m_bRideHorse)
	{
		//m_DataRes.SetArmor(121);
		//m_DataRes.SetHelm(16);
		//m_DataRes.SetMantle(37);
		//m_DataRes.SetHorse(17);
		//m_DataRes.SetWeapon(201);
		if(m_ArmorType >= 42 && m_ArmorType <= 46
		&& (m_ClientDoing <= cdo_run || m_ClientDoing == cdo_sit))
		{
			m_DataRes.SetArmor(m_ArmorType);
		}
		else if(m_ArmorType >= 42 && m_ArmorType <= 46)
		{
			m_DataRes.SetArmor(18);
		}
		else
		{
			m_DataRes.SetArmor(m_ArmorType);
		...
		//g_DebugLog("amryp[%d][%d]",m_ArmorType, m_HelmType);
	}
	else
	{
		m_DataRes.SetArmor(m_ArmorType);
	...
	{
		if (IR_CGetCurrentTime() - m_nCurChatTime > NPC_SHOW_CHAT_TIME)
		{
			m_nChatContentLen = 0;
			m_nChatNumLine = 0;
			m_nChatFontWidth = 0;
...
//				this->SendCommand(do_stand);
		...
		if (m_ActiveAuraID)
		{
			...
			//		if (pOrdinSkill->GetSkillStyle() == SKILL_SS_Missles)
				...
				int nMaxCount = MAX_BROADCAST_COUNT;
				BroadCast(&SkillCmd, sizeof(SkillCmd), nMaxCount);
#endif				
...
void KNpc::DoSkill(int nX, int nY)
{
	...
	if(pSkill)
	{
		...
		if (!IsPlayer() ||
			(Cost(pSkill->GetSkillCostType(), pSkill->GetSkillCost(this))
			&& m_SkillList.CanCast(m_ActiveSkillID, SubWorld[m_SubWorldIndex].m_dwCurrentTime)
			&& pSkill->CanCastSkill(m_Index, nX, nY))
			)
#endif
		{
...
#ifndef _SERVER
	if (m_RegionIndex < 0 || m_RegionIndex >= MAX_REGION)
	{
...
void KNpc::SendCommand(NPCCMD cmd,int x,int y, int z)
{
	...
	m_Command.Param_Z = z;
#ifndef _SERVER
	if(m_Command.CmdKind == do_skill && m_Index != Player[CLIENT_PLAYER_INDEX].m_nIndex
...
int KNpc::RandomSeries()
{
	if (m_SubWorldIndex >= 0)
	{
		...
		{
			nTotal += SubWorld[m_SubWorldIndex].m_nSeriesRate[i];
		}
		if(nTotal > 0)
		{
			int nRand = g_Random(nTotal);
			nTotal = 0;
			for(i=0;i<series_num;++i)
			{
				if(SubWorld[m_SubWorldIndex].m_nSeriesRate[i] <= 0)
					continue;
				nTotal += SubWorld[m_SubWorldIndex].m_nSeriesRate[i];
				if(nRand < nTotal)
					return i;
...
BOOL KNpc::SendSyncData(int nClient)
{
	...
	NpcSync.Life = m_CurrentLife;
	NpcSync.LifeMax = m_CurrentLifeMax;
	if (this->IsPlayer())
...
//	NpcSync.NpcEnchant			= (WORD)m_NpcEnchant;
	...
	if (IsPlayer())
	{
		...
		PlayerSync.ArmorType		= (short)m_ArmorType;
		PlayerSync.HelmType			= (short)m_HelmType;
		PlayerSync.WeaponType		= (short)m_WeaponType;
		PlayerSync.MantleType		= (short)m_MantleType;
		PlayerSync.AttackSpeed		= (BYTE)m_AttackSpeed;
		PlayerSync.CastSpeed		= (BYTE)m_CastSpeed;
		if(m_bRideHorse)
			PlayerSync.HorseType		= (BYTE)m_HorseType;
		else
			PlayerSync.HorseType		= (BYTE)(-1);
		...
		Player[m_nPlayerIdx].m_cTong.GetTongName(PlayerSync.szTong);
		Player[m_nPlayerIdx].m_cTong.GetTitle(PlayerSync.szTitle);
...
void KNpc::BroadCast(void* pBuffer, DWORD dwSize, int &nMaxCount)
{
	...
	CURREGION.BroadCast(pBuffer, dwSize, nMaxCount, m_MapX, m_MapY);
	int j;
	for (j = 0; j < 8; j++)
	{
		...
		SubWorld[m_SubWorldIndex].m_Region[nConRegion].BroadCast(pBuffer, dwSize, nMaxCount, m_MapX - POff[j].x, m_MapY - POff[j].y);
	}
	for (j = 1; j <= 3; j++)
	{
		int nConRegion = CURREGION.m_nConnectRegion[j];
		if (nConRegion == -1)
			continue;
		nConRegion = SubWorld[m_SubWorldIndex].m_Region[nConRegion].m_nConnectRegion[2];
		if (nConRegion == -1)
			continue;
		SubWorld[m_SubWorldIndex].m_Region[nConRegion].BroadCast(pBuffer, dwSize, nMaxCount, m_MapX - POffMr1[j-1].x, m_MapY - POffMr1[j-1].y);
	}
	for (j = 5; j <= 7; j++)
	{
		int nConRegion = CURREGION.m_nConnectRegion[j];
		if (nConRegion == -1)
			continue;
		nConRegion = SubWorld[m_SubWorldIndex].m_Region[nConRegion].m_nConnectRegion[6];
		if (nConRegion == -1)
			continue;
		SubWorld[m_SubWorldIndex].m_Region[nConRegion].BroadCast(pBuffer, dwSize, nMaxCount, m_MapX - POffMr2[j-5].x, m_MapY - POffMr2[j-5].y);
	}
}
// 平时数据的同步
void KNpc::NormalSync()
{
	...
	BroadCast(&NpcSync, sizeof(NPC_NORMAL_SYNC), nMaxCount);
	...
	if (IsPlayer())
	{
		...
		PlayerSync.ID			= m_dwID;
		PlayerSync.Mana			= m_CurrentMana;
		PlayerSync.ManaMax		= m_CurrentManaMax;
		...
		nMaxCount = MAX_PLAYER;//MAX_BROADCAST_COUNT;
		BroadCast(&PlayerSync, sizeof(PLAYER_NORMAL_SYNC), nMaxCount);
...
void KNpc::BroadCastRevive(int nType)
{
	...
	int nMaxCount = MAX_BROADCAST_COUNT;
	BroadCast((BYTE*)&NpcReviveSync, sizeof(NPC_REVIVE_SYNC), nMaxCount);
}
...
/*		case camp_animal:
		...
		{
			nHeightOffset += nFontSize + 1;
			g_pRepresent->OutputText(nFontSize, m_szTTitle, KRF_ZERO_END, nMpsX - nFontSize * g_StrLen(m_szTTitle) / 4, nMpsY, dwColor, 0, nHeightOffset, dwBorderColor);
		}
	}
	...
	{
		NpcSet.m_sSeriesImg[m_Series].oPosition.nX = nMpsX + nFontSize * g_StrLen(Name)/4 + 5;
		if((nMpsX + SHOW_LIFE_WIDTH/2 + 5) > NpcSet.m_sSeriesImg[m_Series].oPosition.nX)
			NpcSet.m_sSeriesImg[m_Series].oPosition.nX = nMpsX + SHOW_LIFE_WIDTH/2 + 5;
		NpcSet.m_sSeriesImg[m_Series].oPosition.nY = nMpsY;
		NpcSet.m_sSeriesImg[m_Series].oPosition.nZ = nHeightOff;
		g_pRepresent->DrawPrimitives(1, &NpcSet.m_sSeriesImg[m_Series], RU_T_IMAGE, false);
	}
...
int	KNpc::PaintChat(int nHeightOffset)
{
	...
	if(!m_bChatUFont)
		...
		g_pRepresent->DrawUFontRich("couri", nFontSize, &sParam, m_szChatBuffer, m_nChatContentLen, nWidth);
	return sParam.nZ;
}
...
int	KNpc::SetChatInfo(const char* Name, const char* pMsgBuff, unsigned short nMsgLength)
{
	...
	if (nMsgLength)
	{
		...
		if (nMsgLength)
		{
			...
			{
				memcpy(szChatBuffer + nOffset, pMsgBuff, 128);
				nOffset += 128;
			}
			...
			m_nChatContentLen = nOffset;
			memcpy(m_szChatBuffer, szChatBuffer, m_nChatContentLen);
			if(!m_bChatUFont)
			...
			m_nChatNumLine = g_pRepresent->GetEncodedTextLineCount("couri", m_szChatBuffer, m_nChatContentLen, SHOW_CHAT_WIDTH*8, m_nChatFontWidth, nFontSize, 0, 0, true);
			//if (m_nChatNumLine >= 2)
			//	m_nChatNumLine = 2;
			m_nCurChatTime = IR_CGetCurrentTime();
			return true;
		}
	}
	return false;
...
int	KNpc::PaintLife(int nHeightOffset, bool bSelect)
{
	if (!bSelect &&
		(m_Kind != kind_player &&
		m_Kind != kind_partner && m_Kind != kind_normal)
		)
		return nHeightOffset;
...
void KNpc::Paint()
{
	...
	m_DataRes.Draw(m_Index, m_ResDir, m_Frames.nTotalFrame, m_Frames.nCurrentFrame, FALSE, Player[CLIENT_PLAYER_INDEX].m_nIndex != m_Index);
...
void	KNpc::OnRunAttack()
{
	if (m_SpecialSkillStep == 0)
	{
		...
		GetMpsPos(&x, &y);
        if (m_Doing == do_stand
		|| g_GetDistance(x, y, m_DesX, m_DesY) < 64 //khi den' sat' muc tieu
		|| m_nCurrentMeleeTime > pSkill->GetAttackRadius()/31) //khi du~ khoang~ cach'
		{
			m_SpecialSkillStep ++;
			m_nCurrentMeleeTime = 0;
			DoRunAttack();
		...
		else if (IsReachFrame(ATTACKACTION_EFFECT_PERCENT))
		{
			...
			if (nCurPhySkillId > 0)
			{
				...
				if (pOrdinSkill)
                {
					int x,y;
					GetMpsPos(&x, &y);
					x += ((32 * g_DirCos(m_Dir, 64)) >> 10);
					y += ((32 * g_DirSin(m_Dir, 64)) >> 10);
				    pOrdinSkill->Cast(m_Index, x, y);
                }
			}
			DoStand();
			m_ProcessAI = 1;
...
#ifdef _SERVER
int KNpc::GetDataFromScript(char* pDataName, int nLevel, int nSeries)
{
	if(!pDataName[0])
		return 0;
	int nNpcTemplateId = m_NpcSettingIdx;
	...
	if(!strcmp(pDataName, "Life"))
	{
		...
		int nValue = KNpcTemplate::GetNpcLevelDataFromScript(pLevelScript, nSeries, "Life", nLevel, nParam1, nParam2, nParam3);
		nValue = (double)nParam * nValue / 100.0;
		if (nValue <= 0) nValue = 100;
		return nValue;
	}
	else if(!strcmp(pDataName, "Exp"))
	{
		...
		int nValue = KNpcTemplate::GetNpcLevelDataFromScript(pLevelScript, nSeries, "Exp", nLevel, nParam1, nParam2, nParam3);
		nValue = (double)nParam * nValue / 100.0;
		return nValue;
	}
	else if(!strcmp(pDataName, "LifeReplenish"))
	{
		char szValue1[MAX_PATH];
		g_NpcSetting.GetString(nNpcTempRow, "LifeReplenish", "", szValue1, MAX_PATH);
		return KNpcTemplate::GetNpcLevelDataFromScript(pLevelScript, nSeries, "LifeReplenish", nLevel, szValue1);
	}
	else if(!strcmp(pDataName, "AR"))
	{
		...
		int nValue = KNpcTemplate::GetNpcLevelDataFromScript(pLevelScript, nSeries, "AR", nLevel, nParam1, nParam2, nParam3);
		nValue = (double)nParam * nValue / 100.0;
		if (nValue <= 0) nValue = 100;
		return nValue;
	}
	else if(!strcmp(pDataName, "Defense"))
	{
		...
		int nValue = KNpcTemplate::GetNpcLevelDataFromScript(pLevelScript, nSeries, "Defense", nLevel, nParam1, nParam2, nParam3);
		nValue = (double)nParam * nValue / 100.0;
		return nValue;
	}
	else if(!strcmp(pDataName, "MinDamage"))
	{
		...
		int nValue = KNpcTemplate::GetNpcLevelDataFromScript(pLevelScript, nSeries, "MinDamage", nLevel, nParam1, nParam2, nParam3);
		nValue = (double)nParam * nValue / 100.0;
		return nValue;
	}
	else if(!strcmp(pDataName, "MaxDamage"))
	{
		...
		int nValue = KNpcTemplate::GetNpcLevelDataFromScript(pLevelScript, nSeries, "MaxDamage", nLevel, nParam1, nParam2, nParam3);
		nValue = (double)nParam * nValue / 100.0;
		return nValue;
	}
	else if(!strcmp(pDataName, "FireResist"))
	{
		char szValue1[MAX_PATH];
		g_NpcSetting.GetString(nNpcTempRow, "FireResist", "", szValue1, MAX_PATH);
		return KNpcTemplate::GetNpcLevelDataFromScript(pLevelScript, nSeries, "FireResist", nLevel, szValue1);
	}
	else if(!strcmp(pDataName, "ColdResist"))
	{
		char szValue1[MAX_PATH];
		g_NpcSetting.GetString(nNpcTempRow, "ColdResist", "", szValue1, MAX_PATH);
		return KNpcTemplate::GetNpcLevelDataFromScript(pLevelScript, nSeries, "ColdResist", nLevel, szValue1);
	}
	else if(!strcmp(pDataName, "LightResist"))
	{
		char szValue1[MAX_PATH];
		g_NpcSetting.GetString(nNpcTempRow, "LightResist", "", szValue1, MAX_PATH);
		return KNpcTemplate::GetNpcLevelDataFromScript(pLevelScript, nSeries, "LightResist", nLevel, szValue1);
	}
	else if(!strcmp(pDataName, "PoisonResist"))
	{
		char szValue1[MAX_PATH];
		g_NpcSetting.GetString(nNpcTempRow, "PoisonResist", "", szValue1, MAX_PATH);
		return KNpcTemplate::GetNpcLevelDataFromScript(pLevelScript, nSeries, "PoisonResist", nLevel, szValue1);
	}
	else if(!strcmp(pDataName, "PhysicsResist"))
	{
		char szValue1[MAX_PATH];
		g_NpcSetting.GetString(nNpcTempRow, "PhysicsResist", "", szValue1, MAX_PATH);
		return KNpcTemplate::GetNpcLevelDataFromScript(pLevelScript, nSeries, "PhysicsResist", nLevel, szValue1);
	}
	return 0;
}
#endif
...
#ifdef _SERVER
		g_pNpcTemplate[nNpcTemplateId][nLevel]->m_Series = m_Series;
...
void KNpc::LoseSingleItem(int nBelongPlayer)
{
	...
	int nIdx = ItemSet.Add(nGenre, nSeries, nLevel, nLuck, nDetail, nParticular, pnMagicLevel);
...
#else
	m_Series = RandomSeries();
	if(g_pNpcTemplate[m_NpcSettingIdx][m_Level]->m_Series != m_Series)
	{
		KLuaScript * pLevelScript = NULL;		
			pLevelScript = (KLuaScript*)g_GetScript(
			g_pNpcTemplate[m_NpcSettingIdx][0]->m_dwLevelSettingScript
			);
		
		if (pLevelScript == NULL)
			pLevelScript = g_pNpcLevelScript;
		g_pNpcTemplate[m_NpcSettingIdx][m_Level]->m_Series = m_Series;
		g_pNpcTemplate[m_NpcSettingIdx][m_Level]->InitNpcLevelData(&g_NpcKindFile, m_NpcSettingIdx, pLevelScript, m_Level);
		LoadDataFromTemplate(m_NpcSettingIdx, m_Level);
	}
	else
		RestoreNpcBaseInfo();
#endif
...
{
	int nMaxCount = MAX_BROADCAST_COUNT;
	BroadCast(pBuffer, dwSize, nMaxCount);
}
#endif
...
void	KNpc::BroadCastState()
{
	...
	int nMaxCount = MAX_BROADCAST_COUNT;
	BroadCast(&StateInfo, sizeof(NPC_SYNC_STATEINFO), nMaxCount);
}
#endif
...
void KNpc::OnRunByFPS(int nStep, int nSpeed)
{
	if(nSpeed <= 0 || nStep <= 0)
		return;
	if(nSpeed > SubWorld[m_SubWorldIndex].m_nCellWidth)
		nSpeed = SubWorld[m_SubWorldIndex].m_nCellWidth - 1;
	int x, y;
	if(m_nPosShiftStep == 0) //tim huong'
	{
		SubWorld[m_SubWorldIndex].Map2Mps(m_RegionIndex, m_MapX, m_MapY, 0, 0, &x, &y);
		x = (x << 10) + m_OffX;
		y = (y << 10) + m_OffY;
		int nRet = m_PathFinder.GetDir(x, y, m_Dir, m_DesX, m_DesY, nSpeed, &m_Dir);
		if(nRet == 1)
		{
			//buoc nay` co duong di
		}
		else if (nRet == 0)
		{
			DoStand();
	...
	int nTrueSpeed = nSpeed/nStep;
	int nAddPerStep = nSpeed % nStep;
	...
	x = g_DirCos(m_Dir, 64) * nTrueSpeed;
	y = g_DirSin(m_Dir, 64) * nTrueSpeed;
	int nOldRegion = m_RegionIndex;
	int nOldMapX = m_MapX;
	int nOldMapY = m_MapY;
	int nOldOffX = m_OffX;
...
void KNpc::OnWalkByFPS(int nStep, int nSpeed)
{
	if(nSpeed <= 0 || nStep <= 0)
		return;
	if(nSpeed > SubWorld[m_SubWorldIndex].m_nCellWidth)
...
void KNpc::OnJumpByFPS(int nStep, int nSpeed)
{
	if(nSpeed <= 0 || nStep <= 0)
		return;
	if(nSpeed > SubWorld[m_SubWorldIndex].m_nCellWidth)
	...
	ServeJump(nTrueSpeed);
}

void KNpc::NewFPSMove(int nStep)
{
	if(m_Doing == do_run)
	{
		OnRunByFPS(nStep, m_CurrentRunSpeed);
	}
	else if(m_Doing == do_runattack && m_SpecialSkillStep == 0)
	{
		OnRunByFPS(nStep, 32);
		if (m_Doing == do_stand) 
		{
	...
	{
		OnWalkByFPS(nStep, m_CurrentWalkSpeed);
	}
	else if(m_Doing == do_jump || m_Doing == do_jumpattack)
	{
		OnJumpByFPS(nStep, m_CurrentJumpSpeed);
	}
	else
		return;
	m_DataRes.SetAction(m_ClientDoing);
	int		nMpsX, nMpsY;
	SubWorld[0].Map2Mps(m_RegionIndex, m_MapX, m_MapY, m_OffX, m_OffY, &nMpsX, &nMpsY);
	m_DataRes.SetPos(m_Index, nMpsX, nMpsY, m_Height, Player[CLIENT_PLAYER_INDEX].m_nIndex == m_Index);
}
