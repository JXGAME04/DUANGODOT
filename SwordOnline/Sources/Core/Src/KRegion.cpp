...
//	g_SetFilePath(szFilePath);
	...
	if (cData.Open(szFile))
	{
		...
		ObjSet.ServerLoadRegionObj(nSubWorld, &cData, sElemFile[REGION_OBJ_FILE_INDEX].uLength);
...
std::map<UINT, int> Trapmap;
	...
	for (i = 0; i < sTrapFileHead.uNumTrap; i++)
	{
		...
		for (j = 0; j < sTrapCell.cNumCell; j++)
		{
			m_dwTrap[sTrapCell.cX + j][sTrapCell.cY] = sTrapCell.uTrapId;
#ifdef defDEBUG_TRAP
			Trapmap[sTrapCell.uTrapId] = 1;
#endif
		}
	}
#ifdef defDEBUG_TRAP
	for (std::map<UINT,int>::iterator it = Trapmap.begin();it != Trapmap.end(); ++it)
	{
		UINT uKey   = it->first;
		KLuaScript * pScript = (KLuaScript* )g_GetScript(uKey);
		if(pScript)
			g_DebugLog("[Trap][%u][%s]", uKey, pScript->m_szScriptName);
		else
			g_DebugLog("[Trap][%u][not found in \\script]", uKey);
	}
#endif
	return TRUE;
}
...
#define	MAX_SYNC_RANGE	40
	...
	while(pNode && nMaxCount > 0)
	{
		...
		int nNpcIndex = Player[pNode->m_nIndex].m_nIndex;
		int nDX = Npc[nNpcIndex].m_MapX - nOX;
		int nDY = Npc[nNpcIndex].m_MapY - nOY;
		//if((nDX * nDX + nDY * nDY) > 0)
		//g_DebugLog("BroadCast r[%X] n[%d][%d]",m_RegionID, nNpcIndex,(nDX * nDX + nDY * nDY));
