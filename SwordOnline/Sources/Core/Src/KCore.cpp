...
#ifndef __linux
	g_RandomSeed((unsigned)time( NULL ) ^ GetTickCount());
#else
	g_RandomSeed(clock());
#endif
...
void g_ReleaseCore()
{
	...
	for (i = 0; i < (unsigned long)nNpcTemplateNum; i++)
	{
	...
	if (g_pTransServer) //guve
	{
		g_pTransServer->Release();
		g_pTransServer = NULL;
	}
	if (g_pTongServer)
	{
		g_pTongServer->Release();
		g_pTongServer = NULL;
	}
	if (g_pChatServer)
	{
		g_pChatServer->Release();
		g_pChatServer = NULL;
	}
