...
//	g_SetFilePath("\\");
	...
	for (int k = 0; k < nHeight; k++)
	{
		...
		case item_townportal:
			...
			break;
		case item_task:
			ItemGen.Gen_Quest(ItemGenerator.nDetailType, &m_Item[k], 0);
			break;
		case item_gengold:
			ItemGen.Gen_GoldEquip(0, ItemGenerator.nDetailType, &m_Item[k], NULL);
			break;
		case item_mascript:
			ItemGen.Gen_MAScript(ItemGenerator.nDetailType,
				ItemGenerator.nParticularType,
				ItemGenerator.nLevel,
				ItemGenerator.nSeriesReq,
				0, NULL, &m_Item[k]);
...
BOOL KBuySell::Buy(int nPlayerIdx, int nBuy, int nBuyIdx, int nPlace, int nX, int nY)
{
	...
	if(Player[nPlayerIdx].m_ItemList.CheckCanPlaceInEquipment(
			m_Item[nIdx].GetWidth(), m_Item[nIdx].GetHeight(), &x, &y))
	{
		...
		Player[nPlayerIdx].m_ItemList.Add(nItemIdx, pos_equiproom, x, y);
	}
	else
		return FALSE;
	return TRUE;
