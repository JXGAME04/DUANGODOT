...
public:
	...
	KStateSpr		m_cStateSpr[MAX_STATE_ID];						// 状态特效，0 1 为头顶 2 3 为脚底 4 5 为身上
...
public:
	...
	void			Draw(int nNpcIdx, int nDir, int nAllFrame, int nCurFrame, BOOL bInMenu = FALSE, BOOL bOther = FALSE);		// 绘制
	void			DrawBorder();
	...
	BOOL			SetMantle(int nMantleType);
