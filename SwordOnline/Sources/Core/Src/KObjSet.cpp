...
int		KObjSet::Add(int nDataID, KMapPos MapPos, KObjItemInfo sItemInfo)
{
	...
	cObjAdd.m_btColorID = 0;
	if(sItemInfo.m_nItemID > 0 && Item[sItemInfo.m_nItemID].GetGenre() == item_equip)
	{
		int iexType = Item[sItemInfo.m_nItemID].GetExType();
		if(iexType == extype_gold)	//ho祅g kim
		{
			cObjAdd.m_btColorID = 2;
		}
		else if(iexType == extype_platina)	//b筩h kim
		{
			cObjAdd.m_btColorID = 1;
		}
		else if(iexType == extype_purple)	//t輒
		{
			cObjAdd.m_btColorID = 4;
		}
		else if (Item[sItemInfo.m_nItemID].m_aryMagicAttrib[0].nAttribType)	// xanh
		{
			cObjAdd.m_btColorID = 3;
		}
	}
	if(sItemInfo.m_nItemID > 0)
	{
		int nGenre = Item[sItemInfo.m_nItemID].GetGenre();
		cObjAdd.m_btColorID = cObjAdd.m_btColorID | (nGenre << 4);
	}
	Object[nAddNo].m_nColorID = cObjAdd.m_btColorID;
	cObjAdd.m_btFlag = 0;
...
int		KObjSet::ClientAdd(int nID, int nDataID, int nState, int nDir, int nCurFrame, int nXpos, int nYpos, KObjItemInfo sInfo)
{
	...
	Object[nAddIndex].m_nColorID = sInfo.m_nColorID & 0xf;
	Object[nAddIndex].m_nItemGenre = (sInfo.m_nColorID >> 4) & 0xf;
