...
KNpcSet	NpcSet;
#ifdef _SERVER
#define NPC_REPLACE_NAME_FILE	"\\Settings\\npc\\replacename_npc.txt"
static std::map<std::string, std::string> gNpcNameMap;
#endif
...
//	g_SetFilePath("\\");
	...
	KTabFile NpcRepName;
	if (NpcRepName.Load(NPC_REPLACE_NAME_FILE))
	{
		char sName[32];
		char dName[32];
		for(int i=0;i<NpcRepName.GetHeight()-1;++i)
		{
			NpcRepName.GetString(i + 2, 1, "", sName, sizeof(sName));
			NpcRepName.GetString(i + 2, 2, "", dName, sizeof(dName));
			gNpcNameMap[sName] = dName;
		}
	}
#endif
	...
	ZeroMemory(m_sSeriesImg, sizeof(KRUImage)*series_num);
	KIniFile	cNSIni;
	if (cNSIni.Load(NPC_IMG_SETTING))
	{
		char szKey[32];
		for(i=0;i<series_num;++i)
		{
			m_sSeriesImg[i].nType = ISI_T_SPR;
			m_sSeriesImg[i].Color.Color_b.a = 255;
			m_sSeriesImg[i].bRenderStyle = IMAGE_RENDER_STYLE_ALPHA;
			m_sSeriesImg[i].nISPosition = IMAGE_IS_POSITION_INIT;
			sprintf(szKey, "%d", i);
			cNSIni.GetString("Series", szKey, "", m_sSeriesImg[i].szImage, sizeof(m_sSeriesImg[i].szImage));
		}
	}
#endif
...
#ifdef _SERVER
		std::string strName = pKSNpcInfo->szName;
		if(gNpcNameMap.find(strName) != gNpcNameMap.end())
		g_StrCpyLen(Npc[nRet].Name, gNpcNameMap[strName].c_str(), sizeof(Npc[nRet].Name));
#endif
...
#ifndef _SERVER
	if (nRegion >= 0 && nRegion < MAX_REGION)
