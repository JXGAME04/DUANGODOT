...
//	void				SetBaseRunSpeed(int nSpeed);							// 设定跑步速度
	...
	void				BroadCast(void* pBuffer, DWORD dwSize, int &nMaxCount);
	...
	int					GetDataFromScript(char* pDataName, int nLevel, int nSeries);
#endif
	...
	void			OnRunByFPS(int nStep, int nSpeed);
	void			OnWalkByFPS(int nStep, int nSpeed);
	void			OnJumpByFPS(int nStep, int nSpeed);
	void			NewFPSMove(int nStep);
#endif
