...
//----关于不隶属于场景地图的对象与场景关系的操作----
	...
	void EnableForceLookAt() {m_bForceLookAt = true;};
	...
	void GetClickPos(int nScrW, int nScrH, int nX, int nY);
private:
...
private:
	...
	enum SP_WORKING_PARAM { 
		...
		SPWP_NUM_REGIONS_IN_PROCESS_MORE = 6,
	};
...
private:
	...
	//##Documentation
	//## 进入处理范围的以加载区域的指针。
	KScenePlaceRegionC* m_pInProcessAreaRegions[SPWP_NUM_REGIONS_IN_PROCESS_AREA+SPWP_NUM_REGIONS_IN_PROCESS_MORE];

	KRUImage	m_RegionGroundImages[SPWP_NUM_REGIONS_IN_PROCESS_AREA+SPWP_NUM_REGIONS_IN_PROCESS_MORE];
