	...
	if (m_bInited == false && g_pRepresent)
	{
		...
		for (nIndex = i = 0; nIndex < nCount && i < 0x03335688; i++)
		{
			...
			pCell->uImageId = g_pRepresent->CreateImage(
				pCell->szImageName, MAP_CELL_MAP_WIDTH, MAP_CELL_MAP_HEIGHT, ISI_T_BITMAP16, true);
			if (pCell->uImageId)
			{
				pCell++;
				nIndex++;
...
void KScenePlaceMapC::FlagOnTarget(int x, int y)
{
	...
	//if(x < m_MapCoverArea.left || y < m_MapCoverArea.top ||
	//x >= m_MapCoverArea.right || y >= m_MapCoverArea.bottom)
	//	return;
