...
public:
	...
	BOOL				IsPhysicalSkillId() {if(m_nId == 1 || m_nId == 2 || m_nId == 53 || m_nId == 2135) return TRUE; return FALSE;};
	KMagicAttrib*		GetImmediateAttribs()  { return m_ImmediateAttribs;};
	KMagicAttrib*		GetStateAttribs()  { return m_StateAttribs;};
	int					GetImmediateNum()  { return m_nImmediateAttribsNum;};
	int					GetStateNum()  { return m_nStateAttribsNum;};
private:
	...
	int					CastLine					(TOrdinSkillParam * pSkillParam, int nDir, int nRefPX, int nRefPY, int nDesX, int nDesY) const ;
	int					CastSpread					(TOrdinSkillParam * pSkillParam, int nDir, int nRefPX, int nRefPY, BOOL bNeedNarrow = FALSE) const ;
	int					CastCircle					(TOrdinSkillParam * pSkillParam, int nDir, int nRefPX, int nRefPY, BOOL bNeedNarrow = FALSE) const ;
