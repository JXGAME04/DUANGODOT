...
public:
	...
	BOOL			ApplyUseItem(int nItemID, ItemPos SrcPos, int nTargetPos = 0);	// 向服务器申请使用某个物品（鼠标右键点击该物品）
...
//	void			UpdateEnterGamePos(DWORD dwSubWorldID, int nX, int nY, int nFightMode);
	...
	void			AutoArrangeItem(int nType = 0);
#endif
