...
BOOL CHostConnect::FindPlayerByID(DWORD dwNameID, std::_tstring* pRole, std::_tstring* pAcc, UINT* pParam)
{
	DUMMY_AUTOLOCKREAD(m_lockPlayer);

	for(ROLEMAP::iterator it = m_mapRole.begin();it != m_mapRole.end();++it)
	{
		const ROLEINFO& infoRole = (*it).second;
		if(infoRole.nameID == dwNameID)
		{
			if (pAcc != NULL)
				*pAcc = infoRole.acc;
			if (pParam != NULL)
