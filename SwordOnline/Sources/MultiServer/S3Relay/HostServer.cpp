...
BOOL CHostServer::FindPlayerByID(DWORD dwNameID, std::_tstring* pRole, CNetConnectDup* pConnDup, std::_tstring* pAcc, UINT* pParam)
{
	AUTOLOCKREAD(m_lockIpMap);
	...
	for (IP2CONNECTMAP::iterator it = m_mapIp2Connect.begin(); it != m_mapIp2Connect.end(); it++)
	{
		...
		if (pConnMe)
		{
			if (pConnMe->FindPlayerByID(dwNameID, pRole, pAcc, pParam))
			{
				if (pConnDup != NULL)
					*pConnDup = *pConnMe;
