...
static UINT gs_timerUpdateTong = 0;
	...
	*/
	...
	if (g_cTongDB.Open())
	{
		...
		if (g_cTongSet.InitFromDB())
		{
			...
			HWND mainwnd = gGetMainWnd();//guve
			if(mainwnd)
			gs_timerUpdateTong = ::SetTimer(mainwnd, timer_updatetong, 12*60000, NULL);//phut' cap nhat bang hoi (12 phut)
		}
		else
...
BOOL gUninitialize()
{
	...
	try
	{
		...
		g_cTongDB.Close();
		if(gs_timerUpdateTong) //guve
		{
			HWND mainwnd = gGetMainWnd();
			if (mainwnd)
			{
				::KillTimer(mainwnd, gs_timerUpdateTong);
				gs_timerUpdateTong = 0;
			}
		}
	}
	catch (...)
					   ...
					   const void* pLosePckg, size_t sizeLosePckg, void* pLoseway, size_t sizeLoseway)
{
	...
	pLoseData->routeDateLength = (WORD)sizeLosePckg;
...
UINT g_String2Id(LPCSTR pString)
{
	UINT Id = 0;
