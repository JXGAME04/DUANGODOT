...
static UINT gs_timerUpdatePerTong = 0; //guve
static int gs_TUpdateIndex = 0;
/////////////////////////////////////////////////
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	switch (message) 
	{
		...
		case WM_DESTROY:
			...
			//guve
			if(gs_timerUpdatePerTong)
			{
				::KillTimer(hWnd, gs_timerUpdatePerTong);
				gs_timerUpdatePerTong = 0;
			}
		...
		case WM_TIMER:
			...
			case timer_updatetong: //guve
			{
				gs_TUpdateIndex = 0;
				gs_timerUpdatePerTong = ::SetTimer(g_mainwnd, timer_updatepertong, 250, NULL);//moi~ bang cap nhat cach nhau 1/4s
			}
			break;
			case timer_updatepertong:
			{
				gs_TUpdateIndex = g_cTongSet.Update(gs_TUpdateIndex);
				if(gs_TUpdateIndex < 0)
				{
					gs_TUpdateIndex = 0;
					if (gs_timerUpdatePerTong)
					{
						::KillTimer(g_mainwnd, gs_timerUpdatePerTong);
						gs_timerUpdatePerTong = 0;
					}
				}
			}
