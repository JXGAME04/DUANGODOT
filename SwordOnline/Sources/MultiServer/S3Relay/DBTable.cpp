...
bool ZDBTable::open() {
	...
	if(!db_create(&primary_db, dbenv, 0)) {
		if(!primary_db->open(primary_db, NULL, table_name, NULL, DB_BTREE, DB_CREATE|DB_AUTO_COMMIT, 0664)) {	//打开主数据库
			...
			//else while(--index) (index_db[index])->close(index_db[index], 0); //sai
			else
			{
				while (index > 0)
				{
...
bool ZDBTable::remove(const char *key_ptr, int key_size) {
	DBT data, key;
	memset(&key, 0, sizeof(DBT));
	memset(&data, 0, sizeof(DBT));
	key.data = (void *)key_ptr;
...
//index = -1 se~ lay primary_db nam vi tri ngay truoc index_db
char* ZDBTable::GetRecordEx(int &size,
...
{//取得按游标某一个数据(新版函数)
	...
	char* aBuffer = new char[data.size];
