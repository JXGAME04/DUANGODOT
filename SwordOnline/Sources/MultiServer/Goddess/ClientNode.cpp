...
void CClientNode::QueryGSPlayer(const char* szRoleName)
{
	tagDBSelPlayer s;
	s.cProtocol = s2c_queryplayer;
	strcpy(s.szRoleName, szRoleName);
	for ( CClientNode::stdMap::iterator it = CClientNode::m_theClientMap.begin();
	it != CClientNode::m_theClientMap.end();
	it ++ )
	{
		CClientNode *pNode = ( CClientNode * )( ( *it ).second );
		...
		pNode->m_pServer->SendData( pNode->m_nIndentity, ( const void * )&s, sizeof( s ) );
	}
}
