...
int	g_nTongPCSize[defTONG_PROTOCOL_CLIENT_NUM] = 
{
	...
	sizeof(STONG_ACCEPTMEMBER_FAIL_SYNC),	// enumS2C_TONG_ACCEPTMEMBER_FAIL
	-1,										// enumS2C_TONG_HEAD_INFO
	-1,										// enumS2C_TONG_MEMBER_INFO
	-1,										// enumS2C_TONG_PAGE_INFO
	-1,										// enumS2C_TONG_RECORD_AFFAIRHISTORY
	sizeof(STONG_RECORD_ANNOUNCE_SYNC),		// enumS2C_TONG_RECORD_ANNOUNCE
	sizeof(STONG_RECORD_WEEKTASK_SYNC),		// enumS2C_TONG_RECORD_WEEKTASK
	...
	sizeof(STONG_DISTRIB_RIGHT_COMMAND),	// enumS2C_TONG_DISTRIB_RIGHT_RET
	sizeof(STONG_BE_RIGHT_SYNC),			// enumS2C_TONG_BE_RIGHT
	sizeof(STONG_CHANGERECRUIT_SYNC),		// enumS2C_TONG_CHANGERECRUIT
	sizeof(STONG_DELETE_SYNC),				// enumS2C_TONG_DELETE
	sizeof(STONG_CONTRIBMONEY_SYNC),		// enumS2C_TONG_CONTRIBMONEY
	sizeof(STONG_WITHDRAWMONEY_SYNC),		// enumS2C_TONG_WITHDRAWMONEY
	sizeof(STONG_CONTRIBMONEY_SYNC),		// enumS2C_TONG_STOREOFFER
	sizeof(STONG_CONTRIBMONEY_SYNC),		// enumS2C_BE_DISPENSEOFFER
	sizeof(STONG_ASSIGNFUND_SYNC),			// enumS2C_ASSIGN_MONEY_RET
	sizeof(STONG_ASSIGNFUND_SYNC),			// enumS2C_ASSIGN_OFFER_RET
	sizeof(STONG_WITHDRAWMONEY_SYNC),		// enumS2C_TONG_TRANSMONEY
	sizeof(STONG_STOREBUILDFUND_SYNC),		// enumS2C_TONG_STOREBUILDFUND
	sizeof(STONG_WEEKLYRESET_SYNC),			// enumS2C_TONG_WEEKLYRESET
	sizeof(STONG_RECEIVEPRICE_SYNC),		// enumS2C_TONG_RECEIVEPRICE_RET
	sizeof(STONG_PLAYERPRICE_SYNC),			// enumS2C_TONG_PLAYERPRICE_RET
	sizeof(STONG_CHANGETITLE_SYNC),			// enumS2C_TONG_CHANGETITLE_UI
	sizeof(STONG_CHANGETITLE_SYNC),			// enumS2C_TONG_BE_CHANGETITLE
	sizeof(STONG_ADDWEEKGOAL_SYNC),			// enumS2C_TONG_ADDWEEKGOAL_RET
	sizeof(STONG_CHANGETITLE_SYNC),			// enumS2C_TONG_CHANGETITLEALL
	sizeof(STONG_CHANGECAMP_SYNC),			// enumS2C_TONG_CHANGECAMP
};
...
#ifndef _STANDALONE
void __stdcall ServerTransNotify(
#else
void ServerTransNotify(								 
#endif
			LPVOID lpParam,
			const unsigned long &ulnID,
			const unsigned long &ulnEventType )
{
	...
	{
	case enumClientConnectCreate:
		g_SOServer.SetServerTrans(ulnID);
		printf("--->Transfer connected\n");
		break;
	case enumClientConnectClose:
		break;
	}
...
#ifndef _STANDALONE
void __stdcall ServerChatNotify(
#else
void ServerChatNotify(								 
#endif
			LPVOID lpParam,
			const unsigned long &ulnID,
			const unsigned long &ulnEventType )
{
	...
	{
	case enumClientConnectCreate:
		g_SOServer.SetServerChat(ulnID);
...
#ifndef _STANDALONE
void __stdcall ServerTongNotify(
#else
void ServerTongNotify(								 
#endif
			LPVOID lpParam,
			const unsigned long &ulnID,
			const unsigned long &ulnEventType )
{
	...
	{
	case enumClientConnectCreate:
		g_SOServer.SetServerTong(ulnID);
		printf("--->Tong connected\n");
		break;
	case enumClientConnectClose:
		break;
	}
...
BOOL KSwordOnLineSever::Init()
{
	...
	if (!m_pGameStatus)
	{
		...
		memset(m_pGameStatus, 0, sizeof(GameStatus)*m_nMaxPlayer);//khoi tao
	}
...
//	strcpy(m_szGameSvrIP, OnlineGameLib::Win32::net_ntoa(dwIp));
	...
	if ( pFactroyFun && SUCCEEDED( pFactroyFun( IID_IServerFactory, reinterpret_cast< void ** >( &pServerFactory ) ) ) )
	{
		...
		pServerFactory->CreateServerInterface( IID_IIOCPServer, reinterpret_cast< void ** >( &m_pTransServer ) );
		pServerFactory->CreateServerInterface( IID_IIOCPServer, reinterpret_cast< void ** >( &m_pChatServer ) );
		pServerFactory->CreateServerInterface( IID_IIOCPServer, reinterpret_cast< void ** >( &m_pTongServer ) );
	...
	if (!m_pServer || !m_pTransServer || !m_pChatServer || !m_pTongServer) //guve
	{
	...
	m_pTransServer->Startup();

	m_pTransServer->RegisterMsgFilter( reinterpret_cast< void * >( m_pTransServer ), ServerTransNotify );

	if ( FAILED( m_pTransServer->OpenService( INADDR_ANY, 5006 ) ) )
	{
		return FALSE;
	}
	
	m_pChatServer->Startup();

	m_pChatServer->RegisterMsgFilter( reinterpret_cast< void * >( m_pChatServer ), ServerChatNotify );

	if ( FAILED( m_pChatServer->OpenService( INADDR_ANY, 5007 ) ) )
	{
		return FALSE;
	}

	m_pTongServer->Startup();

	m_pTongServer->RegisterMsgFilter( reinterpret_cast< void * >( m_pTongServer ), ServerTongNotify );

	if ( FAILED( m_pTongServer->OpenService( INADDR_ANY, 5008 ) ) )
	{
		return FALSE;
	}
...
void KSwordOnLineSever::Release()
{
	...
	if (m_pTransServer) //guve
	{
		m_pTransServer->CloseService();
		m_pTransServer->ShutdownClient(m_TransSVID);
		m_pTransServer->RegisterMsgFilter( reinterpret_cast< void * >( m_pTransServer ), NULL );
		if (m_pTransServer)
		{
			m_pTransServer->Cleanup();
			m_pTransServer->Release();
			m_pTransServer = NULL;
		}
	}
	if (m_pChatServer)
	{
		m_pChatServer->CloseService();
		m_pChatServer->ShutdownClient(m_ChatSVID);
		m_pChatServer->RegisterMsgFilter( reinterpret_cast< void * >( m_pChatServer ), NULL );
		if (m_pChatServer)
		{
			m_pChatServer->Cleanup();
			m_pChatServer->Release();
			m_pChatServer = NULL;
		}
	}
	if (m_pTongServer)
	{
		m_pTongServer->CloseService();
		m_pTongServer->ShutdownClient(m_TongSVID);
		m_pTongServer->RegisterMsgFilter( reinterpret_cast< void * >( m_pTongServer ), NULL );
		if (m_pTongServer)
		{
			m_pTongServer->Cleanup();
			m_pTongServer->Release();
			m_pTongServer = NULL;
		}
	}
...
void KSwordOnLineSever::TongMessageProcess(const char *pChar, size_t nSize)
{
	...
	if (pHeader->ProtocolFamily == pf_extend)
	{
		if (pHeader->ProtocolID == extend_s2c_passtosomeone)
		{
			...
			if (CheckPlayerID(pEps->lnID ,pEps->nameid))
			{
				...
				pExHdr->wLength = (WORD)(pckgsize - 1);
	...
	else if (pHeader->ProtocolFamily == pf_tong)
	{
		...
		case enumS2C_TONG_CREATE_SUCCESS:
			{
				...
				if(pSync->m_uNetID >= 0 && pSync->m_uNetID < m_nMaxPlayer)
				{
					STONG_SERVER_TO_CORE_CREATE_SUCCESS sSuccess;
					sSuccess.m_nCamp = pSync->m_btCamp;
					sSuccess.m_nPlayerIdx = m_pGameStatus[pSync->m_uNetID].nPlayerIndex;
		...
		case enumS2C_TONG_CREATE_FAIL:
			{
				...
				if(pFail->m_uNetID >= 0 && pFail->m_uNetID < m_nMaxPlayer)
				{
					m_pCoreServerShell->OperationRequest(SSOI_TONG_CREATE_FAIL, pFail->m_dwPlayerNameID, m_pGameStatus[pFail->m_uNetID].nPlayerIndex);
				}
			}
			break;
		case enumS2C_TONG_ADD_MEMBER_SUCCESS:
			{
				...
				if(pSync->m_uNetID >= 0 && pSync->m_uNetID < m_nMaxPlayer)
				{
					...
					sAdd.m_nPlayerIdx = m_pGameStatus[pSync->m_uNetID].nPlayerIndex;
					sAdd.m_nCamp = pSync->m_btCamp;
					sAdd.m_nFigure = pSync->m_btFigure;
			...
			break;
		case enumS2C_TONG_ACCEPTMEMBER_FAIL:
			{
				STONG_ACCEPTMEMBER_FAIL_SYNC	*pSync = (STONG_ACCEPTMEMBER_FAIL_SYNC*)pChar;
				if ((UINT)m_pCoreServerShell->GetGameData(SGDI_CHARACTER_ID, pSync->m_nPlayerIdx, 0)
					!= pSync->m_dwPlayerNameID)
					break;
				if((UINT)m_pCoreServerShell->GetGameData(SGDI_TONG_GET_TONG_NAMEID, 0, pSync->m_nPlayerIdx)
					!= pSync->m_dwTongNameID)
					break;
				int nNetID = m_pCoreServerShell->GetGameData(SGDI_CHARACTER_NETID, pSync->m_nPlayerIdx, 0);
				if (nNetID < 0)
					break;
				TONG_ACCEPTMEMBER_FAIL_SYNC	sSync;
				sSync.ProtocolType		= s2c_extendtong;
				sSync.m_btMsgId			= enumTONG_SYNC_ID_ACCEPTMEMBER_FAIL;
				sSync.m_wLength			= sizeof(sSync) - 1;
				if (m_pServer)
					m_pServer->PackDataToClient(nNetID, &sSync, sSync.m_wLength + 1);
			}
			break;
		case enumS2C_TONG_HEAD_INFO:
			{
				...
				if (nNetID < 0)
					break;
				if((UINT)m_pCoreServerShell->GetGameData(SGDI_CHARACTER_ID, pInfo->m_dwParam, 0)
					!= pInfo->m_dwPID)
					break;
				TONG_HEAD_INFO_SYNC	sInfo;
				sInfo.ProtocolType		= s2c_extendtong;
				sInfo.m_btMsgId			= enumTONG_SYNC_ID_HEAD_INFO;
				sInfo.m_bSelf			= pInfo->m_bSelf;
				sInfo.m_nMoney			= pInfo->m_nMoney;
				sInfo.m_nBuildMoney		= pInfo->m_nBuildMoney;
				sInfo.m_btCamp			= pInfo->m_btCamp;
				sInfo.m_btLevel			= pInfo->m_btLevel;
				sInfo.m_MemberNum		= pInfo->m_MemberNum;
				sInfo.m_nOffer			= pInfo->m_nOffer;
				sInfo.m_nBuilLevel		= pInfo->m_nBuilLevel;
				sInfo.m_bLockRecruit	= pInfo->m_bLockRecruit;
				sInfo.m_btExpPercent	= pInfo->m_btExpPercent;
				memcpy(sInfo.m_szTongName, pInfo->m_szTongName, sizeof(pInfo->m_szTongName));
				memcpy(sInfo.m_szMaster, pInfo->m_szMaster, sizeof(pInfo->m_szMaster));
				memcpy(sInfo.m_szUnion, pInfo->m_szUnion, sizeof(pInfo->m_szUnion));
		...
		case enumS2C_TONG_MEMBER_INFO:
			{
				...
				sInfo.m_nPage			= pInfo->m_nPage;
				sInfo.m_nMemNum			= pInfo->m_nMemNum;
				sInfo.m_bBegin			= pInfo->m_bBegin;
				memcpy(sInfo.sMem, pInfo->sMem, sizeof(sInfo.sMem));
				sInfo.m_wLength = sizeof(sInfo) - 1 - sizeof(sInfo.sMem) + sizeof(STONG_MEMSUBINFO)*pInfo->m_nMemNum;
				if (m_pServer)
					m_pServer->PackDataToClient(nNetID, &sInfo, sInfo.m_wLength + 1);
			}
			break;
		case enumS2C_TONG_PAGE_INFO:
			{
				STONG_TONGPAGE_INFO_SYNC	*pInfo = (STONG_TONGPAGE_INFO_SYNC*)pChar;
				int nNetID = m_pCoreServerShell->GetGameData(SGDI_CHARACTER_NETID, pInfo->m_dwParam, 0);
				if (nNetID < 0)
					break;
				if((UINT)m_pCoreServerShell->GetGameData(SGDI_CHARACTER_ID, pInfo->m_dwParam, 0)
				...
				TONG_PAGE_INFO_SYNC	sInfo;
				sInfo.ProtocolType		= s2c_extendtong;
				sInfo.m_btMsgId			= enumTONG_SYNC_ID_TONGPAGE_INFO;
				sInfo.m_nPage			= pInfo->m_nPage;
				sInfo.m_nMemNum			= pInfo->m_nMemNum;
				sInfo.m_bBegin			= pInfo->m_bBegin;
				memcpy(sInfo.sMem, pInfo->sMem, sizeof(sInfo.sMem));
				sInfo.m_wLength = sizeof(sInfo) - 1 - sizeof(sInfo.sMem) + sizeof(STONG_PAGEMEM)*pInfo->m_nMemNum;
				if (m_pServer)
					m_pServer->PackDataToClient(nNetID, &sInfo, sInfo.m_wLength + 1);
			}
			break;
		case enumS2C_TONG_RECORD_AFFAIRHISTORY: //ket qua lay thong tin record
			{
				STONG_RECORD_AFFAIRHISTORY_SYNC	*pSync = (STONG_RECORD_AFFAIRHISTORY_SYNC*)pChar;
				if ((UINT)m_pCoreServerShell->GetGameData(SGDI_CHARACTER_ID, pSync->m_nPlayerIdx, 0)
					!= pSync->m_dwPlayerNameID)
					break;
				if((UINT)m_pCoreServerShell->GetGameData(SGDI_TONG_GET_TONG_NAMEID, 0, pSync->m_nPlayerIdx)
				...
				if (nNetID < 0)
					break;
				TONG_RECORD_AFFAIRHISTORY_SYNC	sSync;
				sSync.ProtocolType		= s2c_extendtong;
				sSync.m_btMsgId			= enumTONG_SYNC_ID_RECORD_AFFAIRHISTORY;
				sSync.m_wLength			= sizeof(sSync) - 1;
				sSync.m_nMsgCount			= pSync->m_nMsgCount;
				memcpy(sSync.m_szMsg, pSync->m_szMsg, sizeof(sSync.m_szMsg));

				if (m_pServer)
					m_pServer->PackDataToClient(nNetID, &sSync, sSync.m_wLength + 1);
			}
			break;
		case enumS2C_TONG_RECORD_ANNOUNCE: //ket qua lay thong tin record
			{
				STONG_RECORD_ANNOUNCE_SYNC	*pSync = (STONG_RECORD_ANNOUNCE_SYNC*)pChar;
				if ((UINT)m_pCoreServerShell->GetGameData(SGDI_CHARACTER_ID, pSync->m_nPlayerIdx, 0)
					!= pSync->m_dwPlayerNameID)
					break;
				if((UINT)m_pCoreServerShell->GetGameData(SGDI_TONG_GET_TONG_NAMEID, 0, pSync->m_nPlayerIdx)
				...
				if (nNetID < 0)
					break;
				TONG_RECORD_ANNOUNCE_SYNC	sSync;
				sSync.ProtocolType		= s2c_extendtong;
				sSync.m_btMsgId			= enumTONG_SYNC_ID_RECORD_ANNOUNCE;
				sSync.m_wLength			= sizeof(sSync) - 1;
				strcpy(sSync.m_szAnnounce, pSync->m_szAnnounce);

				if (m_pServer)
					m_pServer->PackDataToClient(nNetID, &sSync, sSync.m_wLength + 1);
			}
			break;
		case enumS2C_TONG_RECORD_WEEKTASK: //ket qua lay thong tin record
			{
				STONG_RECORD_WEEKTASK_SYNC	*pSync = (STONG_RECORD_WEEKTASK_SYNC*)pChar;
				if ((UINT)m_pCoreServerShell->GetGameData(SGDI_CHARACTER_ID, pSync->m_nPlayerIdx, 0)
					!= pSync->m_dwPlayerNameID)
					break;
				if((UINT)m_pCoreServerShell->GetGameData(SGDI_TONG_GET_TONG_NAMEID, 0, pSync->m_nPlayerIdx)
				...
				if (nNetID < 0)
					break;
				TONG_RECORD_WEEKTASK_SYNC	sSync;
				sSync.ProtocolType		= s2c_extendtong;
				sSync.m_btMsgId			= enumTONG_SYNC_ID_RECORD_WEEKTASK;
				sSync.m_wLength			= sizeof(sSync) - 1;
				sSync.m_OldWeekGoal		= pSync->m_OldWeekGoal;
				sSync.m_NewWeekGoal		= pSync->m_NewWeekGoal;
				sSync.m_nOldWGCompleted = pSync->m_nOldWGCompleted;
				sSync.m_nNewWGCompleted = pSync->m_nNewWGCompleted;
				sSync.m_bWGType			= pSync->m_bWGType;
				sSync.m_bGetPrice		= pSync->m_bGetPrice;
				sSync.m_uWeekCount		= pSync->m_uWeekCount;
				sSync.m_DayNum			= pSync->m_DayNum;
				sSync.m_nTaskLevel		= pSync->m_nTaskLevel;
				sSync.m_bOrdeal			= pSync->m_bOrdeal;
				if (m_pServer)
					m_pServer->PackDataToClient(nNetID, &sSync, sSync.m_wLength + 1);
			}
			break;
		case enumS2C_TONG_BE_INSTATED:
			{
				...
				if(pSync->m_uNetID >= 0 && pSync->m_uNetID < m_nMaxPlayer)
				{
					...
					sInstated.m_nPlayerIdx = m_pGameStatus[pSync->m_uNetID].nPlayerIndex;
					sInstated.m_dwPlayerNameID = pSync->m_dwPlayerNameID;
					sInstated.m_btFigure = pSync->m_btFigure;
					sInstated.m_dwTongNameID = pSync->m_dwTongNameID;
					m_pCoreServerShell->GetGameData(SGDI_TONG_BE_INSTATED, (unsigned int)&sInstated, 0);
				}
			}
			break;

		case enumS2C_TONG_INSTATE: //list full, bo nhiem that bai
			{
				STONG_INSTATE_SYNC	*pSync = (STONG_INSTATE_SYNC*)pChar;

				if ((UINT)m_pCoreServerShell->GetGameData(SGDI_CHARACTER_ID, pSync->m_nSenderPIdx, 0)
					!= pSync->m_uSenderID)
					break;
				if((UINT)m_pCoreServerShell->GetGameData(SGDI_TONG_GET_TONG_NAMEID, 0, pSync->m_nSenderPIdx)
					!= pSync->m_dwTongNameID)
					break;
				int nNetID = m_pCoreServerShell->GetGameData(SGDI_CHARACTER_NETID, pSync->m_nSenderPIdx, 0);
				if (nNetID < 0)
					break;
		...
		case enumS2C_TONG_KICK: //cho nguoi kich
			{
				...
				sKick.m_uDestID			= pSync->m_uDestID;
				sKick.m_nNeedMoney		= pSync->m_nNeedMoney;
		...
		case enumS2C_TONG_BE_KICKED: //cho nguoi bi. kich
			{
				STONG_BE_KICKED_SYNC	*pSync = (STONG_BE_KICKED_SYNC*)pChar;
				if(pSync->m_uNetID >= 0 && pSync->m_uNetID < m_nMaxPlayer)
				{
					STONG_SERVER_TO_CORE_BE_KICKED	sKicked;
					sKicked.m_nPlayerIdx = m_pGameStatus[pSync->m_uNetID].nPlayerIndex;
					sKicked.m_dwPlayerNameID = pSync->m_dwPlayerNameID;
					sKicked.m_dwTongNameID = pSync->m_dwTongNameID;
		...
		case enumS2C_TONG_LEAVE:
			{
				...
				sLeave.m_nPlayerIdx			= pSync->m_nPlayerIdx;
				sLeave.m_dwTongNameID		= pSync->m_dwTongNameID;
				sLeave.m_dwPlayerNameID		= pSync->m_dwPlayerNameID;
				sLeave.m_btSuccessFlag		= pSync->m_btSuccessFlag;
		...
		case enumS2C_TONG_CHANGE_AS:
			{
				...
				sChange.m_nPlayerIdx = pAs->m_nPlayerIdx;
				sChange.m_dwPlayerNameID = pAs->m_dwPlayerNameID;
		...
		case enumS2C_TONG_CHANGE_MASTER:
			{
				...
				m_pCoreServerShell->GetGameData(SGDI_TONG_CHANGE_MASTER, (unsigned int)pMaster, 0);
			}
			break;
		...
		case enumS2C_TONG_LOGIN_DATA:
			{
				...
				sLogin.m_uRight		= pLogin->m_uRight;
				sLogin.m_nFlag		= pLogin->m_btFlag;
				sLogin.m_nCamp		= pLogin->m_btCamp;
				sLogin.m_nFigure	= pLogin->m_btFigure;
				sLogin.m_nWeekOffer	= pLogin->m_nWeekOffer;
				sLogin.m_bWGType	= pLogin->m_bWGType;
			...
			break;
		case enumS2C_TONG_DISTRIB_RIGHT_RET: //cho bang chu~
			{
				STONG_DISTRIB_RIGHT_COMMAND	*pInfo = (STONG_DISTRIB_RIGHT_COMMAND*)pChar;
				m_pCoreServerShell->GetGameData(SGDI_TONG_RIGHT_SENDER_RET, (unsigned int)pInfo, 0);
			}
			break;
		case enumS2C_TONG_BE_RIGHT: //cho nguoi duoc phan quyen
			{
				STONG_BE_RIGHT_SYNC	*pInfo = (STONG_BE_RIGHT_SYNC*)pChar;
				if(pInfo->m_uNetID >= 0 && pInfo->m_uNetID < m_nMaxPlayer)
				m_pCoreServerShell->GetGameData(SGDI_TONG_BE_RIGHT_RET, (unsigned int)pInfo, m_pGameStatus[pInfo->m_uNetID].nPlayerIndex);
			}
			break;
		case enumS2C_TONG_CHANGERECRUIT: //cho nguoi thay doi tuyen dung
			{
				STONG_CHANGERECRUIT_SYNC	*pInfo = (STONG_CHANGERECRUIT_SYNC*)pChar;
				m_pCoreServerShell->GetGameData(SGDI_TONG_CHANGERECRUIT_RET, (unsigned int)pInfo, 0);
			}
			break;
		case enumS2C_TONG_DELETE: //xoa bang
			{
				STONG_DELETE_SYNC	*pInfo = (STONG_DELETE_SYNC*)pChar;
				m_pCoreServerShell->GetGameData(SGDI_TONG_DELETE, pInfo->m_dwTongNameID, 0);
			}
			break;
		case enumS2C_TONG_CONTRIBMONEY: //ket qua gui tien ngan quy~
			{
				STONG_CONTRIBMONEY_SYNC	*pSync = (STONG_CONTRIBMONEY_SYNC*)pChar;
				if ((UINT)m_pCoreServerShell->GetGameData(SGDI_CHARACTER_ID, pSync->m_nPlayerIdx, 0)
					!= pSync->m_dwPlayerNameID)
					break;
				if((UINT)m_pCoreServerShell->GetGameData(SGDI_TONG_GET_TONG_NAMEID, 0, pSync->m_nPlayerIdx)
			...
			break;
		case enumS2C_TONG_WITHDRAWMONEY: //ket qua rut tien ngan quy~
			{
				STONG_WITHDRAWMONEY_SYNC	*pSync = (STONG_WITHDRAWMONEY_SYNC*)pChar;
				if ((UINT)m_pCoreServerShell->GetGameData(SGDI_CHARACTER_ID, pSync->m_nPlayerIdx, 0)
					!= pSync->m_dwPlayerNameID)
					break;
				if((UINT)m_pCoreServerShell->GetGameData(SGDI_TONG_GET_TONG_NAMEID, 0, pSync->m_nPlayerIdx)
					...
					break;
				if(pSync->m_nMoney >= 0)
					m_pCoreServerShell->GetGameData(SGDI_TONG_WITHDRAWMONEY_SUCC, pSync->m_nPlayerIdx, pSync->m_nAddMoney);
				int nNetID = m_pCoreServerShell->GetGameData(SGDI_CHARACTER_NETID, pSync->m_nPlayerIdx, 0);
				if (nNetID < 0)
					break;
				TONG_CONTRIBMONEY_SYNC	sSync;
			...
			break;
		case enumS2C_TONG_STOREOFFER: //ket qua gui cong hien du tru
			{
				STONG_CONTRIBMONEY_SYNC	*pSync = (STONG_CONTRIBMONEY_SYNC*)pChar;
				if ((UINT)m_pCoreServerShell->GetGameData(SGDI_CHARACTER_ID, pSync->m_nPlayerIdx, 0)
					!= pSync->m_dwPlayerNameID)
				...
				TONG_CONTRIBMONEY_SYNC	sSync;
				sSync.ProtocolType		= s2c_extendtong;
				sSync.m_btMsgId			= enumTONG_SYNC_ID_STOREOFFER;
				sSync.m_wLength			= sizeof(sSync) - 1;
				sSync.m_nMoney			= pSync->m_nMoney;
			...
			break;
		case enumS2C_BE_DISPENSEOFFER: //ket qua nguoi duoc nhan cong hien du tru
			{
				STONG_CONTRIBMONEY_SYNC	*pSync = (STONG_CONTRIBMONEY_SYNC*)pChar;
				if ((UINT)m_pCoreServerShell->GetGameData(SGDI_CHARACTER_ID, pSync->m_nPlayerIdx, 0)
					!= pSync->m_dwPlayerNameID)
				...
				if((UINT)m_pCoreServerShell->GetGameData(SGDI_TONG_GET_TONG_NAMEID, 0, pSync->m_nPlayerIdx)
					!= pSync->m_dwTongNameID)
					break;
				m_pCoreServerShell->GetGameData(SGDI_TONG_BE_DISPENSEOFFER, pSync->m_nPlayerIdx, pSync->m_nMoney);
			}
			break;
		case enumS2C_ASSIGN_MONEY_RET: //ket qua phat ngan quy~
			{
				STONG_ASSIGNFUND_SYNC	*pSync = (STONG_ASSIGNFUND_SYNC*)pChar;
				if(pSync->m_bSuccess)
					m_pCoreServerShell->GetGameData(SGDI_TONG_ASSIGNMONEY_RET, (unsigned int)pSync, 0);
				...
				if (nNetID < 0)
					break;
				TONG_ASSIGNFUND_SYNC	sSync;
				sSync.ProtocolType		= s2c_extendtong;
				sSync.m_btMsgId			= enumTONG_SYNC_ID_ASSIGNMONEY;
				sSync.m_wLength			= sizeof(sSync) - 1;
				sSync.m_nMoney			= pSync->m_nMoney;
				sSync.m_bSuccess		= pSync->m_bSuccess;
				if (m_pServer)
					m_pServer->PackDataToClient(nNetID, &sSync, sSync.m_wLength + 1);
			}
			break;
		case enumS2C_ASSIGN_OFFER_RET: //ket qua phat cong hien
			{
				STONG_ASSIGNFUND_SYNC	*pSync = (STONG_ASSIGNFUND_SYNC*)pChar;
				if(pSync->m_bSuccess)
					m_pCoreServerShell->GetGameData(SGDI_TONG_ASSIGNOFFER_RET, (unsigned int)pSync, 0);
				...
				TONG_ASSIGNFUND_SYNC	sSync;
				sSync.ProtocolType		= s2c_extendtong;
				sSync.m_btMsgId			= enumTONG_SYNC_ID_ASSIGNOFFER;
				sSync.m_wLength			= sizeof(sSync) - 1;
				sSync.m_nMoney			= pSync->m_nMoney;
				sSync.m_bSuccess		= pSync->m_bSuccess;
				if (m_pServer)
			...
			break;
		case enumS2C_TONG_TRANSMONEY: //ket qua chuyen kien thiet
			{
				STONG_WITHDRAWMONEY_SYNC	*pSync = (STONG_WITHDRAWMONEY_SYNC*)pChar;				
				if ((UINT)m_pCoreServerShell->GetGameData(SGDI_CHARACTER_ID, pSync->m_nPlayerIdx, 0)
					!= pSync->m_dwPlayerNameID)
				...
				if (nNetID < 0)
					break;
				TONG_TRANSMONEY_SYNC	sSync;
				sSync.ProtocolType		= s2c_extendtong;
				sSync.m_btMsgId			= enumTONG_SYNC_ID_TRANSMONEY;
				sSync.m_wLength			= sizeof(sSync) - 1;
				sSync.m_nMoney			= pSync->m_nMoney;
				sSync.m_nBuildFund		= pSync->m_nAddMoney;
				if (m_pServer)
					m_pServer->PackDataToClient(nNetID, &sSync, sSync.m_wLength + 1);
			}
			break;
		case enumS2C_TONG_STOREBUILDFUND: //ket qua gui vao kien thiet
			{
				STONG_STOREBUILDFUND_SYNC	*pSync = (STONG_STOREBUILDFUND_SYNC*)pChar;				
				if ((UINT)m_pCoreServerShell->GetGameData(SGDI_CHARACTER_ID, pSync->m_nPlayerIdx, 0)
					!= pSync->m_dwPlayerNameID)
					break;
				if((UINT)m_pCoreServerShell->GetGameData(SGDI_TONG_GET_TONG_NAMEID, 0, pSync->m_nPlayerIdx)
					!= pSync->m_dwTongNameID)
					break;
				m_pCoreServerShell->GetGameData(SGDI_TONG_STOREBUILDFUND_RET, (unsigned int)pSync, pSync->m_nPlayerIdx);
				...
				TONG_CONTRIBMONEY_SYNC	sSync;
				sSync.ProtocolType		= s2c_extendtong;
				sSync.m_btMsgId			= enumTONG_SYNC_ID_BUILDFUND;
				sSync.m_wLength			= sizeof(sSync) - 1;
				sSync.m_nMoney			= pSync->m_nBuildFund;
				if (m_pServer)
					m_pServer->PackDataToClient(nNetID, &sSync, sSync.m_wLength + 1);
			}
			break;
		case enumS2C_TONG_WEEKLYRESET:
			{
				STONG_WEEKLYRESET_SYNC	*pSync = (STONG_WEEKLYRESET_SYNC*)pChar;
				m_pCoreServerShell->GetGameData(SGDI_TONG_WEEKLYRESET,
										pSync->m_dwTongNameID, pSync->m_nWeekGoalType);
			}
			break;
		case enumS2C_TONG_RECEIVEPRICE_RET: //ket qua nhan thuong~ bang hoi
			{
				STONG_RECEIVEPRICE_SYNC	*pSync = (STONG_RECEIVEPRICE_SYNC*)pChar;
				if ((UINT)m_pCoreServerShell->GetGameData(SGDI_CHARACTER_ID, pSync->m_nPlayerIdx, 0)
					!= pSync->m_dwPlayerNameID)
					break;
				if((UINT)m_pCoreServerShell->GetGameData(SGDI_TONG_GET_TONG_NAMEID, 0, pSync->m_nPlayerIdx)
					!= pSync->m_dwTongNameID)
					break;
				m_pCoreServerShell->GetGameData(SGDI_TONG_RECEIVEPRICE_RET, pSync->m_btSuccessFlag, pSync->m_nPlayerIdx);
			}
			break;
		case enumS2C_TONG_PLAYERPRICE_RET: //ket qua nhan thuong~ ca nhan
			{
				STONG_PLAYERPRICE_SYNC	*pSync = (STONG_PLAYERPRICE_SYNC*)pChar;
				if ((UINT)m_pCoreServerShell->GetGameData(SGDI_CHARACTER_ID, pSync->m_nPlayerIdx, 0)
					!= pSync->m_dwPlayerNameID)
					break;
				if((UINT)m_pCoreServerShell->GetGameData(SGDI_TONG_GET_TONG_NAMEID, 0, pSync->m_nPlayerIdx)
					!= pSync->m_dwTongNameID)
					break;
				m_pCoreServerShell->GetGameData(SGDI_TONG_PLAYERPRICE_RET, (unsigned int)pSync, pSync->m_nPlayerIdx);
			}
			break;
		case enumS2C_TONG_CHANGETITLE_UI: //ket qua doi~ ten cho sender
			{
				STONG_CHANGETITLE_SYNC	*pSync = (STONG_CHANGETITLE_SYNC*)pChar;
				if ((UINT)m_pCoreServerShell->GetGameData(SGDI_CHARACTER_ID, pSync->m_nPlayerIdx, 0)
					!= pSync->m_dwPlayerNameID)
					break;
				if((UINT)m_pCoreServerShell->GetGameData(SGDI_TONG_GET_TONG_NAMEID, 0, pSync->m_nPlayerIdx)
				...
				sSync.ProtocolType		= s2c_extendtong;
				sSync.m_btMsgId			= enumTONG_SYNC_ID_CHANGETITLE;
				sSync.m_wLength			= sizeof(sSync) - 1;
				sSync.m_uDestID			= pSync->m_uDestID;
				strcpy(sSync.m_szName, pSync->m_szName);
				if (m_pServer)
					m_pServer->PackDataToClient(nNetID, &sSync, sSync.m_wLength + 1);
			}
			break;
		case enumS2C_TONG_BE_CHANGETITLE: //ket qua cho nguoi duoc doi~ ten
			{
				STONG_CHANGETITLE_SYNC	*pSync = (STONG_CHANGETITLE_SYNC*)pChar;
				if(pSync->m_nPlayerIdx >= 0 && pSync->m_nPlayerIdx < m_nMaxPlayer)
				{
					m_pCoreServerShell->GetGameData(SGDI_TONG_BE_CHANGETITLE, (unsigned int)pSync, m_pGameStatus[pSync->m_nPlayerIdx].nPlayerIndex);
				}
			}
			break;
		case enumS2C_TONG_ADDWEEKGOAL_RET: //ket qua cho doi tuong addweekgoal
			{
				STONG_ADDWEEKGOAL_SYNC	*pSync = (STONG_ADDWEEKGOAL_SYNC*)pChar;
				if(pSync->m_uNetID >= 0 && pSync->m_uNetID < m_nMaxPlayer)
				{
					m_pCoreServerShell->GetGameData(SGDI_TONG_ADDWEEKGOAL_RET, (unsigned int)pSync, m_pGameStatus[pSync->m_uNetID].nPlayerIndex);
				}
			}
			break;
		case enumS2C_TONG_CHANGETITLEALL: //ket qua doi~ ten tat ca thanh vien
			{
				STONG_CHANGETITLE_SYNC	*pSync = (STONG_CHANGETITLE_SYNC*)pChar;
				m_pCoreServerShell->GetGameData(SGDI_TONG_CHANGETITLEALL_RET, (unsigned int)pSync, 0);
			}
			break;
		case enumS2C_TONG_CHANGECAMP: //ket qua doi~ phe
			{
				STONG_CHANGECAMP_SYNC	*pSync = (STONG_CHANGECAMP_SYNC*)pChar;
				m_pCoreServerShell->GetGameData(SGDI_TONG_CHANGECAMP_RET, (unsigned int)pSync, 0);
			}
			break;
		default:
			break;
...
void KSwordOnLineSever::DatabaseMessageProcess(const char* pData, size_t dataLength)
{
	...
	case s2c_roleserver_saverole_result:
		...
		break;
	case s2c_queryplayer://dang ket goddess
		{
			/*tagDBSelPlayer* p = (tagDBSelPlayer *)pData;
			int nIdx = m_pCoreServerShell->GetGameData(SGDI_CHARACTER_FIND, (unsigned int)&p->szRoleName, 0);
			int nNetID = m_pCoreServerShell->GetGameData(SGDI_CHARACTER_NETID, nIdx, 0);
			if(nNetID >= 0)
			{
		...
		case c2c_transferroleinfo:
			{
				...
				if ( pGI->datalength > 0 )
				{
					...
					m_pTransServer->SendData(m_TransSVID,pRD, sizeof(RELAY_DATA) + sizeof(tagPermitPlayerExchange));
...
//				SavePlayerData(nIndex);
				...
				if (m_pTransServer)
					m_pTransServer->SendData(m_TransSVID,&lg2, sizeof(tagLeaveGame2));
	...
	case c2c_notifyexchange:
		{
			...
			while(pBuffer)
			{
...
#ifndef _STANDALONE
				pRD->routeDateLength = (WORD)pBuffer->GetUsed();
#else
				...
				m_pTransServer->SendData(m_TransSVID,szData, pRD->routeDateLength + sizeof(RELAY_DATA));
#ifndef _STANDALONE
				try
				{
					if (pBuffer)
...
//#ifndef _STANDALONE
	...
	{
		IServer* pClient = NULL;//guve
		unsigned int uFnID = -1;
		switch (protocoltype)
		{
		case c2s_extend: pClient = m_pTransServer; uFnID = m_TransSVID; break;
		case c2s_extendchat: pClient = m_pChatServer; uFnID = m_ChatSVID; break;
		case c2s_extendfriend: pClient = m_pTongServer; uFnID = m_TongSVID; break;
		default:
			return;
		};
		...
		{		
			...
			pClient->SendData(uFnID, pPckg2, pckgsize);
#ifndef WIN32
			delete (char*)pPckg2;
#endif
		}
	...
	case enumPlayerSyncEnd:
		if (ProcessSyncReplyProtocol(lnID, pData, dataLength))
		{
			...
			eg2.lnID = lnID;
			if (m_pTransServer)
				m_pTransServer->SendData(m_TransSVID, (const void *)&eg2, sizeof(tagEnterGame2) );
			...
			{
				UINT	dwTongNameID = m_pCoreServerShell->GetGameData(SGDI_TONG_GET_TONG_NAMEID, 0, nPlayerIdx);
				char	szName[32];
				szName[0] = 0;
				...
				if (dwTongNameID > 0 && szName[0])
				{
					...
					if (m_pTongServer)
						m_pTongServer->SendData(m_TongSVID,(const void*)&sLogin, sizeof(sLogin));
				}
				else //guve
	...
	case s2c_querymapinfo:
		{
			...
			pUMI = ( tagUpdateMapID * )new BYTE[ nMapCount * sizeof(USHORT) + sizeof( tagUpdateMapID ) ];
			memset( pUMI, 0, nMapCount * sizeof(USHORT) + sizeof( tagUpdateMapID ) );
			
			pUMI->cProtocol = c2s_updatemapinfo;
			pUMI->cMapCount = nMapCount;
			memcpy( pUMI->szMapID, sMapID, nMapCount * sizeof(USHORT));
			
/*			printf("-----Send \ndatalen = %u, data = ", nMapCount * sizeof(char) + sizeof( tagUpdateMapID ));
			for (int i = 0; i < 32; i++)
				printf("[%02X] ", ((BYTE *)pUMI)[i]);
			printf("\n");*/
			m_pGatewayClient->SendPackToServer( ( const void * )pUMI, nMapCount * sizeof(USHORT) + sizeof( tagUpdateMapID ) );
...
//					pSW->ProtocolID = c2c_notifyexchange;
					...
					m_pTransServer->SendData(m_TransSVID,pRAD, sizeof(RELAY_ASKWAY_DATA) + pRAD->wMethodDataLength + pRAD->routeDateLength);
...
void KSwordOnLineSever::SetServerTrans(unsigned int lnID) //guve
{
	m_TransSVID = lnID;
	IServer *pCloneServer = NULL;
	m_pTransServer->QueryInterface( IID_IIOCPServer, ( void ** )&pCloneServer );
	m_pCoreServerShell->SetServerTrans((LPVOID)pCloneServer, m_TransSVID);
}

void KSwordOnLineSever::SetServerChat(unsigned int lnID)
{
	m_ChatSVID = lnID;
	IServer *pCloneServer = NULL;
	m_pChatServer->QueryInterface( IID_IIOCPServer, ( void ** )&pCloneServer );
	m_pCoreServerShell->SetServerChat((LPVOID)pCloneServer, m_ChatSVID);
}

void KSwordOnLineSever::SetServerTong(unsigned int lnID)
{
	m_TongSVID = lnID;
	IServer *pCloneServer = NULL;
	m_pTongServer->QueryInterface( IID_IIOCPServer, ( void ** )&pCloneServer );
	m_pCoreServerShell->SetServerTong((LPVOID)pCloneServer, m_TongSVID);
}
...
void KSwordOnLineSever::ExitAllPlayer()
{
	...
	for (lnID = 0; lnID < m_nMaxPlayer; lnID++)
	{
		...
		if (m_pCoreServerShell->IsCharacterQuiting(nIndex))
		{
			...
			m_pGatewayClient->SendPackToServer(&sLeaveGame, sizeof(tagLeaveGame));
			if (m_pTransServer)
				m_pTransServer->SendData(m_TransSVID,&sLeaveGame, sizeof(tagLeaveGame));
			m_pCoreServerShell->RemoveQuitingPlayer(nIndex);
		}
#ifdef WIN32
			Sleep(1);
...
void KSwordOnLineSever::TransferAskWayMessageProcess(const char *pData, size_t dataLength)
{
	...
	if (!ConformAskWay(pRAD, dataLength, &lnID))
	{
		...
		m_pTransServer->SendData(m_TransSVID,pRD, sizeof(RELAY_DATA) + dataLength);
	}
	else
	{
		pRD->ProtocolID = relay_c2c_data;
		...
		case rm_map_id:
			...
			m_pTransServer->SendData(m_TransSVID,pRD, sizeof(RELAY_DATA) + pRD->routeDateLength);
			break;
		case rm_role_id:
		case rm_account_id:
			{
	...
	case enumTONG_COMMAND_ID_APPLY_CREATE:
		{
			...
			sApply.m_szTongName[sizeof(sApply.m_szTongName)-1] = 0;
			int nRet = 0;
			if (m_pCoreServerShell)
			...
			if (nRet)
			{
				...
				sCreate.m_nSex = m_pCoreServerShell->GetGameData(SGDI_CHARACTER_SEX, 0, nPlayerIdx);
				sCreate.m_dwPlayerNameID = g_FileName2Id(szPlayerName);
				sCreate.m_btPlayerNameLength = (BYTE)strlen(szPlayerName);
				sCreate.m_btTongNameLength = (BYTE)strlen(sApply.m_szTongName);
				...
				if (m_pTongServer)
					m_pTongServer->SendData(m_TongSVID,(const void*)&sCreate, sCreate.m_wLength);
			}
		}
		break;
	case enumTONG_COMMAND_ID_APPLY_ADD:
		{
			...
			sAdd.m_uDestID = pApply->m_uDestID;
			if (m_pCoreServerShell)
	...
	case enumTONG_COMMAND_ID_ACCEPT_ADD:
		{
			...
			if (!pAccept->m_btFlag)
			{
				...
				sRefuse.m_dwNameID = pAccept->m_dwNameID;//targetid
			...
			else
			{
				...
				sAdd.m_dwNameID = pAccept->m_dwNameID;//targetid

				int		nRet = FALSE;
				UINT	uTongID = 0;
				if (m_pCoreServerShell)
					nRet = m_pCoreServerShell->GetGameData(SGDI_TONG_CHECK_ADD_CONDITION, (unsigned int)&uTongID, (unsigned int)&sAdd);
				if (nRet)
				{
					char	szPlayerName[32];
					szPlayerName[0] = 0;
					...
					sTong.m_dwTongNameID = uTongID;
					sTong.m_dwPlayerNameID = sAdd.m_dwNameID;
					sTong.m_uSenderID = (UINT)m_pCoreServerShell->GetGameData(SGDI_CHARACTER_ID, nPlayerIdx, 0);
					sTong.m_nSenderPIdx = nPlayerIdx;
					sTong.m_btFigure = 0;//member
					sTong.m_nSex = m_pCoreServerShell->GetGameData(SGDI_CHARACTER_SEX, 0, sAdd.m_nTargetIdx);
					sTong.m_btPlayerNameLength = (BYTE)strlen(szPlayerName);
					memcpy(&sTong.m_szBuffer[0], szPlayerName, sTong.m_btPlayerNameLength);
					sTong.m_wLength = sizeof(STONG_ADD_MEMBER_COMMAND) - sizeof(sTong.m_szBuffer) + sTong.m_btPlayerNameLength;
					if (m_pTongServer)
						m_pTongServer->SendData(m_TongSVID,(const void*)&sTong, sTong.m_wLength);
				}
			}
		}
		break;
	case enumTONG_COMMAND_ID_APPLY_INFO:
		{
			...
			if (pInfo->m_wLength < sizeof(TONG_APPLY_INFO_COMMAND) - 1)
				break;
			...
			case enumTONG_APPLY_INFO_ID_TONG_HEAD:
				{
					...
					STONG_GET_TONG_HEAD_INFO_COMMAND	sCmd;
					sCmd.ProtocolFamily	= pf_tong;
					sCmd.ProtocolID		= enumC2S_TONG_GET_HEAD_INFO;
					sCmd.m_dwParam		= nPlayerIdx;
					sCmd.m_dwPID		= m_pCoreServerShell->GetGameData(SGDI_CHARACTER_ID, nPlayerIdx, 0);
					sCmd.m_dwTongNameID	= pInfo->m_nParam1;
					sCmd.m_bSelf		= sGet.m_nParam2;
					if (m_pTongServer)
						m_pTongServer->SendData(m_TongSVID,(const void*)&sCmd, sizeof(sCmd));
				}
				break;
			case enumTONG_APPLY_INFO_MEMBERPAGE:
				{
					...
					STONG_GET_MEMBER_INFO_COMMAND	sCmd;
					sCmd.ProtocolFamily	= pf_tong;
					sCmd.ProtocolID		= enumC2S_TONG_GET_MEMBER_INFO;
					sCmd.m_dwParam		= nPlayerIdx;
					sCmd.m_dwPID		= m_pCoreServerShell->GetGameData(SGDI_CHARACTER_ID, nPlayerIdx, 0);
					sCmd.m_dwTongNameID	= pInfo->m_nParam1;
					sCmd.m_nPage		= pInfo->m_nParam2;
					if (m_pTongServer)
						m_pTongServer->SendData(m_TongSVID,(const void*)&sCmd, sizeof(sCmd));
				}
				break;
			case enumTONG_APPLY_INFO_TONGPAGE:
				{
					...
					STONG_GET_MEMBER_INFO_COMMAND	sCmd;
					sCmd.ProtocolFamily	= pf_tong;
					sCmd.ProtocolID		= enumC2S_TONG_GET_TONGPAGE_INFO;
					sCmd.m_dwParam		= nPlayerIdx;
					sCmd.m_dwPID		= m_pCoreServerShell->GetGameData(SGDI_CHARACTER_ID, nPlayerIdx, 0);
					sCmd.m_dwTongNameID	= pInfo->m_nParam1;
					sCmd.m_nPage		= pInfo->m_nParam2;
			...
			case enumTONG_APPLY_INFO_RECORD:
				{
					...
					STONG_GET_MEMBER_INFO_COMMAND	sCmd;
					sCmd.ProtocolFamily	= pf_tong;
					sCmd.ProtocolID		= enumC2S_TONG_GET_RECORD_INFO;
					sCmd.m_dwParam		= nPlayerIdx;
					sCmd.m_dwPID		= m_pCoreServerShell->GetGameData(SGDI_CHARACTER_ID, nPlayerIdx, 0);
					sCmd.m_dwTongNameID	= (UINT)m_pCoreServerShell->GetGameData(SGDI_TONG_GET_TONG_NAMEID, 0, nPlayerIdx);
					sCmd.m_nPage		= pInfo->m_nParam2;
					if (m_pTongServer)
						m_pTongServer->SendData(m_TongSVID,(const void*)&sCmd, sizeof(sCmd));
				}
	...
	case enumTONG_COMMAND_ID_APPLY_INSTATE:
		{
			...
			sInstate.m_nSenderPIdx	= nPlayerIdx;
			sInstate.m_uSenderID	= (UINT)m_pCoreServerShell->GetGameData(SGDI_CHARACTER_ID, nPlayerIdx, 0);
			sInstate.m_dwTongNameID	= (UINT)m_pCoreServerShell->GetGameData(SGDI_TONG_GET_TONG_NAMEID, 0, nPlayerIdx);
			sInstate.m_uDestID		= pApply->m_uDestID;
			if (m_pTongServer)
				m_pTongServer->SendData(m_TongSVID,(const void*)&sInstate, sizeof(sInstate));
		}
		break;
	...
	case enumTONG_COMMAND_ID_APPLY_KICK:
		{
			...
			int nRet = 0;
			if (m_pCoreServerShell)
				nRet = m_pCoreServerShell->GetGameData(SGDI_TONG_KICK_POWER, pKick->m_uDestID, nPlayerIdx);
			if (!nRet)
				break;
			...
			sKick.m_nSenderPIdx		= nPlayerIdx;
			sKick.m_dwTongNameID	= (UINT)m_pCoreServerShell->GetGameData(SGDI_TONG_GET_TONG_NAMEID, 0, nPlayerIdx);
			sKick.m_uSenderID		= (UINT)m_pCoreServerShell->GetGameData(SGDI_CHARACTER_ID, nPlayerIdx, 0);
			sKick.m_uDestID			= pKick->m_uDestID;
			if (m_pTongServer)
				m_pTongServer->SendData(m_TongSVID,(const void*)&sKick, sizeof(sKick));
		}
		break;
	case enumTONG_COMMAND_ID_APPLY_LEAVE:
		{
			...
			int nRet = 0;
			if (m_pCoreServerShell)
				nRet = m_pCoreServerShell->GetGameData(SGDI_TONG_LEAVE_POWER, 0, nPlayerIdx);
			if (nRet == 0)
				break;
			...
			sLeave.m_nPlayerIdx		= nPlayerIdx;
			sLeave.m_dwPlayerNameID = (UINT)m_pCoreServerShell->GetGameData(SGDI_CHARACTER_ID, nPlayerIdx, 0);
			sLeave.m_dwTongNameID	= (UINT)m_pCoreServerShell->GetGameData(SGDI_TONG_GET_TONG_NAMEID, 0, nPlayerIdx);

			if (m_pTongServer)
				m_pTongServer->SendData(m_TongSVID,(const void*)&sLeave, sizeof(sLeave));
		}
		break;
	case enumTONG_COMMAND_ID_APPLY_CHANGE_MASTER:
		{
			...
			int nRet = 0;
			if (m_pCoreServerShell)
				nRet = m_pCoreServerShell->GetGameData(SGDI_TONG_CHANGE_MASTER_POWER, pChange->m_uDestID, nPlayerIdx);
			if (nRet <= 0)
				break;
			...
			sAccept.m_uDestID		= pChange->m_uDestID;
			sAccept.m_nDestPIdx		= nRet;
			sAccept.m_dwTongNameID	= (UINT)m_pCoreServerShell->GetGameData(SGDI_TONG_GET_TONG_NAMEID, 0, nPlayerIdx);
			sAccept.m_uSenderID		= (UINT)m_pCoreServerShell->GetGameData(SGDI_CHARACTER_ID, nPlayerIdx, 0);
			sAccept.m_nSenderPIdx	= nPlayerIdx;
			if (m_pTongServer)
				m_pTongServer->SendData(m_TongSVID,(const void*)&sAccept, sizeof(sAccept));
		}
		break;
	case enumTONG_COMMAND_ID_APPLY_RIGHT:
		{
			TONG_APPLY_RIGHT_COMMAND	*pApply = (TONG_APPLY_RIGHT_COMMAND*)pData;
			if (pApply->m_wLength != sizeof(TONG_APPLY_RIGHT_COMMAND) - 1)
				break;
			STONG_SERVER_TO_CORE_APPLY_ADD	sAdd;
			sAdd.m_nPlayerIdx = nPlayerIdx;
			sAdd.m_uDestID = pApply->m_uDestID;
			if (m_pCoreServerShell && m_pCoreServerShell->GetGameData(SGDI_TONG_APPLY_RIGHT, (unsigned int)&sAdd, 0))
			{
				STONG_DISTRIB_RIGHT_COMMAND	sSendCmd;
				sSendCmd.ProtocolFamily	= pf_tong;
				sSendCmd.ProtocolID		= enumC2S_TONG_DISTRIB_RIGHT;
				sSendCmd.m_uDestID		= pApply->m_uDestID;
				sSendCmd.m_dwTongNameID	= (UINT)m_pCoreServerShell->GetGameData(SGDI_TONG_GET_TONG_NAMEID, 0, nPlayerIdx);
				sSendCmd.m_nSenderPIdx	= nPlayerIdx;
				sSendCmd.m_uSenderID	= (UINT)m_pCoreServerShell->GetGameData(SGDI_CHARACTER_ID, nPlayerIdx, 0);
				sSendCmd.m_uRight		= pApply->m_uRight;
				if (m_pTongServer)
					m_pTongServer->SendData(m_TongSVID,(const void*)&sSendCmd, sizeof(sSendCmd));
			}
		}
		break;
	case enumTONG_COMMAND_ID_APPLY_RECRUIT:
		{
			TONG_APPLY_LEAVE_COMMAND	*pRecruit = (TONG_APPLY_LEAVE_COMMAND*)pData;
			if (pRecruit->m_wLength + 1 != sizeof(TONG_APPLY_LEAVE_COMMAND))
				break;
			int nRet = 0;
			if (m_pCoreServerShell)
				nRet = m_pCoreServerShell->GetGameData(SGDI_TONG_CHANGERECRUIT_POWER, 0, nPlayerIdx);
			if (nRet == 0)
				break;

			STONG_LEAVE_COMMAND	sRecruit;
			sRecruit.ProtocolFamily	= pf_tong;
			sRecruit.ProtocolID		= enumC2S_TONG_CHANGERECRUIT;
			sRecruit.m_nPlayerIdx		= nPlayerIdx;
			sRecruit.m_dwPlayerNameID = (UINT)m_pCoreServerShell->GetGameData(SGDI_CHARACTER_ID, nPlayerIdx, 0);
			sRecruit.m_dwTongNameID	= (UINT)m_pCoreServerShell->GetGameData(SGDI_TONG_GET_TONG_NAMEID, 0, nPlayerIdx);

			if (m_pTongServer)
				m_pTongServer->SendData(m_TongSVID,(const void*)&sRecruit, sizeof(sRecruit));
		}
		break;
	case enumTONG_COMMAND_ID_APPLY_CONTRIBMONEY:
		{
			TONG_APPLY_CONTRIBMONEY_COMMAND	*pCmd = (TONG_APPLY_CONTRIBMONEY_COMMAND*)pData;
			if (pCmd->m_wLength + 1 != sizeof(TONG_APPLY_CONTRIBMONEY_COMMAND))
				break;
			int nRet = 0;
			if (m_pCoreServerShell)
				nRet = m_pCoreServerShell->GetGameData(SGDI_TONG_CONTRIBMONEY_POWER, nPlayerIdx, pCmd->m_nMoney);
			if (nRet == 0)
				break;
			STONG_CONTRIBMONEY_COMMAND	sCmd;
			sCmd.ProtocolFamily	= pf_tong;
			sCmd.ProtocolID		= enumC2S_TONG_CONTRIBMONEY;
			sCmd.m_nPlayerIdx		= nPlayerIdx;
			sCmd.m_dwPlayerNameID = (UINT)m_pCoreServerShell->GetGameData(SGDI_CHARACTER_ID, nPlayerIdx, 0);
			sCmd.m_dwTongNameID	= (UINT)m_pCoreServerShell->GetGameData(SGDI_TONG_GET_TONG_NAMEID, 0, nPlayerIdx);
			sCmd.m_nMoney = pCmd->m_nMoney;
			if (m_pTongServer)
				m_pTongServer->SendData(m_TongSVID,(const void*)&sCmd, sizeof(sCmd));
		}
		break;
	case enumTONG_COMMAND_ID_APPLY_WITHDRAWMONEY:
		{
			TONG_APPLY_CONTRIBMONEY_COMMAND	*pCmd = (TONG_APPLY_CONTRIBMONEY_COMMAND*)pData;
			if (pCmd->m_wLength + 1 != sizeof(TONG_APPLY_CONTRIBMONEY_COMMAND))
				break;
			int nRet = 0;
			if (m_pCoreServerShell)
				nRet = m_pCoreServerShell->GetGameData(SGDI_TONG_WITHDRAWMONEY_POWER, nPlayerIdx, pCmd->m_nMoney);
			if (nRet == 0)
				break;
			STONG_CONTRIBMONEY_COMMAND	sCmd;
			sCmd.ProtocolFamily	= pf_tong;
			sCmd.ProtocolID		= enumC2S_TONG_WITHDRAWMONEY;
			sCmd.m_nPlayerIdx		= nPlayerIdx;
			sCmd.m_dwPlayerNameID = (UINT)m_pCoreServerShell->GetGameData(SGDI_CHARACTER_ID, nPlayerIdx, 0);
			sCmd.m_dwTongNameID	= (UINT)m_pCoreServerShell->GetGameData(SGDI_TONG_GET_TONG_NAMEID, 0, nPlayerIdx);
			sCmd.m_nMoney = pCmd->m_nMoney;
		...
		break;
	case enumTONG_COMMAND_ID_APPLY_STOREOFFER:
		{
			TONG_APPLY_CONTRIBMONEY_COMMAND	*pCmd = (TONG_APPLY_CONTRIBMONEY_COMMAND*)pData;
			if (pCmd->m_wLength + 1 != sizeof(TONG_APPLY_CONTRIBMONEY_COMMAND))
				break;
			int nRet = 0;
			if (m_pCoreServerShell)
				nRet = m_pCoreServerShell->GetGameData(SGDI_TONG_STOREOFFER_POWER, nPlayerIdx, pCmd->m_nMoney);
			if (nRet == 0)
				break;
			STONG_CONTRIBMONEY_COMMAND	sCmd;
			sCmd.ProtocolFamily	= pf_tong;
		...
		break;
	case enumTONG_COMMAND_ID_APPLY_DISPENSEOFFER:
		{
			TONG_APPLY_DISPENSEOFFER_COMMAND	*pCmd = (TONG_APPLY_DISPENSEOFFER_COMMAND*)pData;
			if (pCmd->m_wLength + 1 != sizeof(TONG_APPLY_DISPENSEOFFER_COMMAND))
				break;
			int nRet = 0;
			if (m_pCoreServerShell)
				nRet = m_pCoreServerShell->GetGameData(SGDI_TONG_DISPENSEOFFER_POWER, (unsigned int)pCmd, nPlayerIdx);
			if (nRet == 0)
				break;
			STONG_DISPENSEOFFER_COMMAND	sCmd;
			sCmd.ProtocolFamily	= pf_tong;
			sCmd.ProtocolID		= enumC2S_TONG_DISPENSEOFFER;
			sCmd.m_nPlayerIdx		= nPlayerIdx;
			sCmd.m_dwPlayerNameID = (UINT)m_pCoreServerShell->GetGameData(SGDI_CHARACTER_ID, nPlayerIdx, 0);
			sCmd.m_dwTongNameID	= (UINT)m_pCoreServerShell->GetGameData(SGDI_TONG_GET_TONG_NAMEID, 0, nPlayerIdx);
			sCmd.m_nMoney = pCmd->m_nMoney;
			sCmd.m_nDestPIdx		= nRet;
			sCmd.m_uDestID = (UINT)m_pCoreServerShell->GetGameData(SGDI_CHARACTER_ID, nRet, 0);
			if (m_pTongServer)
				m_pTongServer->SendData(m_TongSVID,(const void*)&sCmd, sizeof(sCmd));
		}
		break;
	case enumTONG_COMMAND_ID_APPLY_ASSIGNMONEY:
		{
			TONG_APPLY_ASSIGNFUND_COMMAND	*pCmd = (TONG_APPLY_ASSIGNFUND_COMMAND*)pData;
			if (pCmd->m_wLength + 1 != sizeof(TONG_APPLY_ASSIGNFUND_COMMAND))
				break;
			int nRet = 0;
			if (m_pCoreServerShell)
				nRet = m_pCoreServerShell->GetGameData(SGDI_TONG_ASSIGNMONEY_POWER, (unsigned int)pCmd, nPlayerIdx);
			if (nRet == 0)
				break;
			STONG_ASSIGNFUND_COMMAND	sCmd;
			sCmd.ProtocolFamily	= pf_tong;
			sCmd.ProtocolID		= enumC2S_TONG_ASSIGNMONEY;
			sCmd.m_nPlayerIdx		= nPlayerIdx;
			sCmd.m_dwPlayerNameID = (UINT)m_pCoreServerShell->GetGameData(SGDI_CHARACTER_ID, nPlayerIdx, 0);
			sCmd.m_dwTongNameID	= (UINT)m_pCoreServerShell->GetGameData(SGDI_TONG_GET_TONG_NAMEID, 0, nPlayerIdx);
			sCmd.nMemberPoint	= pCmd->nMemberPoint;
			sCmd.nManagerPoint	= pCmd->nManagerPoint;
			sCmd.nDirectorPoint	= pCmd->nDirectorPoint;
			if (m_pTongServer)
				m_pTongServer->SendData(m_TongSVID,(const void*)&sCmd, sizeof(sCmd));
		}
		break;
	case enumTONG_COMMAND_ID_APPLY_ASSIGNOFFER:
		{
			TONG_APPLY_ASSIGNFUND_COMMAND	*pCmd = (TONG_APPLY_ASSIGNFUND_COMMAND*)pData;
			if (pCmd->m_wLength + 1 != sizeof(TONG_APPLY_ASSIGNFUND_COMMAND))
				break;
			...
			STONG_ASSIGNFUND_COMMAND	sCmd;
			sCmd.ProtocolFamily	= pf_tong;
			sCmd.ProtocolID		= enumC2S_TONG_ASSIGNOFFER;
			sCmd.m_nPlayerIdx		= nPlayerIdx;
			sCmd.m_dwPlayerNameID = (UINT)m_pCoreServerShell->GetGameData(SGDI_CHARACTER_ID, nPlayerIdx, 0);
			sCmd.m_dwTongNameID	= (UINT)m_pCoreServerShell->GetGameData(SGDI_TONG_GET_TONG_NAMEID, 0, nPlayerIdx);
			sCmd.nMemberPoint	= pCmd->nMemberPoint;
		...
		break;
	case enumTONG_COMMAND_ID_APPLY_TRANSMONEY:
		{
			TONG_APPLY_CONTRIBMONEY_COMMAND	*pCmd = (TONG_APPLY_CONTRIBMONEY_COMMAND*)pData;
			if (pCmd->m_wLength + 1 != sizeof(TONG_APPLY_CONTRIBMONEY_COMMAND))
				break;
			int nRet = 0;
			if (m_pCoreServerShell)
				nRet = m_pCoreServerShell->GetGameData(SGDI_TONG_TRANSMONEY_POWER, nPlayerIdx, pCmd->m_nMoney);
			if (nRet == 0)
				break;
			STONG_CONTRIBMONEY_COMMAND	sCmd;
			sCmd.ProtocolFamily	= pf_tong;
			sCmd.ProtocolID		= enumC2S_TONG_TRANSMONEY;
			sCmd.m_nPlayerIdx		= nPlayerIdx;
			sCmd.m_dwPlayerNameID = (UINT)m_pCoreServerShell->GetGameData(SGDI_CHARACTER_ID, nPlayerIdx, 0);
			sCmd.m_dwTongNameID	= (UINT)m_pCoreServerShell->GetGameData(SGDI_TONG_GET_TONG_NAMEID, 0, nPlayerIdx);
			sCmd.m_nMoney	= pCmd->m_nMoney;
		...
		break;
	case enumTONG_COMMAND_ID_APPLY_STOREBUILDFUND:
		{
			TONG_APPLY_CONTRIBMONEY_COMMAND	*pCmd = (TONG_APPLY_CONTRIBMONEY_COMMAND*)pData;
			if (pCmd->m_wLength + 1 != sizeof(TONG_APPLY_CONTRIBMONEY_COMMAND))
				break;
			int nRet = 0;
			if (m_pCoreServerShell)
				nRet = m_pCoreServerShell->GetGameData(SGDI_TONG_STOREBUILDFUND_CHECK, nPlayerIdx, pCmd->m_nMoney);
			if (nRet == 0)
				break;
			STONG_CONTRIBMONEY_COMMAND	sCmd;
			sCmd.ProtocolFamily	= pf_tong;
			sCmd.ProtocolID		= enumC2S_TONG_STOREBUILDFUND;
			sCmd.m_nPlayerIdx		= nPlayerIdx;
			sCmd.m_dwPlayerNameID = (UINT)m_pCoreServerShell->GetGameData(SGDI_CHARACTER_ID, nPlayerIdx, 0);
			sCmd.m_dwTongNameID	= (UINT)m_pCoreServerShell->GetGameData(SGDI_TONG_GET_TONG_NAMEID, 0, nPlayerIdx);
			sCmd.m_nMoney	= pCmd->m_nMoney;
		...
		break;
	case enumTONG_COMMAND_ID_APPLY_ANNOUNCE:
		{
			TONG_APPLY_ANNOUNCE_COMMAND	*pCmd = (TONG_APPLY_ANNOUNCE_COMMAND*)pData;
			if (pCmd->m_wLength + 1 != sizeof(TONG_APPLY_ANNOUNCE_COMMAND))
				break;
			int nRet = 0;
			if (m_pCoreServerShell)
				nRet = m_pCoreServerShell->GetGameData(SGDI_TONG_ANNOUNCE_CHECK, nPlayerIdx, 0);
			if (nRet == 0)
				break;
			STONG_ANNOUNCE_COMMAND	sCmd;
			sCmd.ProtocolFamily	= pf_tong;
			sCmd.ProtocolID		= enumC2S_TONG_ANNOUNCE;
			sCmd.m_dwPlayerNameID = (UINT)m_pCoreServerShell->GetGameData(SGDI_CHARACTER_ID, nPlayerIdx, 0);
			sCmd.m_dwTongNameID	= (UINT)m_pCoreServerShell->GetGameData(SGDI_TONG_GET_TONG_NAMEID, 0, nPlayerIdx);
			memcpy(sCmd.m_szAnnounce, pCmd->m_szAnnounce, sizeof(sCmd.m_szAnnounce));
			sCmd.m_szAnnounce[sizeof(sCmd.m_szAnnounce)-1] = 0;
			if (m_pTongServer)
				m_pTongServer->SendData(m_TongSVID,(const void*)&sCmd, sizeof(sCmd));
		}
		break;
	case enumTONG_COMMAND_ID_APPLY_CHANGETITLE:
		{
			TONG_APPLY_CHANGETITLE_COMMAND	*pCmd = (TONG_APPLY_CHANGETITLE_COMMAND*)pData;
			if (pCmd->m_wLength + 1 != sizeof(TONG_APPLY_CHANGETITLE_COMMAND))
				break;
			int nRet = 0;
			if (m_pCoreServerShell)
				nRet = m_pCoreServerShell->GetGameData(SGDI_TONG_CHANGETITLE_CHECK, (unsigned int)pCmd, nPlayerIdx);
			if (nRet == 0)
				break;
			...
			STONG_CHANGETITLE_COMMAND	sCmd;
			sCmd.ProtocolFamily	= pf_tong;
			sCmd.ProtocolID		= enumC2S_TONG_CHANGETITLE;
			sCmd.m_dwPlayerNameID = (UINT)m_pCoreServerShell->GetGameData(SGDI_CHARACTER_ID, nPlayerIdx, 0);
			sCmd.m_dwTongNameID	= (UINT)m_pCoreServerShell->GetGameData(SGDI_TONG_GET_TONG_NAMEID, 0, nPlayerIdx);
			sCmd.m_nPlayerIdx	= nPlayerIdx;
			sCmd.m_uDestID		= pCmd->m_uDestID;
			sCmd.m_nDestFigure	= pCmd->m_nDestFigure;
			strcpy(sCmd.m_szName, szTitle);
			if (m_pTongServer)
				m_pTongServer->SendData(m_TongSVID,(const void*)&sCmd, sizeof(sCmd));
		}
		break;
	case enumTONG_COMMAND_ID_APPLY_CHANGETITLEALL:
		{
			TONG_APPLY_CHANGETITLE_COMMAND	*pCmd = (TONG_APPLY_CHANGETITLE_COMMAND*)pData;
			if (pCmd->m_wLength + 1 != sizeof(TONG_APPLY_CHANGETITLE_COMMAND))
				break;
			int nRet = 0;
			if (m_pCoreServerShell)
				nRet = m_pCoreServerShell->GetGameData(SGDI_TONG_CHANGETITLEALL_CHECK, (unsigned int)pCmd, nPlayerIdx);
			if (nRet == 0)
				break;
			char szTitle[defTONG_NAME_MAX_LENGTH+1];
			memcpy(szTitle, pCmd->m_szName, defTONG_NAME_MAX_LENGTH);
			...
			STONG_CHANGETITLE_COMMAND	sCmd;
			sCmd.ProtocolFamily	= pf_tong;
			sCmd.ProtocolID		= enumC2S_TONG_CHANGETITLEALL;
			sCmd.m_dwPlayerNameID = (UINT)m_pCoreServerShell->GetGameData(SGDI_CHARACTER_ID, nPlayerIdx, 0);
			sCmd.m_dwTongNameID	= (UINT)m_pCoreServerShell->GetGameData(SGDI_TONG_GET_TONG_NAMEID, 0, nPlayerIdx);
			sCmd.m_nPlayerIdx	= nPlayerIdx;
			sCmd.m_uDestID		= pCmd->m_uDestID;
			sCmd.m_nDestFigure	= pCmd->m_nDestFigure?1:0;
			strcpy(sCmd.m_szName, szTitle);
			if (m_pTongServer)
				m_pTongServer->SendData(m_TongSVID,(const void*)&sCmd, sizeof(sCmd));
		}
		break;
	case enumTONG_COMMAND_ID_APPLY_CHANGECAMP:
		{
			TONG_APPLY_CHANGECAMP_COMMAND	*pCmd = (TONG_APPLY_CHANGECAMP_COMMAND*)pData;
			if (pCmd->m_wLength + 1 != sizeof(TONG_APPLY_CHANGECAMP_COMMAND))
				break;
			int nRet = 0;
			if (m_pCoreServerShell)
				nRet = m_pCoreServerShell->GetGameData(SGDI_TONG_CHANGECAMP_CHECK, pCmd->m_btCamp, nPlayerIdx);
			if (nRet == 0)
				break;
			
			STONG_CHANGECAMP_COMMAND	sCmd;
			sCmd.ProtocolFamily	= pf_tong;
			sCmd.ProtocolID		= enumC2S_TONG_CHANGECAMP;
			sCmd.m_dwPlayerNameID = (UINT)m_pCoreServerShell->GetGameData(SGDI_CHARACTER_ID, nPlayerIdx, 0);
			sCmd.m_dwTongNameID	= (UINT)m_pCoreServerShell->GetGameData(SGDI_TONG_GET_TONG_NAMEID, 0, nPlayerIdx);
			sCmd.m_nPlayerIdx	= nPlayerIdx;
			sCmd.m_btCamp		= pCmd->m_btCamp;
			if (m_pTongServer)
				m_pTongServer->SendData(m_TongSVID,(const void*)&sCmd, sizeof(sCmd));
		}
		break;
	case enumTONG_COMMAND_ID_APPLY_ACTION:
		{
			TONG_APPLY_ACTION_COMMAND	*pCmd = (TONG_APPLY_ACTION_COMMAND*)pData;
			if (pCmd->m_wLength + 1 != sizeof(TONG_APPLY_ACTION_COMMAND))
				break;
			m_pCoreServerShell->GetGameData(SGDI_TONG_ACTION, (unsigned int)pCmd, nPlayerIdx);
		}
		break;
	default:
		break;
