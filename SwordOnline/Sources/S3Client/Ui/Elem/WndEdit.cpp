...
int KWndEdit::Init(KIniFile* pIniFile, const char* pSection)
{
	if (KWndWindow::Init(pIniFile, pSection))
	{
		...
		pIniFile->GetInteger(pSection, "FocusBKColorAlpha", 0, &nValue);
...
//			CreateCaret(::g_GetMainHWnd(), (HBITMAP)NULL, 2, m_nFontSize);
			...
			if (m_pParentWnd)	//guve pass parent
				m_pParentWnd->WndProc(WND_M_SET_FOCUS, (unsigned int)(KWndWindow*)this, 0);
...
//        DestroyCaret();
		...
		m_Flag &= ~WNDEDIT_F_HAVE_FOCUS;
		if (m_pParentWnd)	//guve pass parent
			m_pParentWnd->WndProc(WND_M_KILL_FOCUS, (unsigned int)(KWndWindow*)this, 0);
		break;
	case WM_SYSKEYDOWN:
		if (m_pParentWnd)
...
int KWndEdit::InsertString(const char* pString, int nLen)
{
	...
	if (pString)
	{
		...
		{
			if (TIsTCVNText() /*&& (unsigned char)(pString[nPos]) <= 0x80*/)
			{
...
{
	if (!TIsTCVNText() && nChar > 0x80)	//此判断与语言编码规则相关
	{
		if (m_Flag & WNDEDIT_ES_IME_AVAILABLE)
	...
	else if ((BYTE)nChar >= ' ' /*&& nChar <= '~'*/)
	{
		if ((m_Flag & WNDEDIT_ES_CHAR_TYPE_FILTER) || (nChar >= '0' && nChar <= '9'))
			return (InsertChar((char)((BYTE)nChar & 0xff), 0));
	}
	return false;
}
...
int SplitStringLine(char* pString, int nLine, int nLineByte)
{
	...
	if (pString)
	{
		...
		while(pString[nPos])
		{
			if (!bVNFont && (BYTE)pString[nPos] > 0x80)
			{
