...
void KWndImage::SetSprByFrame(const char* pszImgName, int nFrame, int* pX, int* pY)
{
	m_Image.nType = ISI_T_SPR;
	strncpy(m_Image.szImage, pszImgName, sizeof(m_Image.szImage));
	m_Image.szImage[sizeof(m_Image.szImage) - 1] = 0;
	...
	if (g_pRepresentShell)
	{
		KRPosition2 sSize;
		KRPosition2 sOffset;
		if(g_pRepresentShell->GetImageFrameParam(pszImgName, nFrame, &sOffset, &sSize, ISI_T_SPR))
		{
			SetSize(sSize.nX, sSize.nY);
			if(pX)
			*pX = sOffset.nX;
			if(pY)
			*pY  = sOffset.nY;
...
void KWndImage::PaintWindow()
{
	...
	if (g_pRepresentShell)
	{
		m_Image.oPosition.nX = m_nAbsoluteLeft + m_ImgOffX;
		m_Image.oPosition.nY = m_nAbsoluteTop + m_ImgOffY;
