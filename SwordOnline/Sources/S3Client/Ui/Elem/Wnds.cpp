...
void Wnd_ProcessInput(unsigned int uMsg, unsigned int uParam, int nParam)
{
	...
	{//====处理其他消息（其实都是键盘输入消息）====
		...
		if (uMsg == WM_KEYDOWN && s_WndStation.pGameSpaceWnd) //fdb
			s_WndStation.pGameSpaceWnd->WndProc(uMsg, uParam, nParam);
...
void Wnd_PreventCursor(bool bPrev)
{
	m_bPreventCursor = bPrev;
}
//设置屏幕范围大小
...
void Wnd_AutoWndAlign()
{
	KWndWindow* pWnd = s_WndStation.TopLayerRoot.GetNextWnd();
	while(pWnd)
	{
		pWnd->AutoWndAlign();
		pWnd = pWnd->GetNextWnd();
	}
	pWnd = s_WndStation.NormalLayerRoot.GetNextWnd();
	while(pWnd)
	{
		pWnd->AutoWndAlign();
		pWnd = pWnd->GetNextWnd();
	}
	pWnd = s_WndStation.LowLayerRoot.GetNextWnd();
	while(pWnd)
	{
		pWnd->AutoWndAlign();
		pWnd = pWnd->GetNextWnd();
