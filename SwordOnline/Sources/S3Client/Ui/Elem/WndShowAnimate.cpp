...
int KWndShowAnimate::Init(KIniFile* pIniFile, const char* pSection)
{
	if (KWndMovingImage::Init(pIniFile, pSection))
	{
		...
		m_OriAppearRange = m_AppearRange;
		m_AppearRange.cx -= m_oFixPos.x;
		m_AppearRange.cy -= m_oFixPos.y;
		int w,h;
		...
		if(m_AppearRange.cx)
		{
			if(m_OriAppearRange.cx > 0)
				m_AppearRange.cx = m_OriAppearRange.cx + (w-SCREEN_WIDTH) - m_oFixPos.x;
			else
				m_AppearRange.cx = m_OriAppearRange.cx - m_oFixPos.x;
		}
		if(m_AppearRange.cy)
		{
			if(m_OriAppearRange.cy > 0)
				m_AppearRange.cy = m_OriAppearRange.cy + (h-SCREEN_HEIGHT) - m_oFixPos.y;
			else
				m_AppearRange.cy = m_OriAppearRange.cy - m_oFixPos.y;
		}
...
void KWndShowAnimate::AutoWndAlign()
{
	if(m_pParentWnd)
		return;
	...
	{
		m_oFixPos.x = w - m_Width - m_nWndAlignW;
	}
	else if(m_Style & WND_S_WINDOW_ALIGN_CENTERW)
	{
		m_oFixPos.x = w/2 - m_Width/2;
	}
	if(m_Style & WND_S_WINDOW_ALIGN_B_EDGE)
	{
		m_oFixPos.y = h - m_Height - m_nWndAlignH;
	}
	else if(m_Style & WND_S_WINDOW_ALIGN_CENTERH)
	{
		m_oFixPos.y = h/2 - m_Height/2;
	}
	if (m_AppearRange.cx || m_AppearRange.cy)
	{
		...
		if(m_DisappearRange.cx)
		{
			if(m_OriDisRange.cx > 0)
				m_DisappearRange.cx = m_OriDisRange.cx + (w-SCREEN_WIDTH) - m_oFixPos.x;
			else
				m_DisappearRange.cx = m_OriDisRange.cx - m_oFixPos.x;
		}
		if(m_DisappearRange.cy)
		{
			if(m_OriDisRange.cy > 0)
				m_DisappearRange.cy = m_OriDisRange.cy + (h-SCREEN_HEIGHT) - m_oFixPos.y;
			else
				m_DisappearRange.cy = m_OriDisRange.cy - m_oFixPos.y;
		}
