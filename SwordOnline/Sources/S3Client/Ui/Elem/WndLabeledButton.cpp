...
int KWndLabeledButton::Init(KIniFile* pIniFile, const char* pSection)
{
	if (KWndButton::Init(pIniFile, pSection))
	{
		...
		pIniFile->GetInteger(pSection, "LabelXOffset", 0, &m_nLabelXOffset);//guve
		int nValue;
		pIniFile->GetInteger(pSection, "Enable", 1, &nValue);
		if(!nValue)
			Enable(0);
		pIniFile->GetInteger(pSection, "FullText", 0, &m_bFullText);
		...
		if(!(m_SelectColor & 0xffffff))
			m_SelectColor = m_OverColor;
...
void KWndLabeledButton::PaintWindow()
{
	...
	if (g_pRepresentShell)
	{
		...
		const char* pShowString = NULL;
		if(!m_bFullText)
			pShowString = TGetLimitLenString(m_Label, -1, Buffer, nMaxLen);
		else
			pShowString = &m_Label[0];
		if (pShowString)
		{
			...
			else if(m_Flag & WNDBTN_F_CHECKED)
			{
				uColor = m_SelectColor;
			}
			int nXOffset = (m_Width - nLen * m_nFontSize / 2) / 2; //guve
			if(m_nLabelXOffset > 0)
				nXOffset = m_nLabelXOffset;
			g_pRepresentShell->OutputText(m_nFontSize, pShowString, nLen,
				m_nAbsoluteLeft + nXOffset,
