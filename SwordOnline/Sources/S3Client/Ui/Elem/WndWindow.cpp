...
int KWndWindow::Init(KIniFile* pIniFile, const char* pSection)
{
	...
	if (pIniFile && pSection)
	{
		...
		if(m_pParentWnd)
		{
			m_Style &= ~WND_S_WINDOW_ALIGN_CENTERW;
			m_Style &= ~WND_S_WINDOW_ALIGN_CENTERH;
			m_Style &= ~WND_S_WINDOW_ALIGN_R_EDGE;
			m_Style &= ~WND_S_WINDOW_ALIGN_B_EDGE;
		}
		else
		{
			nValue1 = nValue2 = 0;
			pIniFile->GetInteger2(pSection, "WCenter", &nValue1, &nValue2);
			if(nValue1)
				m_Style |= WND_S_WINDOW_ALIGN_CENTERW;
			else
				m_Style &= ~WND_S_WINDOW_ALIGN_CENTERW;
			...
			pIniFile->GetInteger(pSection, "WAlignR", 0, &nValue1);
			if(nValue1)
			{
				m_Style |= WND_S_WINDOW_ALIGN_R_EDGE;
				m_Style &= ~WND_S_WINDOW_ALIGN_CENTERW;
				...
				m_Style &= ~WND_S_WINDOW_ALIGN_R_EDGE;
			pIniFile->GetInteger(pSection, "WAlignB", 0, &nValue1);
			if(nValue1)
			{
				m_Style |= WND_S_WINDOW_ALIGN_B_EDGE;
				m_Style &= ~WND_S_WINDOW_ALIGN_CENTERH;
				...
				m_Style &= ~WND_S_WINDOW_ALIGN_B_EDGE;
			if(m_Style & WND_S_WINDOW_ALIGN_R_EDGE)
			{
				Wnd_GetScreenSize(nValue1, nValue2);
				m_nWndAlignW = SCREEN_WIDTH - (m_Left + m_Width);
				SetPosition(nValue1 - m_Width - m_nWndAlignW, m_Top);
			}
			else if(m_Style & WND_S_WINDOW_ALIGN_CENTERW)
			{
				Wnd_GetScreenSize(nValue1, nValue2);
				SetPosition(nValue1/2 - m_Width/2, m_Top);
			}
			if(m_Style & WND_S_WINDOW_ALIGN_B_EDGE)
			{
				Wnd_GetScreenSize(nValue1, nValue2);
				m_nWndAlignH = SCREEN_HEIGHT - (m_Top + m_Height);
				SetPosition(m_Left, nValue2 - m_Height - m_nWndAlignH);
			}
			else if(m_Style & WND_S_WINDOW_ALIGN_CENTERH)
			{
				Wnd_GetScreenSize(nValue1, nValue2);
				SetPosition(m_Left, nValue2/2 - m_Height/2);
			}
		}
		return true;
	}
...
void KWndWindow::AutoWndAlign()
{
	...
	if(m_Style & WND_S_WINDOW_ALIGN_R_EDGE)
	{
		SetPosition(w - m_Width - m_nWndAlignW, m_Top);
	}
	else if(m_Style & WND_S_WINDOW_ALIGN_CENTERW)
	{
		SetPosition(w/2 - m_Width/2, m_Top);
	}
	if(m_Style & WND_S_WINDOW_ALIGN_B_EDGE)
	{
		SetPosition(m_Left, h - m_Height - m_nWndAlignH);
	}
	else if(m_Style & WND_S_WINDOW_ALIGN_CENTERH)
	{
		SetPosition(m_Left, h/2 - m_Height/2);
	}
}

void KWndWindow::InitWndAlign(int nStyle)
{
	if(m_pParentWnd)
		return;
	if(nStyle & WND_S_WINDOW_ALIGN_CENTERW)
		m_Style |= WND_S_WINDOW_ALIGN_CENTERW;
	else
		m_Style &= ~WND_S_WINDOW_ALIGN_CENTERW;
	if(nStyle & WND_S_WINDOW_ALIGN_CENTERH)
		m_Style |= WND_S_WINDOW_ALIGN_CENTERH;
	else
		m_Style &= ~WND_S_WINDOW_ALIGN_CENTERH;
	
	if(nStyle & WND_S_WINDOW_ALIGN_R_EDGE)
	{
		m_Style |= WND_S_WINDOW_ALIGN_R_EDGE;
		m_Style &= ~WND_S_WINDOW_ALIGN_CENTERW;
	}
		...
		m_Style &= ~WND_S_WINDOW_ALIGN_R_EDGE;
	if(nStyle & WND_S_WINDOW_ALIGN_B_EDGE)
	{
		m_Style |= WND_S_WINDOW_ALIGN_B_EDGE;
		m_Style &= ~WND_S_WINDOW_ALIGN_CENTERH;
	}
...
void KWndWindow::Paint()
{
	if (m_Style & WND_S_VISIBLE)
	{
		PaintWindow(); //ph輆 di child
		if (m_pFirstChild)
			m_pFirstChild->Paint();
		PaintOverChild(); //ph輆 tr猲 child
	}
	if (m_pNextWnd)
		m_pNextWnd->Paint();
}
