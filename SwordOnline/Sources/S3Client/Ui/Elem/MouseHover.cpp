...
************************************************************************/
void KMouseOver::SetMouseHoverTitle(const char *pTitleText, int nTitleTextLen, UINT uColor, BOOL bUfont, int nUSize, const char* pszUFontName)
{
	m_bUFont = bUfont;
	if(m_bUFont)
	{
		m_nFontSize = nUSize;
		strcpy(m_UFontName, pszUFontName);
	}
...
void KMouseOver::Update(int nX, int nY)
{
	...
	if (m_nTitleLen > 0)
	{
		if(!m_bUFont)
		{
			m_nTitleLineNum = TGetEncodedTextLineCountAE(
				m_ObjTitle, m_nTitleLen, 64, nMaxTitleLen, m_nFontSize, 0, 0, 0, m_nTitleLen);
		}
		else
		{
			m_nTitleLineNum = g_pRepresentShell->GetEncodedTextLineCount(m_UFontName,
				m_ObjTitle, m_nTitleLen, 384, nMaxTitleLen, m_nFontSize, 0, 0, 0, &m_nTitleLen);
		}
...
*/
	...
	if(!m_bUFont)
		...
		m_nWndWidth = m_nMaxLineLen + m_nIndent * 2;
	...
	if (m_bFollowCursor)
	{
		...
		if(m_nLeft + m_nWndWidth > w)
			m_nLeft = nX - m_nWndWidth;
		if(m_nTop + m_nWndHeight > h)
			m_nTop = nY - m_nWndHeight;
	}
	else
	{
...
void KMouseOver::LoadScheme(const char* pScheme)
{
	...
	if (Ini.Load(Buff))
	{
		...
		m_uTitleBgColor = ((GetColor(Buff) & 0xffffff) | 0xb0000000);

		Ini.GetString("Main", "PropBgColor", "0, 0, 0", Buff, sizeof(Buff));
		m_uPropBgColor = ((GetColor(Buff) & 0xffffff) | 0xb0000000);

		Ini.GetString("Main", "DescBgColor", "0, 0, 0", Buff, sizeof(Buff));
		m_uDescBgColor = ((GetColor(Buff) & 0xffffff) | 0xb0000000);
...
void KMouseOver::PaintMouseHoverInfo()
{
	...
	if(m_nTitleLen > 0)
	{
		...
		while(true)
		{
			if (TGetEncodedTextLineCount(m_ObjTitle, m_nTitleLen, m_nMaxLineLen, nLineLen, m_nFontSize, Param.nSkipLine, 1) == 0)
				break;
		...
		while(true)
		{
			if (g_pRepresentShell->GetEncodedTextLineCount(m_UFontName, m_ObjTitle, m_nTitleLen, m_nMaxLineLen, nLineLen, m_nFontSize, Param.nSkipLine, 1) == 0)
				break;
			//g_DebugLog("title [%d][%d][%d][%d] [%d]",Param.nSkipLine, m_nTitleLineNum, m_nMaxLineLen, m_nWndWidth, nLineLen);
			Param.nX = m_nLeft + m_nWndWidth/2 - nLineLen/2;
			g_pRepresentShell->DrawUFontRich(m_UFontName, m_nFontSize, &Param, m_ObjTitle, m_nTitleLen, 0);
			Param.nSkipLine ++;
			Param.nY += m_nFontSize + 1;
		}}
	}
