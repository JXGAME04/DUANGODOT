...
{
	"·¢ËÍÏûÏ¢",	"É¾³ý", "ÒÆ¶¯µ½ÆäËü×é"
};

#define FRIEND_INDEX	0
#define FRIEND_UNITNAME "B¹n h÷u"

#define BROTHER_INDEX	1
#define BROTHER_UNITNAME "Th©n h÷u"

#define ENEMY_INDEX		2
#define ENEMY_UNITNAME	 "Cõu nh©n"

#define MATE_INDEX		3
#define MATE_UNITNAME	 "Phèi ngÉu"

#define DEFAULT_GROUPNAME	""	//h¶o h÷u .. "cña t«i" (kh«ng cÇn thiÕt -> bá tr¾ng)
...
void ServerNotify::SendNotifyGroupFriend(const char* Unit, const std::string& group, const STRINGLIST& friends)
{
	...
	for (STRINGLIST::const_iterator itFriend = friends.begin(); itFriend != friends.end(); itFriend++)
	{
		...
		if (cursor + appendsize + 1 + 1 > max_packagesize)
		{
			...
			pExHeader->wLength = (WORD)(cursor - 1);
...
void KUiChatCentre::LoadScheme(class KIniFile* pIni)
{
	...
	InitWndAlign(WND_S_WINDOW_ALIGN_R_EDGE | WND_S_WINDOW_ALIGN_B_EDGE);
	KWndShowAnimate::AutoWndAlign();
}
...
void KUiChatCentre::OnSelGroupMenu(int nGroupIndex, int nCmd)
{
	...
	case GROUP_MENU_ITEM_RENAME:
		{
			...
			KUiGetString::OpenWindow("ÊäÈëÐÂµÄ×éÃû³Æ", Name,
	...
	case GROUP_MENU_ITEM_NEW:
		KUiGetString::OpenWindow("ÊäÈëÐÂ×éµÄÃû³Æ", "",
...
void KUiChatCentre::AdjustSize(int nNumLine)
{
	...
	m_Invisible.GetPosition(&x, &y);
	y += nOffsetY;
	m_Invisible.SetPosition(x, y);
