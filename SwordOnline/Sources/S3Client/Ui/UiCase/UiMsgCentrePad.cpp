...
int KSysMsgCentrePad::WndProc(unsigned int uMsg, unsigned int uParam, int nParam)
{
	...
	if (uMsg == WND_N_BUTTON_CLICK)
	{
		...
		else if (uParam == (unsigned int)(KWndWindow*)&m_UpButton)
		{
			nUpSpeed = 0;
			return 1;
		}
		else if (uParam == (unsigned int)(KWndWindow*)&m_DownButton)
		{
			nDownSpeed = 0;
			return 1;
		}
...
#define	SCHEME_INI_LEFT		"ÏûÏ¢¼¯ºÏÃæ°å_×ó.ini"
#define	SCHEME_INI_RIGHT	"ÏûÏ¢¼¯ºÏÃæ°å_ÓÒ.ini"
...
KUiMsgCentrePad::KUiMsgCentrePad()
{
	...
	m_nMinTopPos = m_nOrigMinTP = 20;
	m_nMaxBottomPos = m_nOrigMaxBP = 500;
...
void KUiMsgCentrePad::LoadScheme(KIniFile* pIni)
{
	...
	m_nOrigMinTP = m_nMinTopPos;
	m_nOrigMaxBP = m_nMaxBottomPos;
	...
	for (nCh = 0; nCh < MAX_CHANNELRESOURCE; nCh++)
	{
		...
		if (pIni->GetString("Channels", ChName, "", m_ChannelsResource[nCh].cTitle, 32))
		{
			...
			if(!strcmpi(m_ChannelsResource[nCh].cFormatName, "NEARBY"))
			{
				strcpy(m_ChannelsResource[nCh].cFormatName, s_keys[ch_Screen]);
			}
			else if(!strcmpi(m_ChannelsResource[nCh].cFormatName, "CITY"))
			{
				strcpy(m_ChannelsResource[nCh].cFormatName, s_keys[ch_City]);
			}
	...
	int XOffset, StartX, StartY;
	pIni->GetInteger("TabButtons", "XOffset", 60, &XOffset);
	pIni->GetInteger("TabButtons", "StartX", 0, &StartX);
	pIni->GetInteger("TabButtons", "StartY", 140, &StartY);
	for(int i=0; i<MAX_CHATTAB; ++i)
	{
		AddChild(&m_ChatTabButton[i]);
		sprintf(szImage, "ChatTabLabel_%d", i);
		pIni->GetString("ChatTab", szImage, defCT[i], ChName, sizeof(ChName));
		m_ChatTabButton[i].Init(pIni, "TabButton");
		m_ChatTabButton[i].SetText(ChName, strlen(ChName));
		m_ChatTabButton[i].SetPosition(StartX, StartY);
		StartX += XOffset;
	}
	m_ChatTabButton[0].CheckButton(TRUE);
	RECT rc;
	...
	InitWndAlign(WND_S_WINDOW_ALIGN_B_EDGE);
	Wnd_GetScreenSize(StartX, StartY);
	m_nMinTopPos += StartY - SCREEN_HEIGHT;
	m_nMaxBottomPos += StartY - SCREEN_HEIGHT;
}

void KUiMsgCentrePad::AutoWndAlign()
{
	KWndWindow::AutoWndAlign();
	int x,y;
	Wnd_GetScreenSize(x, y);
	m_nMinTopPos = m_nOrigMinTP + (y - SCREEN_HEIGHT);
	m_nMaxBottomPos = m_nOrigMaxBP + (y - SCREEN_HEIGHT);
}
...
int	KUiMsgCentrePad::PtInWindow(int x, int y)
{
	...
	if (KWndWindow::PtInWindow(x, y))
	{
		...
		if(!nRet)
		{
			for(int i=0; i<MAX_CHATTAB; ++i)
			{
				nRet = m_ChatTabButton[i].PtInWindow(x, y);
				if(nRet)
					break;
			}
		}
...
int KUiMsgCentrePad::WndProc(unsigned int uMsg, unsigned int uParam, int nParam)
{
	...
	case WND_N_BUTTON_CLICK:
		...
		else
		{
			...
			for(int i=0; i<MAX_CHATTAB; ++i)
			{
				if (uParam == (unsigned int)(KWndWindow*)&m_ChatTabButton[i])
				{
					if(m_ChatTabButton[i].IsButtonChecked())
						break;
					for(int j=0; j<MAX_CHATTAB; ++j)
					{
						m_ChatTabButton[j].CheckButton(FALSE);
					}
					m_ChatTabButton[i].CheckButton(TRUE);
					nClick = i;
					break;
				}
			}
			if(nClick >= 0) //id nót kªnh võa ®­îc click
			{
...
void KUiMsgCentrePad::DragWndSize(int nMoveOffset)
{
	...
	else
	{
		...
		if (ChangeHeight(m_nSizeBtnDragOffsetY, m_bSizeUp))	//¸Ä±äHeight³É¹¦ºó£¬¾Í²»ÓÃ»ýÀÛÁË
			m_nSizeBtnDragOffsetY = 0;
	}
}

int KUiMsgCentrePad::ChangeHeight(int nOffsetY, bool bAtTop)
{
	if (bAtTop)
	...
	if (nOffsetY != 0)
	{
		int x, y;
		GetSize(&x, &y);
		if (y + nOffsetY >= m_ChatRoom.GetMinHeight() + 20)
		{
			if (bAtTop)
			{
...
int KUiMsgCentrePad::CheckChannel(int nChannelIndex, bool b)
{
	...
	if (n >= 0)
	{
		...
		SystemMessageArrival(Buffer, (unsigned short)strlen(Buffer));
	}
	return 1;
}
