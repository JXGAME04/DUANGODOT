...
#define	MSG_RENASCENCE		"ÄãÒÑ¾­ÉíÊÜÖØÉË£¡"
#define	MSG_BACK_TO_TOWN	"»Ø³ÇÐÝÑø"
...
int KUiSysMsgCentre::WndProc(unsigned int uMsg, unsigned int uParam, int nParam)
{
	...
	case WND_N_BUTTON_CLICK:
		for (i = 0; i < MAX_SYS_MSG_TYPE; i++)
		{
			...
			{
				DeleteMsgInHeap(0, 0, (uMsg == WND_N_BUTTON_MR_DOWN), true);
				break;
			}
		}
		break;
	case WND_N_MOUSE_ENTER_CHILD:
		for (i = 0; i < MAX_SYS_MSG_TYPE; i++)
		{
			if (uParam == (unsigned int)(KWndWindow*)&m_MsgIconBtn[i])
			{
				...
				if (m_MsgHeap[0].nNumValid)
				{
					m_MsgTextWnd.SetText(m_MsgHeap[0].pMsgList[0]->szMessage);
					m_bShowMsgText = true;
				}
				break;
			}
...
bool KUiSysMsgCentre::AMessageArrival(KSystemMessage* pMsg, void* pParam)
{
	...
	{
		KUiMsgCentrePad::SystemMessageArrival(pMsg->szMessage, (unsigned short)strlen(pMsg->szMessage));	// ¼ÓÈëÒ»ÌõÏûÏ¢
		return true;
	}
	...
	if (pCloneMsg)
	{
		...
		{
			pCloneMsg->uReservedForUi = IR_GetCurrentTime();
...
void KUiSysMsgCentre::ConfirmMsg(KSystemMessage* pMsg, bool bImmedDel)
{
	...
	case SMCT_UI_TEAM_INVITE:	//´ðÓ¦»ò¾Ü¾ø¼ÓÈë¶ÓÎéµÄÑûÇë
		...
		sprintf(szBuf, "%sÑûÇëÄã¼ÓÈëËûµÄ¶ÓÎé", pPlayer->Name);
		pFirstBtnText = "Í¬Òâ";
		pSecBtnText = "¾Ü¾ø";
	...
	case SMCT_UI_TEAM_APPLY:	//´ðÓ¦»ò¾Ü¾ø¼ÓÈë¶ÓÎéµÄÉêÇë
		_ASSERT (pMsg->byParamSize >= sizeof(KUiPlayerItem));
		pPlayer = (KUiPlayerItem*)(&pMsg[1]);
		sprintf(szBuf, "%sÉêÇë¼ÓÈëÄãµÄ¶ÓÎé", pPlayer->Name);
		pFirstBtnText = "Í¬Òâ";
		pSecBtnText = "¾Ü¾ø";
		_ASSERT(m_pHandlingMsg == NULL);
		m_pHandlingMsg = pMsg;
	...
	case SMCT_UI_TRADE:
		_ASSERT (pMsg->byParamSize >= sizeof(KUiPlayerItem));
		pPlayer = (KUiPlayerItem*)(&pMsg[1]);
		sprintf(szBuf, "%sÏ£ÍûÓëÄã½»Ò×", pPlayer->Name);
		pFirstBtnText = "Í¬Òâ";
		pSecBtnText = "¾Ü¾ø";
		_ASSERT(m_pHandlingMsg == NULL);
		m_pHandlingMsg = pMsg;
	...
	case SMCT_UI_TONG_JOIN_APPLY:
		_ASSERT (pMsg->byParamSize >= sizeof(KUiPlayerItem));
		pPlayer = (KUiPlayerItem*)(&pMsg[1]);
		sprintf(szBuf, "%s xin gia nhËp bang héi", pPlayer->Name);
		pFirstBtnText = "ChÊp nhËn";
		pSecBtnText = "Tõ chèi";
		_ASSERT(m_pHandlingMsg == NULL);
		m_pHandlingMsg = pMsg;
		pMsg = NULL;
