...
KUiInit* KUiInit::OpenWindow(bool bStartMusic, bool bJustLaunched)
{
	...
	if (m_pSelf)
	{
		if (bJustLaunched)
		{
			...
			//KUiOptions::LoadSetting(true, true);//load option from uicommon.ini
			...
			if (bStartMusic)
				PlayTitleMusic();
			KUiOptions* p = KUiOptions::OpenWindow();
			if(p)
				p->TempHide();
		}
		else
