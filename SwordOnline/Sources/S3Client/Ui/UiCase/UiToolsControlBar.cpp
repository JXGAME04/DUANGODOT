...
void KUiToolsControlBar::LoadScheme(const char* pScheme)
{
	...
	if (m_pSelf)
	{
		...
		if (Ini.Load(Buff))
		{
			...
			Ini.GetInteger("Main", "PosOffset",  0, &m_pSelf->m_nPosOffset);
			Ini.GetInteger("Main", "PBWidth",  0, &m_pSelf->m_nPBWidth);
			int x,y,w,h;
			m_pSelf->GetPosition(&x, &y);
			Wnd_GetScreenSize(w, h);
			m_pSelf->SetPosition(w/2 - m_pSelf->m_nPBWidth/2 + m_pSelf->m_nPosOffset, y);
		}
	}
}
...
void KUiToolsControlBar::AutoWndAlign()
{
	KWndWindow::AutoWndAlign();
	int x,y,w,h;
	GetPosition(&x, &y);
	Wnd_GetScreenSize(w, h);
	SetPosition(w/2 - m_nPBWidth/2 + m_nPosOffset, y);
}
