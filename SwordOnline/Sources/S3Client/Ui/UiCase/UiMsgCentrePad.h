...
#define MAX_SHORTNAME 3
#define MAX_CHATTAB 6
struct KChannelResourceInfo
{
	char cTitle[32];
	char cShortTitle[MAX_SHORTNAME][32];
	KRColor uTextColor;
...
private:
	...
	void	Initialize();							//初始化
	int		WndProc(unsigned int uMsg, unsigned int uParam, int nParam);//窗口函数
	int		ChangeHeight(int nOffsetY, bool bAtTop);
...
private:
	...
	KWndPureTextBtn		m_ChatTabButton[MAX_CHATTAB];
	int					m_nMinTopPos, m_nMaxBottomPos, m_nOrigMinTP, m_nOrigMaxBP;
	bool				m_bSizingWnd;
