...
#define GAME_LOGO			"½£ÏÀÇéÔµ¡¤ÍøÂç°æ"
...
void GameWorld_DateTime::PaintWindow()
{
	KRUShadow	rect;
	rect.Color.Color_dw = 0x64101010;
	rect.oEndPos.nX = rect.oPosition.nX = m_nAbsoluteLeft;
	rect.oEndPos.nY = rect.oPosition.nY = m_nAbsoluteTop;
	rect.oEndPos.nX += m_Width;
	rect.oEndPos.nY += m_Height+2;
	g_pRepresentShell->DrawPrimitives(1, &rect, RU_T_SHADOW, true);
	KWndLabeledButton::PaintWindow();
}
...
//--------------------------------------------------------------------------
static GameWorld_DateTime*		m_DateTime = NULL;
...
KUiPlayerBar::KUiPlayerBar()
{
	...
	m_nImgHeight = 55;
	m_nImgWidth = 1048;
	strcpy(m_Image.szImage, "\\Spr\\Ui3\\Ö÷½çÃæ\\Ö÷½çÃæÐÂ°æ.spr");
	m_Image.nType = ISI_T_SPR;
	m_Image.Color.Color_b.a = 255;
	m_Image.bRenderStyle = IMAGE_RENDER_STYLE_ALPHA;
	m_Image.bRenderFlag = 0;
	m_Image.uImage = 0;
	m_Image.nFrame = 0;
	m_Image.nISPosition = -1;
}
...
//	_ASSERT(pIni);
	...
	pIni->GetInteger("Main", "ImageW", 0, &m_nImgWidth);
	pIni->GetInteger("Main", "ImageH", 0, &m_nImgHeight);
...
{
	m_DateTime = new GameWorld_DateTime;
	//AddChild(&m_DateTime);
	AddChild(&m_Face);
	AddChild(&m_EscDlg);
	AddChild(&m_Friend);
...
void KUiPlayerBar::PaintWindow()
{
	...
	int w,h;
	GetSize(&w, &h);
	m_Image.oPosition.nX = m_nAbsoluteLeft + w/2 - m_nImgWidth/2;
	m_Image.oPosition.nY = m_nAbsoluteTop + h - m_nImgHeight;
...
void KUiPlayerBar::OnSendChannelMessage(DWORD nChannelID, const char* Buffer, int nLen)				//·¢ËÍÆµµÀÁÄÌìµ½·þÎñÆ÷
{
	if (nChannelID != -1 && Buffer && nLen > 0)
	{
		...
		if (nLeft < 0)
		{
			char szWarning[] = "Ç§Àï´«ÒôÎÞ·¨Á¬ÐøÊ¹ÓÃ£¬ÇëÄúÏÈÔËÏ¢Æ¬¿Ì¡£";
			KUiMsgCentrePad::SystemMessageArrival(szWarning, sizeof(szWarning));
		}
		else if (nLeft > 0)
		{
			char szWarning[64];
			sprintf(szWarning, "ÏûÏ¢½«ÔÚ%dÃëºó·¢³ö!", (nLeft + 999 ) / 1000);
			KUiMsgCentrePad::SystemMessageArrival(szWarning, (unsigned short)(strlen(szWarning) + 1));
		}
	}
}
...
void KUiPlayerBar::Breathe()
{
	...
	while(KUiMsgCentrePad::GetChannelData(dwID, cost, Buffer, nLen))
	{
		...
		if (!bSend)
		{
			...
			{
				if(nUseMana > 0)
					sprintf(szSystem, "Sö dông thiªn lý truyÒn ©m cÇn tiªu hao néi lùc %d ®iÓm", nUseMana);
			}
			else if (nRet == 2 || nRet == 3)
			{
				if(nUseLevel > 0)
					sprintf(szSystem, "CÇn ®¹t cÊp %d trë lªn ®Ó sö dông thiªn lý truyÒn ©m", nUseLevel);
				else if(nUseMana > 0)
					sprintf(szSystem, "Sö dông thiªn lý truyÒn ©m cÇn tiªu hao néi lùc %d ®iÓm", nUseMana);
			}
		}
		...
		{
			if (nUseMana > 0)
				sprintf(szSystem, "Sö dông thiªn lý truyÒn ©m, tiªu hao néi lùc %d ®iÓm", nUseMana);
		}
		...
		if (szSystem[0])
			KUiMsgCentrePad::SystemMessageArrival(szSystem, (unsigned short)strlen(szSystem));
	}
...
{
	if (cost == 0)//khong ton gi
	{
		nUseMoney = 0;
		nUseLevel = 0;
		nUseMana = 0;
	}
	else if (cost == 1)//10% mana
	{
	...
	else if (cost == 2)//2: 20Lv + 20% mana
	{
		...
		if (nMana < nFullMana / 5)
		{
			nUseMana = nFullMana / 5;
			return 2;
		}
		nUseMoney = 0;
		nUseLevel = 0;
